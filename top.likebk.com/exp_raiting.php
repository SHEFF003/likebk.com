...
$p = mysql_fetch_array(mysql_query('SELECT `id`,`login`,`level`,`sex`,`clan`,`align`,`city`,`cityreg` FROM `users` WHERE `id` = "'.mysql_real_escape_string($_GET['user']).'" LIMIT 1'));
	...
	$sort = '`st`.`exp`';
	$sp = mysql_query('SELECT * FROM `users` AS `us` LEFT JOIN `stats` AS `st` ON (`us`.`id` = `st`.`id`) WHERE `us`.`real` = 1 AND `us`.`pass` != "saintlucia" AND (`us`.`pass` NOT LIKE "%saintlucia%" OR `us`.`pass` NOT LIKE "%saintlucia5%") ORDER BY '.$sort.' DESC LIMIT 250');
while( $pl = mysql_fetch_array($sp) ) {
	...
	//рейтинг
	$r .= '<td align="center" height="20" align=right valign="top" class="mystrong">'.number_format($pl['exp'],0,'',' ').'</td>';
	//
	$r .= '</tr>';	
	$i++;
}
