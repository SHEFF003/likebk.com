...
var blockloaddata = false;
var blockloaddata2 = false;
...
function usepriem(id,t)
{
	...
	{
		irn++;
		/*if( blockloaddata2 == true ) {
			console.log('block query battle JX');
		}else*/ 
		//if( top.c.noEr == 0 || true == true ) {
		//	top.c.noEr = 0; //clearTimeout(top.c.noErTmr); /*top.c.noErTmr = setTimeout('top.c.noEr = 0;',1000);*/
		//	if( top.testadmin != undefined ) {
				blockloaddata = true;
				$.post(
					"jx/battle/refresh" + server_fight + ".php?rnd="+ggcode+"&irn="+irn,
					{idlog:top.id_log,usepriem:id,useon:use_on_pers},
					function(data,a,b){
						if( a == 'success' && b.status == '200' ) {
							blockloaddata = false;
							$("#ref").html(data);
							if( battleClass != undefined ) { battleClass.enemyReflesh(); }
						}else{
...
function useitem(id,t,use_item_on)
{
	...
	{
		irn++;
		if( blockloaddata == true ) {
			console.log('block query battle JX');
		}else if( top.c.noEr == 0 || true == true ) {
			top.c.noEr = 0; //clearTimeout(top.c.noErTmr); /*top.c.noErTmr = setTimeout('top.c.noEr = 0;',1000);*/
			if( top.testadmin != undefined ) {
				blockloaddata = true;
				$.post(
					"jx/battle/refresh" + server_fight + ".php?rnd="+ggcode+"&irn="+irn,
					{idlog:top.id_log,useitem:id,useitemon:use_item_on},
					function(data,a,b){
						if( a == 'success' && b.status == '200' ) {
							blockloaddata = false;
							$("#ref").html(data);
				...
				$.post("jx/battle/refresh" + server_fight + ".php?rnd="+ggcode+"&irn="+irn,{idlog:top.id_log,useitem:id,useitemon:use_item_on},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});
			}
		}
		top.goSit(1);
	}else{
  ...
  if( ( battle_end==0 || bl!=null ) && battleFinishData == -1 )
  {
	...
	if( blockloaddata == true ) {
		...
		if( top.testadmin != undefined ) {
			blockloaddata = true;
			$.post(
			"jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,
			{id:'reflesh',idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login,ldrl:leader_login,ldrt:leader_type},
			function(data,a,b){
				if( a == 'success' && b.status == '200' ) {
					blockloaddata = false;
					$("#ref").html(data);
			...
			$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{id:'reflesh',idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login,ldrl:leader_login,ldrt:leader_type},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});	
		}
	}
	if(g_iCount!=45)
    {
...
$(document.body).bind('keyup',function(e){ 
  if (e.keyCode == 32) {
  	var j = 1;
	while(j <= 3 ){
		if(!($('#mainpanel #zatack'+j+'_1').attr('style'))) {
			var sel_atack = Math.floor(Math.random(5)*5+1);
			zas[j] = sel_atack;
		} 
		j++;
	}
	var sel_block = Math.floor(Math.random(5)*5+1);
	zbs[1] = sel_block;
	...
	if( blockloaddata == true ) {
		...
		if( top.testadmin != undefined ) {
			...
			$.post(
				"jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,
				{atack:ago,block:bgo,idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login},
				function(data,a,b){
					if( a == 'success' && b.status == '200' ) {
						blockloaddata = false;
						$("#ref").html(data);
			...
			$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{atack:ago,block:bgo,idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});	
		}
	}
  } 
  return false;
...
function autoatack(){
	// select_atack(sel_atack,i);
	// change_radio(1,jj,'block');
	var j = 1;
	while(j <= 3 ){
		if(!($('#mainpanel #zatack'+j+'_1').attr('style'))) {
			var sel_atack = Math.floor(Math.random(5)*5+1);
  ...
  irn++;
	if( blockloaddata == true ) {
		...
		if( top.testadmin != undefined ) {	
			...
			$.post(
				"jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,
				{atack:ago,block:bgo,idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login,ldrl:leader_login,ldrt:leader_type},
				function(data,a,b){
					if( a == 'success' && b.status == '200' ) {
						blockloaddata = false;
						$("#ref").html(data);
			...
			$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{atack:ago,block:bgo,idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login,ldrl:leader_login,ldrt:leader_type},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});	
		}
	}
}
...
function startCountdown()
{
 ...
 { 
  g_iCount = g_iCount - 1;
  console.log('REF.BATTLE#2-1');
  tmr0057 = setTimeout('startCountdown();console.log("REF.BATTLE#2-2");',1000);
 }else{
  reflesh();
  console.log('REF.BATTLE#1-1');
  tmr0057 = setTimeout('startCountdown();console.log("REF.BATTLE#1-2");',1000);
 }
}
...
function mainstatus(id)
{
  ...
  }else if(id==1) //Можно ударить противника
  {
	...
	document.getElementById('go_auto').style.display = '';
   if(document.getElementById('reflesh_btn')!=undefined)
   {
   	  document.getElementById('wait_progress').style.display = 'none';
	  document.getElementById('reflesh_btn').style.display = 'none';
  ...
  }else if(id==2) //Ожидаем хода противника
  {
	...
	document.getElementById('go_btn').style.display = 'none';
	document.getElementById('go_auto').style.display = 'none';
	document.getElementById('wait_progress').style.display = '';
...
function rand_img()
{
	if(level<4)
	{
	img_battle = "<img src='http://img.likebk.com/i/battle/"+(Math.floor(Math.random(2)*2))+".gif'>";
	} else {
	img_battle = "<img src='http://img.likebk.com/i/battle/"+(Math.floor(Math.random(29)*29))+".jpg'>";
	}
}
var fstlh = 0;
var lsti = 0;
...
function add_log(id,foryou,text,hod_id,my,last_hod,vars)
{
	...
	//{
		...
		//id_log_ar[id] = id;
		//id_log = id;
		if(document.getElementById("battle_log_"+hod_id+"")==undefined && hod_id!=1)
		{
			if(fstlh==0)
			{
				fstlh = hod_id;
			...
			document.getElementById('battle_logg').innerHTML = document.getElementById('battle_logg').innerHTML + '<div style="padding-top:7px;padding-bottom:7px;" id="battle_log_'+hod_id+'" class="battle_hod_style"></div>';
		} else if (document.getElementById("battle_log_"+hod_id+"")==undefined)
		{
			document.getElementById('battle_logg').innerHTML = '<div style="padding-top:7px;padding-bottom:7px;" id="battle_log_'+hod_id+'"></div>' + document.getElementById('battle_logg').innerHTML;
		}
		document.getElementById("battle_log_"+hod_id+"").innerHTML = document.getElementById("battle_log_"+hod_id+"").innerHTML+'<span id="log_id_'+id+'" class="foryou'+foryou+'">'+text+'</span><br>';
	//}
	/*if(document.getElementById("battle_log_"+(hod_id-5))!=undefined)
	{
		var list = document.getElementById("battle_logg");   // Get the <ul> element with id="myList"
		var txt = document.getElementById("battle_log_"+(hod_id-5));   // Get the <ul> element with id="myList"
