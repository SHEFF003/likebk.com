$(document).ready(function(){
	...
	//alert(heightB);
	if(heightB < 480){
		$(".no_chat_mob").css({'display': 'none'});
		$("#reline1").css({'display': 'none'});
		$("#reline2").css({'display': 'none'});
		$("#touchmain").css({'height': '100%'});
		$("iframe#main").css({'height': '100%'});
	}
...
var lastsmuse = ['','','','',''];
function lastSml(id) {
	 if( id != top.lastsmuse[0] && id != top.lastsmuse[1] && id != top.lastsmuse[2] && id != top.lastsmuse[3] && id != top.lastsmuse[4] ) {
		 size = 5;
		 temp = id;
		 for( i = 1; i < size; i++) {
		   top.lastsmuse[i-1] = top.lastsmuse[i];
		 }
		 top.lastsmuse[size-1] = temp;
		 bodyLoaded();
	 }
}
...
function bodyLoaded()
{
	...
	var i2 = 0;
	while( i2 != -1 ) {
		if( top.addsmls[i2] != undefined ) {
			j += '<img style="cursor:pointer" onclick="chat.addSmile(\''+top.addsmls[i2]+'\')" src="http://'+c['img']+'/i/smile/'+top.addsmls[i2]+'.gif" title=":'+top.addsmls[i2]+':" /> ';
		}else{
			i2 = -2;
		}
		i2++;
	...
	if( j != '' ) {
		j = '<div align="center" style="background-color:#a5a5a5;padding:5px;">'+j+'</div>';
	}
...
function infoMenu(u,e,f)
{
	...
	if(d!=undefined)
	{
		...
		var hmmmt = '<DIV onmouseover="top.doTest = false;" onmouseout="top.doTest = true; top.MBcMenu();">'
						...
						+'<div onClick="copyText(\''+u+'\'); top.infoMenuClose(4);" class="topusermenuBtn">COPY</div>';
	...
	add:function(id,title,text,date,type,style,css){
		...
		if($('#win_'+id).attr('id') == undefined){	
			...
			if(type==0){
				...
				//Да \ Нет
				nw = '<div>'+text+'</div><div style="padding:5px"><div style="float:left"><button onClick="'+date.a1+';win.closew(\''+id+'\');" class="btnnew" style="width:100px">Да</button></div><div style="float:right"><button class="btnnew" onClick="'+date.a2+';win.closew(\''+id+'\')" style="width:100px">Нет</button></div><br></div>';
				kyps[0] = ''+date.a1+';top.win.closew(\\\''+id+'\\\');top.win.addaction(0,\\\'\\\');';
			}else if(type==3){
				//Да \ Нет , изображения
			...
			if(title != ''){
				nw = '<div class="wi'+style+'s10" onselectstart="return false">'+
					 ...
					 '<td width="15" align="right"><img style="display:block" onClick="win.closew(\''+id+'\')" src="http://'+c.img+'/i/clear.gif" width="13" height="13"></td>'+
					 '</tr>'+
					 '</table>'+
				     '</div>'+nw;
			}
			...
			//$('#win_'+id).center();
			var w = $(window);
    		$('#win_'+id).css("position","absolute");
    		//$('#win_'+id).css("top",(w.height()-$('#win_'+id).height())/2+w.scrollTop() + "px");
    		$('#win_'+id).css("left",(w.width()-$('#win_'+id).width())/2+w.scrollLeft() + "px");
			$('#win_'+id).css({'top': 140});
			if(type == 2 || type == 3) {
