...
var chat = {
	...
	ignore:function(login)
	{
		if(this.ignoreList[login]!=undefined)
		{
			...
			if($('#ignr_alu').attr('id')!=undefined)
			{
				$('#ignr_alu').html($('#ignr_alu').html()+'<div id="ignr_u_'+this.ignoreList.x+'">    <b>'+login+'</b> <a target="_blank" href="../inf.php?login='+login+'"><img src="http://'+top.c.img+'/i/inf_capitalcity.gif" title="Инф. о '+login+'"></a>   <small><a href="javascript:void(0)" onclick="chat.ignorUn('+this.ignoreList.x+')">Clear</a></small>  </div>');
			}
			//msg
	...
	socketOnline:function(data) {
			...
			if( data[i] != undefined ) {
				...
				if( data[i].room == c.room ) {
					...
					var userArr = [
						...
						0, //banned
						0, //молчанка
						data[i].battle, //поединок
						mtk, //метка
						0, //травма
						0, //пол
						data[i].dealer  //алхимик
					];
					//
					if( userArr[16] == 0 ) {
		...
		var msgtop = '';
		//
		msgtop = '<small><a href="http://likebk.com/buttons.php">Включить стандартный чат</a></small><hr>';
		msgtop = '';
		//
	...
	ignorListOpen:function()
	{
		...
		while(j<=this.ignoreList.x)
		{
			...
			{
				date += '<div id="ignr_u_'+j+'">    <b>'+this.ignoreList.nms[j]+'</b> <a target="_blank" href="/inf.php?login='+this.ignoreList.nms[j]+'"><img src="http://'+top.c.img+'/i/inf_capitalcity.gif" title="Инф. о '+this.ignoreList.nms[j]+'"></a>   <small><a href="javascript:void(0)" onclick="chat.ignorUn('+j+')">Clear</a></small>  </div>';
			}
			j++;
		}
	...
	gUser:function(data,ol)
	{
		...
		if(data[1]!=undefined)
		{
			...
			if(data[16] == 1){
				rt = '<img height="15" title="Алхимик" src="http://'+top.c.img+'/i/align/align50.gif"> '+rt;
			}
			...
			rt += '['+data[2]+']<a href="http://'+c.url+'/inf.php?'+data[0]+'" target="_blank"><img style="vertical-align:baseline" width="12" height="11" src="http://'+top.c.img+'/i/inf_'+data[5]+'.gif" title="Инф. о '+data[1]+'"></a>';
					...
					if(((smid != false || smid == 0) && this.isNumber(txr[i]) != true) || (imsml != undefined && imsml[0] == '%usersmile%')) {
						if(j < 3 && this.isNumber(top.sml[smid]) != true && (top.sml[smid] != undefined || imsml[0] == '%usersmile%')) {
							if(imsml[0] == '%usersmile%') {
								...
								txt = txt.replace("\:%usersmile%-"+imsml[1]+"\:",'<img src="http://'+top.c.img+'/i/smile/'+(imsml[1].toLowerCase())+'.gif" style="max-height:50px;" title="Именной смайлик">');
							}else{
