...
function usepriem(id,t)
{
	if(t==1)
	{
		...
		if( top.c.noEr == 0 ) {
			...
			$.post("jx/battle/refresh" + server_fight + ".php?rnd="+ggcode+"&irn="+irn,{idlog:top.id_log,usepriem:id,useon:use_on_pers},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});
		}
...
function reflesh(bl)
{
  ...
  if( ( battle_end==0 || bl!=null ) && battleFinishData == -1 )
  {
	...
	if( top.c.noEr == 0 ) {
		top.c.noEr = 1; clearTimeout(top.c.noErTmr); /*top.c.noErTmr = setTimeout('top.c.noEr = 0;',1000);*/
		$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{id:'reflesh',idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login},function(data){$("#ref").html(data);if( battleClass != undefined ) { battleClass.enemyReflesh(); }});	
	}
	if(g_iCount!=45)
    {
      g_iCount = 45;
...
function add_log(id,foryou,text,hod_id,my,last_hod,vars)
{
	...
	//{
		...
		if(document.getElementById("battle_log_"+hod_id+"")==undefined && hod_id!=1)
		{
			...
			document.getElementById('battle_logg').innerHTML = '<div style="padding-top:7px;padding-bottom:7px;" id="battle_log_'+hod_id+'" class="battle_hod_style"></div>'+document.getElementById('battle_logg').innerHTML;
		...
		document.getElementById("battle_log_"+hod_id+"").innerHTML = '<span id="log_id_'+id+'" class="foryou'+foryou+'">'+text+'</span><br>'+document.getElementById("battle_log_"+hod_id+"").innerHTML;
	//}
