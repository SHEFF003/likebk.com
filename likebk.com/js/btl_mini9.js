...
function useitemHeal(id,t,use_item_on)
{
	if(t==1)
	{
		irn++;
		if( top.c.noEr == 0 ) {
			...
			$.post("jx/battle/refresh_heal.php?rnd="+ggcode+"&irn="+irn,{idlog:top.id_log,useitem:id,useitemon:use_item_on},function(data){$("#ref").html(data);});
		}
		top.goSit(1);
	}else{
		alert('Not found');
  ...
  if (e.keyCode == 32) {
	...
	if( blockloaddata == true ) {
		...
		if( top.testadmin != undefined ) {
			...
			$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{atack:ago,block:bgo,idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login},function(data){$("#ref").html(data);});	
		}
	}
  } 
  return false;
