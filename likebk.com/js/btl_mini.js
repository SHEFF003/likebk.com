...
var blockbattle = false;

function blockbattlefx(val) {
...
function reflesh(bl)
{
  ...
  if( ( battle_end==0 || bl!=null ) && battleFinishData == -1 )
  {
	...
	if( top.c.noEr == 0 ) {
		top.c.noEr = 1; clearTimeout(top.c.noErTmr); /*top.c.noErTmr = setTimeout('top.c.noEr = 0;',1000);*/
		$.post("jx/battle/refresh" + server_fight + ".php?irn="+irn+"&rnd="+ggcode,{id:'reflesh',idlog:top.id_log,idpr:priem_use,mgid:magic_use,useon:use_on_pers,smn:smena_login},function(data){$("#ref").html(data);});	
	}
	if(g_iCount!=45)
    {
      g_iCount = 45;
...
function startCountdown()
{
 ...
 { 
  g_iCount = g_iCount - 1;
  //console.log('REF.BATTLE#1');
  tmr0057 = setTimeout('startCountdown()',1000);
 }else{
  reflesh();
  //console.log('REF.BATTLE#2');
  tmr0057 = setTimeout('startCountdown()',1000);
 }
}
