...
<script src="http://img.agebk.com/js/Lite/gameEngine.js" type="text/javascript"></script>
<script src="js/jquery-1.11.3.min.js"></script>
<script src="js/ion.rangeSlider.js"></script>
<script type="text/javascript" src="js/title.js"></script>
<link href="http://img.agebk.com/css/main.css" rel="stylesheet" type="text/css">
...
<br>
<center>
<form method="post" action="battle_bot.php">
<?
//

if(isset($_POST['botadminatack2'])) {
	$error = '';	
	$user1 = mysql_fetch_array(mysql_query('SELECT * FROM `users` WHERE `id` = "'.mysql_real_escape_string($_POST['botadminatack1']).'" LIMIT 1'));
	$user2 = mysql_fetch_array(mysql_query('SELECT * FROM `users` WHERE `id` = "'.mysql_real_escape_string($_POST['botadminatack2']).'" LIMIT 1'));
	if(isset($user1['id'],$user2['id'])) {
		...
		$user2['loclon555'] = 1;
		//
		$_POST['kp1'] = (int)$_POST['kp1'];
		$_POST['kp2'] = (int)$_POST['kp2'];
		$_POST['kp3'] = (int)$_POST['kp3'];
		$_POST['kp4'] = (int)$_POST['kp4'];
		mysql_query('INSERT INTO `battle` (`kp1`,`kp2`,`kp3`,`kp4`,`start1` , `city` , `time_start` , `timeout` , `type` ) VALUES (
			"'.$_POST['kp1'].'","'.$_POST['kp2'].'","'.$_POST['kp3'].'","'.$_POST['kp4'].'","'.time().'","capitalcity" , "'.time().'" , "60" , "329"
		)');		
		$logid = mysql_insert_id();		
		//
		$user1['login'] = $user1['login'].' (Клон '.$logid.')';
		$user2['login'] = $user2['login'].' (Клон '.$logid.')';
		//
		$bot1 = $u->addNewbot(0,NULL,$user1,false,false);
		$bot2 = $u->addNewbot(0,NULL,$user2,false,false);		
		mysql_query('UPDATE `users` SET `name` = 1,`battle` = "'.$logid.'" WHERE `id` = "'.$bot1.'" LIMIT 1');
		mysql_query('UPDATE `users` SET `name` = 2,`battle` = "'.$logid.'" WHERE `id` = "'.$bot2.'" LIMIT 1');
		...
		mysql_query('INSERT INTO `battle_logs` (
			...
			"'.time().'","'.$logid.'","1","1","Тестовый бой начался в <b>'.date('d.m.Y H:i:s').'</b>"
		)');
		//
		//Эффекты
		$i = 0;
		...
		function inuser_go_btl($id) {
			if(isset($id['id'])) {
				file_get_contents('http://agebk.com/jx/battle/refresh.php?uid='.$id['id'].'&cron_core='.md5($id['id'].'_brfCOreW@!_'.$id['pass']).'&pass='.$id['pass']);
			}
		}
		...
		//
		$error = 'Бой между <u>'.$user1['login'].'</u> ['.$user1['level'].'] <a  target="_blank" href="/inf.php?'.$bot1.'"><img src="http://img.agebk.com/i/inf_capitalcity.gif"></a> и <u>'.$user2['login'].'</u> ['.$user2['level'].'] <a href="/inf.php?'.$bot2.'" target="_blank"><img src="http://img.agebk.com/i/inf_capitalcity.gif"></a> начался! <a href="/logs.php?log='.$logid.'" target="_blank">Лог боя</a>';
	}else{
...
while( $pl = mysql_fetch_array($sp) ) {
	...
	//
	$ef = '<img onmouseover="hi(this,\'<b>'.$pl['mname'].'</b>'.$ei.'\',event,3,1,1,1,\'\');" onMouseOut="hic();" onMouseDown="hic();" src="http://img.agebk.com/i/eff/'.$pl['img'].'" width="40" height="25">';
...
$sp_m = mysql_query('SELECT * FROM `users` WHERE `icq` = "19734682" ORDER BY `id` ASC');
...
<table width="700" border="0" cellpadding="10" cellspacing="10">
  ...
  <tr>
    <td align="center" valign="middle"><p>Стабильный %:</p>
      <p>Крит: 
        <input name="kp1" type="text" value="-1">
      </p>
      <p>Уворот: 
        <input name="kp2" type="text" value="-1">
      </p></td>
    <td align="center" valign="middle"><br><br><input class="btnnew" type="submit" value="Начать!"></td>
    <td align="center" valign="middle"><p>Стабильный %:</p>
      <p>Крит: 
        <input name="kp3" type="text" value="-1">
      </p>
      <p>Уворот: 
        <input name="kp4" type="text" value="-1">
      </p></td>
  </tr>
  </table>
<br>
