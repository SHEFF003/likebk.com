...
if( isset($_GET['cookie_login']) && $_GET['cookie_login'] != '' ) {
	setcookie('login',$_GET['cookie_login'], 0 ,'',$c['host']); // вместо аргумента 0 тут было time()+60*60*24*7
	setcookie('pass',$_GET['cookie_pass'], 0 ,'',$c['host']); // вместо аргумента 0 тут было time()+60*60*24*7
...
if(isset($_GET['um'])) {
	...
	if(isset($u2['id'])) {
		if( md5($u2['time']) != $_GET['md5'] ) {
			error('Неверный ключ разморозки! Попробуйте авторизировать с главной страницы заново!');
		}else{
			...
			error('Персонаж &quot;'.$u2['login'].'&quot; был успешно разморожен! Авторизируйтесь с главной страницы снова!');
		}
	}else{
		error('Возможно персонаж был разморожен ранее.');
	}
}elseif(!isset($u['id']))
{
	...
	if(isset($u2['id'])) {
		if( $u2['pass'] == md5($_POST['pass'])) {
			error('Персонаж "'.$u2['login'].'" заморожен с '.date('d.m.Y',$u2['time']).'. Если вы хотите разморозить персонажа, нажмите здесь: <a href="/enter.php?um='.$u2['id'].'&md5='.md5($u2['time']).'">Разморозить</a>.');
		}else{
			error('Персонаж "'.$_POST['login'].'" заморожен. Вы указали неверный пароль.');
		}
	}else{
...
}elseif($u['banned']>0)
{
	...
	if( $u['2pass'] != '' && $u['2pass'] != '0' ) {
		...
		if( $_POST['code2'] == $key1 ) {
			...
			$koko = '<center>Неправильный код двухфакторой авторизации</center>';
	...
	if( $u['2pass'] != '' && $u['2pass'] != '0' && $good22 == false ) {
		...
		?>
			...
			<body bgcolor=666666>
			<H3><FONT COLOR="black"><center>Двухфакторная авторизация. Введите ключ:</center></FONT></H3>
			<?=$koko?>
			<div align="center">
                <form method="post" action="/enter.php">
                    <input type="password" value="" name="code2"> <input type="submit" value="Войти">
...
?>
			...
			<div align="center">
                ...
                <hr>
                <form method="post" action="/enter.php">
                	Введите пароль в ручную, если у вас не отображается плагин flash.<br>
                    <input type="password" value="" name="code"> <input type="submit" value="Войти">
                    <input type="hidden" value="<?=$_POST['pass']?>" name="pass">
                </form>
			</div>
			</BODY>
...
<?
	...
	{
		setcookie('login','',0,'',$c['host']); // вместо аргумента 0 тут было time()-60*60*24
		setcookie('pass','',0,'',$c['host']); // вместо аргумента 0 тут было time()-60*60*24
	}
	...
	*/
	...
	setcookie('pass',$u['pass'],0,'',$c['host']); // вместо аргумента 0 тут было time()+60*60*24*7
	...
	{		
		if($u['auth'] != '' && $u['auth'] != '0' && $u['ip'] != IP) {
			mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','capitalcity','0','','".$u['login']."','В предыдущий раз этим персонажем заходили с другого компьютера ".date('d.m.Y H:i',$u['online']).". (Предыдущий ip: %".$u['ip'].")','".time()."','6','0')");
		}
		$apu = "`auth` = '".md5($u['login'].'AUTH'.IP)."',";
		setcookie('auth',md5($u['login'].'AUTH'.IP),0,'','likebk.com'); // вместо аргумента 0 тут было time()+60*60*24*365
	}
	...
	if(isset($_POST['active_code_key'])) {
		...
		if( $u['id'] != 12345 ) {
			...
			while($fr = mysql_fetch_array($friend)){
				 if($fr['friend'] != 0){
					$text_com2 = 'В игру зашел ваш друг: '.$u['login'].'!';
				 }elseif($fr['enemy'] != 0){
					$text_com2 = 'В игру зашел ваш враг: '.$u['login'].'!';
				 }
