...
function microLogin2($bus) {
	...
	$bus['login_BIG'] .= ''.$bus['login'].'</b>['.$bus['level'].']<a target=_blank href=http://likebk.com/inf.php?'.$bus['id'].' ><img width=12 hiehgt=11 src=http://img.likebk.com/i/inf_capitalcity.gif ></a>';
...
function bs_atack($bs,$u1,$u2) {
	...
	if( isset($u1['id'],$u2['id']) ) {
		...
		if( isset($me_real['id']) ) {
			...
			$mereal .= '<b>'.$me_real['login'].'</b>['.$me_real['level'].']<a target=_blank href=http://likebk.com/inf.php?'.$me_real['id'].' ><img width=12 hiehgt=11 src=http://img.likebk.com/i/inf_capitalcity.gif ></a>';
		}else{
			$mereal = '<i>Невидимка</i>[??]';
		}
...
function backusers($pl) {
	...
	while( $pu = mysql_fetch_array($sp) ) {
		...
		$usr = mysql_fetch_array(mysql_query('SELECT `id` FROM `users` WHERE `id` = "'.$pu['inBot'].'" AND `real` = 0 LIMIT 1'));
		if(isset($usr['id'])) {
			...
			mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.$pu['inBot'].'" AND `uid` > 0');
	...
	//Архивариусы
	$sp = mysql_query('SELECT * FROM `users` WHERE `pass` = "bstowerbot" AND `inTurnir` = "'.$pl['id'].'" AND `room` = "362" AND `real` = 0');
	while( $pu = mysql_fetch_array($sp) ) {
		...
		mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.$pu['id'].'" AND `uid` > 0');
			...
			if(  $pl['users'] < 2 ) {
				...
				if(  $pl['users'] == 1 ) {
					...
					if( $pl['arhiv'] == 0 ) {
						...
						$uwin = mysql_fetch_array(mysql_query('SELECT `id`,`money`,`login`,`level`,`align`,`clan` FROM `users` WHERE `inUser` = "'.$uwin_bot['id'].'" AND `login` = "'.$uwin_bot['login'].'" AND `room` = "362" LIMIT 1'));
						...
						if( isset($uwin['id']) ) {
							...
							mysql_query('UPDATE `users` SET `win_bs` = `win_bs` + 1 WHERE `id`=  "'.$uwin['id'].'" LIMIT 1');
