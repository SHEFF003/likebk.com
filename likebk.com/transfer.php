		...
		if($u->tfer['finish1']==0 && $u->tfer['finish2']==0)
		{
			...
			}elseif(isset($_POST['start2']))
			{
				...
				if($u->tfer['good1']>0 && $u->tfer['good2']>0)
				{
					...
					//Обмениваем предметы
					$sp = mysql_query('SELECT `u`.*,`m`.`price1`,`m`.`price2`,`m`.`name` FROM `items_users` AS `u` LEFT JOIN `items_main` AS `m` ON `m`.`id` = `u`.`item_id` WHERE `u`.`data` NOT LIKE "%notransfer%" AND (`u`.`uid`="'.$u->tfer['uid1'].'" OR `u`.`uid`="'.$u->tfer['uid2'].'") AND `u`.`delete`="0" AND `u`.`inOdet`="0" AND `u`.`inShop`="0" AND `u`.`inTransfer` > "0"');
					...
					while($pl = mysql_fetch_array($sp))
					{
						...
						if($pl['uid']==$u->tfer['uid1'])
						{
							$html_data .= ',['.$u->tfer['uid1'].':'.$pl['item_id'].':'.$x.':'.$po['frompisher'].':'.$po['sudba'].':'.$pl['gift'].':'.$nalog.':'.$pl['inGroup'].']';
						}elseif($pl['uid']==$u->tfer['uid2'])
						{
							$html_data .= ',['.$u->tfer['uid2'].':'.$pl['item_id'].':'.$x.':'.$po['frompisher'].':'.$po['sudba'].':'.$pl['gift'].':'.$nalog.':'.$pl['inGroup'].']';
						}
					...
					mysql_query('UPDATE `transfers` SET `html_log` = "'.$html_data.'" WHERE `id` = "'.$u->tfer['id'].'" LIMIT 1');
			...
			{
				$test = mysql_fetch_array(mysql_query('SELECT `item_id`,`inGroup` FROM `items_users` WHERE `id` = "'.mysql_real_escape_string($_POST['cancelid']).'" LIMIT 1'));
				$upd = mysql_query('UPDATE `items_users` SET `inTransfer` = "0" WHERE (`id` = "'.mysql_real_escape_string($_POST['cancelid']).'" OR (`item_id` = "'.$test['item_id'].'" AND `inGroup` = "'.$test['inGroup'].'" AND `inGroup` > 0)) AND `uid`="'.$u->info['id'].'" AND `delete`="0" AND `inOdet`="0" AND `inShop`="0" AND `inTransfer` > "0"');
				if($upd)
				{
					$u->tfer['r0'] = time();
			...
			}elseif($_POST['id']=='sale' && isset($_POST['itemid']) && $u->tfer['good1']==0 && $u->tfer['good2']==0)
			{
				...
				if($g==1)
				{
					...
					$x = $u->itemsX($itm['id'],$itm['uid']);
					if( $x > 1 ) {
						echo $x;
						mysql_query('UPDATE `items_users` SET `inTransfer` = "'.((int)$g).'" WHERE `id` = "'.$itm['id'].'" OR (`item_id` = "'.$itm['item_id'].'" AND `inGroup` = "'.$itm['inGroup'].'" AND `inGroup` > 0 AND `inShop` = 0 AND `inTransfer` = 0 AND `uid` = "'.$u->info['id'].'" AND `inOdet` = 0 AND `delete` = 0) LIMIT '.$x);
					}else{
			...
			if($_POST['id']=='reflesh')
			{
				...
				{
					$itmAll = ''; $itmAllSee = '';
					$itmAll = $u->genInv(5,'`iu`.`uid` = "'.$u->info['id'].'" AND `iu`.`data` NOT LIKE "%notransfer%" AND `iu`.`data` NOT LIKE "%toclan='.$u->info['clan'].'#%" AND `iu`.`delete` = 0 AND `iu`.`inOdet` = 0 AND `iu`.`inShop` = 0 AND `im`.`inRazdel` = "'.$i.'" AND `iu`.`inTransfer` = 0 ORDER BY `lastUPD` DESC');
			...
			if($_POST['id']=='minireflesh' || $_POST['id']=='reflesh')
			{
				...
				if($u->tfer['r'.$f]!=$u->tfer['r0'])
				{
					function itmInfotf($pl,$cl)
					{
						...
						if($pl['uid']==$u->info['id'])
						{
							$r .= '<img width="13" height="13" onClick="cancelitm('.$pl['id'].');" class="clr" src="http://img.likebk.com/i/clear.gif" />';
						}
						$r .= '<a href="items_info.php?id='.$pl['item_id'].'&rnd='.$code.'" target="_blank">'.$pl['name'].''.$x.'</a><br>';
						if($pl['inTransfer']==1)
						{
							$r .= '<img width="16" height="18" title="Этот предмет будет подарен" src="http://img.likebk.com/i/podarok.gif" />';
						}else{
							$r .= '<small style="font-size:10px">(налог: 1кр.)</small>';
						}
						$r = '<table width="100%" border="0" cellspacing="0" cellpadding="5"><tr><td width="50" align="center"><img src="http://img.likebk.com/i/items/'.$pl['img'].'" class="tfii"/></td><td valign="top" class="tfid">'.$r.'</td></tr></table>';
					...
					while($pl = mysql_fetch_array($sp))
					{
						...
						if(!isset($itmsee[$pl['item_id']][$pl['inGroup']]) || $pl['inGroup'] == 0) {
							$itm[1] .= itmInfotf($pl,$cl);
						}
						$itmsee[$pl['item_id']][$pl['inGroup']]++;
					}
