...
var itmjs = {
	...
	infoItem:function( data , i , module ) {
		...
		r += '<a href="http://likebk.com/item/' + itm['iid'] + '" target="_blank">' + itm[1] + '';
		...
		r += ' &nbsp;&nbsp; (Масса: ' + itm[10] + ')';
		if( po['art'] != undefined ) {
			r += ' <img title="Артефакт" src="http://img.likebk.com/i/artefact.gif" width="18" height="16">';				
		}
		if( itm[17] != '' ) {
			r += ' <img title="Вы не можете передать этот предмет кому-либо. Подарок от ' + itm[17] + '" src="http://img.likebk.com/i/desteny.gif" width="16" height="18">';				
		}
		if( po['sudba'] != undefined ) {
			if( po['sudba'] == 0 ) {
				r += ' <img title="Предмет будет связан общей судьбой с первым, кто наденет его. Никто другой не сможет его использовать." src="http://img.likebk.com/destiny0.gif" width="16" height="18">';				
			}else if( po['sudba'] == 1 ) {
				r += ' <img title="Предмет будет связан общей судьбой с первым, кто получит его. Никто другой не сможет его использовать." src="http://img.likebk.com/destiny_pickup.png" width="16" height="18">';
			}else{
				r += ' <img title="Предмет связан общей судьбой с ' + po['sudba'] + '. Никто другой не сможет его использовать." src="http://img.likebk.com/desteny.gif" width="16" height="18">';
			}
		}
		r += '<br>';
		if( module == 'comission' || module == 'comission_pick' ) {
		...
		if( po.srok != undefined || itm[24] > 0 ) {			
			if( po.srok > itm[24] ) {
				var srok = po.srok;
			}else{
				var srok = itm[24];
			}
			srok = Math.ceil(srok) + Math.ceil(itm[25]);
			if( srok-Math.ceil(itm[23]) > 86400 ) {
				r += 'Срок годности: до ' + top._bk.data('d.m.Y H:i',srok) + ' (Осталось: '+top._bk.timeOut(srok-itm[23])+')<br>';
			}else{
				r += '<b><font color=red >Срок годности: до ' + top._bk.data('d.m.Y H:i',srok) + ' (Осталось: '+top._bk.timeOut(srok-itm[23])+')</font></b><br>';
			}
		}
		//
		var tr = '';
