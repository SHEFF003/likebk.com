...
var locjs = {
	...
	start:function() {
		...
		this.selectPoint(0,false,0);
		//this.error('Ошибка подключения к базе.');
	},
	
	reflech:function() {
		if( this.lookIt == 0 ) {
			this.selectPoint( this.selectPointLast , false , this.filtgo );
		}else{
			this.getBaseContent();
		}
	},
	...
	htmlMenu:function() {
		...
		if( this.data.user.level >= 4 ) {
			r += '<input id="comission_give" onclick="locjs.selectPoint(\'give\',true,0)" style="width:40%" class="btnnew';
			if( this.selectPointLast == 'give' ) {
				r += '2';
			}
			r += '" type="button" value="Сдать вещи" />';
			r += '<input id="comission_pick" onclick="locjs.selectPoint(\'pick\',true,0)" style="width:40%" class="btnnew';
		...
		for (var i = 0; i < this.data.menu.length; i++) {
			r += '<div id="comission_pid' + i + '" title="Раздел №' + i + '" onclick="locjs.selectPoint(' + i + ',true,0)" class="comission_menu_point';
		...
		r += '<br>&bull; Налог не возвращается, если вещь не будет продана в течении двух недель (Сама вещь продается в государственный магазин за вычетом износа предмета).';
		...
		r += '<br>&bull; Если вы нашли баг или недочет, сообщите об этом здесь <a href="http://likebk.com/forum?read=2" target="_blank">Баги в комиссионном магазине</a>.';
		r += '</small></div><br><br>';
	...
	selectPointLast:0,
	filtgo:0,
	selectPoint:function( p , clearError , filt ) {
		this.filtgo = filt;
		if( clearError == true ) {
			this.error('');
		}
	...
	htmlTitle:function() {
		...
		if( this.lookIt == 0 ) {
			if( this.selectPointLast == 20 ) {
				r += '<div>Выберите уровень чарки:<br><small>';
					var j = 0
					var kk = ['[0] уровень','[1] уровень','[2] уровень','[3] уровень'];
					while( j != -1 ) {
						if( kk[j] != undefined ) {
							if( j > 0 ) {
								if( j == 8 ) {
									...
									r += ' <font color="grey">|</font> ';
								}
							}
							r += '<a style="cursor:pointer;padding:2px;';
							if( j == this.filtgo ) {
								r += 'background-color:#fff;border-radius:3px;';
							}
							r += '" onclick="locjs.filtgo='+j+';locjs.getBaseContent(); return false;">&nbsp;'+kk[j]+'&nbsp;</a>';
							j++;
						}else{
							j = -1;
						}
				...
				r += '</small></div>';
			}else if( this.selectPointLast == 21) {
				r += '<div>Выберите тип улучшения руны:<br><small>';
					var j = 0
					var kk = ['Уровень здоровья','Уровень маны','Сила','Ловкость','Интуиция','Интеллект','Защита от уронов','Защита от магий','Мощность уронов','Мощность магий','Мф. крита','Мф. против крита','Мф. уворота','Мф. против уворота','Мф. парирования','Мф. контрудара','Мф. мощности крита'];
					while( j != -1 ) {
						if( kk[j] != undefined ) {
							if( j > 0 ) {
								if( j == 8 ) {
	...
	getBaseContent:function() {
		//if( this.baseconnect == false ) {
			...
			$.getJSON('/core/comission/', {
				'point':this.selectPointLast, 'type':this.type, 'lookIt':this.lookIt , 'filtgo':this.filtgo
			},function(data) {
		...
		if( this.baseconnect == false ) {
			...
			$.getJSON('/core/comission/', {
				...
				if( data.buy_item != undefined && (data.buy_error == undefined || data.buy_error == 0) ) {
					...
					if( data.buy_last < 1 ) {
						locjs.selectPoint( locjs.selectPointLast , false , 0 );
					}
		...
		if( this.data.base != undefined ) {
			...
			if( this.selectPointLast == 'give' ) {
				...
				while( i < this.data.base.it.i ) {
					...
					left += '<img src="http://img.likebk.com/i/items/' + this.data.base.it[i][2] + '"><br>';
				...
				if( this.data.base.type == 1 ) {
					...
					while( i < this.data.base.it.i ) {
						left = ''; right = ''; if( color == 'c8c8c8' ) { color = 'd4d4d4'; }else{ color = 'c8c8c8'; }
						left += '<img src="http://img.likebk.com/i/items/' + this.data.base.it[i][5] + '"><br><a href="javascript:void(0);" onclick="locjs.lookItems(' + this.data.base.it[i][1] + ');">подробнее</a>';
						//
						right += '<a href="http://likebk.com/item/' + this.data.base.it[i][1] + '" target="_blank">' + this.data.base.it[i][4] + '</a>';
						...
						if( this.data.base.it[i][10] != '' ) {
							right += '<br><i><small>' + this.data.base.it[i][10] + '</small></i>';
						}
						//
