	...
	public static function getJSON() {
		...
		if( isset($_GET['newprice_item']) && \Core\User::$data['invBlock'] < OK && \Core\User::$data['allLock'] < OK ) {
				...
				if( $upd == true ) {
					...
					*/
					...
					\Core\Database::query( 'INSERT INTO `items_com` (
						`info`,`uid`,`item_id`,`items_id`,`name`,`level`,`img`,`massa`,`point`,`price`,`time`,`city`,`buy`,`delete`,`iznosNow`,`iznosMax`,`tr_align`,`group`
					) VALUES (
						:info , :uid , :item_id , :items_id , :name , :level , :img , :massa , :point , :price , :time , :city , 0 , 0 , :iznosNOW , :iznosMAX , :align , :group
					)' , array(
						'uid' => \Core\User::$data['id'],
			...
			if( !isset($itm['id']) || !isset($itm_user['id']) ) {
				...
				\Core\Database::query( 'INSERT INTO `users_delo` (`uid`,`time`,`text`,`login`,`type`) VALUES (
					:uid , :time , :text , :login , :type
				)' , array(
					'uid'	=> \Core\User::$data['id'],
					'time'	=> OK,
					'text'	=> 'Приобрел предмет &quot;'.$itm['name'].'&quot; (id'.$itm['id'].') из комиссионного магазина за '.$post_money.' кр. (+1 кр. комиссия) у персонажа &quot;'.$user['login'].'&quot;.',
					'login'	=> 'Comission.Shop',
					'type'	=> 1
				) );
				...
				\Core\Database::query( 'INSERT INTO `users_delo` (`uid`,`time`,`text`,`login`,`type`) VALUES (
					...
					'uid'	=> $user['id'],
					'time'	=> OK,
					'text'	=> 'Продал предмет &quot;'.$itm['name'].'&quot; (id'.$itm['id'].') в комиссионном магазине за '.$post_money.' кр. (+1 кр. комиссия) персонажу &quot;'.\Core\User::$data['login'].'&quot;.',
					'login'	=> 'Comission.Shop',
					'type'	=> 1
				) );
			...
			if( $_GET['point'] == 21 ) {
				//Сортировка рун
				$fl = round($_GET['filtgo']);
				$kk = array(
					'Уровень жизни%','Уровень маны%','Сила%','Ловкость%','Интуиция%','Интеллект%',
					'Защита от %урона%','Защита от %маги%','Мф. мощности %урона%','Мф. мощности %магии%',
					'Мф. критического%','Мф. против критического%','Мф. увертывания%','Мф. против увертывания%',
					'Мф. парирования%','Мф. контрудара%','Мф. мощности%крит%'
				);
				if(isset($kk[$fl])) {
					$kk = $kk[$fl];
				}else{
				...
				$pl = \Core\Database::query( 'SELECT * FROM `items_com` WHERE `name` LIKE "'.$kk.'" AND `point` = :point AND `city` = :city AND `delete` = 0 GROUP BY `items_id` ORDER BY `level` DESC' , array(
						'point' 	=> $_GET['point'],
						'city' 		=> \Core\User::$data['city']
				) , true , true );
				$i = 0;
				while( $i < count($pl) ) {
					$slot = \Core\Database::query( 'SELECT `info` FROM `items_main` WHERE `id` = :id LIMIT 1' , array(
						'id' 		=> $pl[$i]['items_id']
					) , true );
					$pl[$i]['info'] = $slot['info'];
					$i++;
				}
			}elseif( $_GET['point'] == 20 ) {
				//Сортировка чарок
				$fl = array(0,1,2,3);
				if(isset($fl[round($_GET['filtgo'])])){
					$fl = $fl[round($_GET['filtgo'])];
				}else{
					$fl = 0;
				}
				$pl = \Core\Database::query( 'SELECT * FROM `items_com` WHERE `name` LIKE "%['.$fl.']" AND `point` = :point AND `city` = :city AND `delete` = 0 GROUP BY `items_id` ORDER BY `level` DESC , `price` ASC' , array(
						'point' 	=> $_GET['point'],
						'city' 		=> \Core\User::$data['city']
				) , true , true );
			}else{
			...
			while( $i < count($pl) ) {
				//
				if($pl[$i]['info'] == '-1' ) {
					$tst = \Core\Database::query( 'SELECT `info` FROM `items_main` WHERE `id` = :id LIMIT 1', array(
						'id' => $pl[$i]['items_id']
					),true);
					if( isset($tst['info']) ){
						$pl[$i]['info'] = $tst['info'];
					...
					\Core\Database::query( 'UPDATE `items_com` SET `info` = :info WHERE `id` = :id LIMIT 1', array(
						'id' => $pl[$i]['id'],
						'info' => $pl[$i]['info']
					));
				}
