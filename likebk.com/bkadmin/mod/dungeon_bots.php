...
<form method="post" action="../../РЕДАКТОРЫ/bkadmin/mod/index.php?mod=dungeon_bots">
<table>
	<tbody>
		<tr>
			<td>По пещере:
...
<form method="post" action="../../РЕДАКТОРЫ/bkadmin/mod/index.php?mod=dungeon_bots"><hr>
			...
			<td valign="top" width="55%" align="left">
				...
				<script>
					function loadItemInfo(th){
						$.post('/bkadmin/lib/loadItemInfo.php', {item_id:$(th).parent('td').children('.items').val()}, function(data){
							$(th).html(data);
						});
					}
					
					function loadListItems(th){
						$.post('/bkadmin/lib/loadItemInfo.php', {getListItems:true}, function(data){
							$(th).html(data);
						});
					}
