...
if(!isset($_GET['test'])) {
	...
	if(getIP() != $_SERVER['SERVER_ADDR'] && getIP() != '127.0.0.1' && getIP() != '' && getIP() != '188.165.201.142') {
...
function microLogin2($bus) {
	...
	$bus['login_BIG'] .= ''.$bus['login'].'</b>['.$bus['level'].']<a target=_blank href=http://likebk.com/info/'.$bus['id'].' ><img width=12 hiehgt=11 src=http://img.likebk.com/i/inf_capitalcity.gif ></a>';
...
function bs_atack($bs,$u1,$u2) {
	...
	if( isset($u1['id'],$u2['id']) ) {
		...
		if( isset($me_real['id']) ) {
			...
			$mereal .= '<b>'.$me_real['login'].'</b>['.$me_real['level'].']<a target=_blank href=http://likebk.com/info/'.$me_real['id'].' ><img width=12 hiehgt=11 src=http://img.likebk.com/i/inf_capitalcity.gif ></a>';
		}else{
			$mereal = '<i>Невидимка</i>[??]';
		}
...
while( $pl = mysql_fetch_array($sp) ) {
	...
	if( $pl['status'] == 1 ) {
		...
		if( $pl['time_start'] < time() - 6*60*60 ) {
			...
			//Добавляем в лог БС
			$text = 'Турнир завершен. Победитель: <i>Отсутствует</i> (Турнир завершился по таймауту). Призовой фонд: <b>'.round($pl['money']*0.85,2).'</b> екр.';
			mysql_query('INSERT INTO `bs_logs` (`type`,`text`,`time`,`id_bs`,`count_bs`,`city`,`m`,`u`) VALUES (
			...
			if(  $pl['users'] < 2 ) {
				...
				if(  $pl['users'] == 1 ) {
					...
					if( $pl['arhiv'] == 0 ) {
						...
						$uwin = mysql_fetch_array(mysql_query('SELECT `id`,`money`,`login`,`level`,`align`,`clan` FROM `users` WHERE `login` = "'.$uwin_bot['login'].'" ORDER BY `id` ASC LIMIT 1'));
						...
						//9-100 , 10 - 300 , 11 - 500 , 12-13 - 600
						$elvl = array(
							0,0,0,0,0,0,0,0,20000,100000,300000,500000,600000,600000
						);
						$swin_bot['exp'] += $elvl[$uwin['level']];
						...
						if( isset($uwin['id']) ) {
							...
							$arch = mysql_fetch_array(mysql_query('SELECT * FROM `achiev` WHERE `uid` = "'.$uwin['id'].'" LIMIT 1'));
							if(isset($arch['id'])) {
								$arch['a12']++;
								mysql_query('UPDATE `achiev` SET `a12` = `a12` + 1 WHERE `id` = "'.$arch['id'].'" LIMIT 1');
							}
							
							e('#'.$pl['usersn'].' Турнир для '.$pl['to_lvl'].' уровней в <b>Башне Смерти</b> завершился. Победитель: '.microLogin2($uwin).'. Приз: <b>'.round($pl['money']*0.85,2).'</b> кр. и <b>'.round($swin_bot['exp']).'</b> опыта. Начало нового турнира через '.timeOut($pl['time_start']-time()-3600).' (<small>'.date('d.m.Y H:i',$pl['time_start']).'</small>)');
						}
						//Добавляем в лог БС
					...
					$pl['time_start'] = time() + $cnfg['time_restart'] * (60*60);
					//Добавляем в лог БС
					$text = 'Турнир завершен. Победитель: <i>Отсутствует</i> (Никто не остался в живых). Призовой фонд: <b>'.round($pl['money']*0.85,2).'</b> екр.';
					mysql_query('INSERT INTO `bs_logs` (`type`,`text`,`time`,`id_bs`,`count_bs`,`city`,`m`,`u`) VALUES (
						"1", "'.mysql_real_escape_string($text).'", "'.time().'", "'.$pl['id'].'", "'.$pl['count'].'", "'.$pl['city'].'",
						"'.round($pl['money']*0.85,2).'","'.$i.'"
					)');
					...
					backusers($pl);
					$pl['count']++;
					e('Турнир для '.$pl['to_lvl'].' уровней в <b>Башне Смерти</b> завершился. Победитель: <i>Отсутствует</i> (Никто не остался в живых). Призовой фонд <b>'.round($pl['money']*0.85,2).'</b> екр. Начало нового турнира через '.timeOut($pl['time_start']-time()-3600).' (<small>'.date('d.m.Y H:i',$pl['time_start']).'</small>)');
		...
		if( nostart( $pl ) == false ) {
			...
			$itm1 = 4176;
			if( $itm1 > 0 && !isset($chek123) ) {
				mysql_query('DELETE FROM `dungeon_items` WHERE `item_id` = 4176');
				$chek123 = true;
				//
				$mp = rand(0,count($map)-1);
				//
...
function testzv($usr) {
	...
	$sp = mysql_query('SELECT `id`,`usermax` FROM `zayvki` WHERE `start` = 0 AND `razdel` = 5 AND `cancel` = 0 AND `min_lvl_1` >= "'.$usr['level'].'" AND `max_lvl_1` <= "'.$usr['level'].'" ORDER BY `id` DESC');
	while( $pl = mysql_fetch_array($sp) ) {
		if( $r == false ) {
			$umx = mysql_fetch_array(mysql_query('SELECT COUNT(`id`) FROM `stats` WHERE `zv` = "'.$pl['id'].'" LIMIT 1'));
			$umx = $umx[0];
			if( $umx < $pl['usermax'] ) {
				$r = $pl;
			}
...
function getmain($key,$id,$login,$pass) {
	return file_get_contents('http://likebk.com/main_bot.php?megabot=true&cron_core='.$key.'&uid='.$id.'&pass='.$pass);
}

function getbattle($key,$id,$login,$pass) {
	return file_get_contents('http://likebk.com/jx/battle/refresh_bot.php?megabot=true&cron_core='.$key.'&uid='.$id.'&pass='.$pass);
}

function changecomplect($usr,$cron_key) {
	global $u;
	...
	$coms = $comps[$usr['level']][rand(0,count($comps[$usr['level']])-1)];
	if( $coms > 0 ) {
		$coms = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `id` = "'.$coms.'" LIMIT 1'));
		if(isset($coms['id'])) {
			
			//Удаляем прошлые вещи и эффекты
			mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.$usr['id'].'"');
			mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$usr['id'].'"');
			...
			while( $i <= 20 ) {
				if( $coms['w'.$i] > 0 ) {
					$itm = $u->addItem($coms['w'.$i],$usr['id'],'|sudba=1');
					if( $itm > 0 ) {						
						mysql_query('UPDATE `items_users` SET `inOdet` = "'.$i.'" WHERE `id` = "'.$itm.'" LIMIT 1');
					}
				}
				$i++;
			}
			...
			while( $i <= 20 ) {
				if($coms['e'.$i] > 0) {
					...
					$eff = mysql_fetch_array(mysql_query('SELECT * FROM `eff_main` WHERE `id2` = "'.$coms['e'.$i].'" LIMIT 1'));	
					mysql_query('INSERT INTO `eff_users` (`overType`,`id_eff`,`uid`,`name`,`timeUse`,`data`,`no_Ace`) VALUES ("'.$eff['oneType'].'","'.$eff['id2'].'","'.$usr['id'].'","'.$eff['mname'].'","'.(time()+9640000).'","'.$eff['mdata'].'","'.$eff['noAce'].'")');
				}
				$i++;
			}
			
			mysql_query('UPDATE `stats` SET `stats` = "'.$coms['stats'].'",`priems` = "'.$priems.'" WHERE `id` = "'.$usr['id'].'" LIMIT 1');
			
			getmain($cron_key,$usr['id'],$usr['login'],$usr['pass']);
...
$zvgo = array();
$btlgo = array();
$sp = mysql_query('SELECT `id`,`login`,`pass`,`battle`,`timereg`,`level`,`room` FROM `users` WHERE `pass` = "megabot" ORDER BY `online` DESC LIMIT 21');
while( $pl = mysql_fetch_array($sp) ) {
	
	$cron_key = md5($pl['id'].'_brfCOreW@!_'.$pl['pass']);
	
	/*if( $pl['level'] == 0 ) {
		//получение уровня и т.д.
		echo '<div style="padding:10px; background-color:#000; color:green;">'.getmain($cron_key,$pl['id'],$pl['login'],$pl['pass']).'</div>';
	}else{*/
		if( $pl['room'] != 250 ) {
			mysql_query('UPDATE `users` SET `room` = "250" WHERE `id` = "'.$pl['id'].'" LIMIT 1');
		}
	//}
	
	echo '[bot '.$pl['id'].' <b>'.$pl['login'].'</b> ['.$pl['level'].']: ';
	if( $pl['online'] < time() ) {
		mysql_query('UPDATE `users` SET `online` = "'.(time()+3600).'" WHERE `id` = "'.$pl['id'].'" LIMIT 1');
		mysql_query('UPDATE `stats` SET `exp` = 0 WHERE `id` = "'.$pl['id'].'" LIMIT 1');
	}
	if( $pl['level'] == 0 ) {
		
	}elseif( $pl['battle'] == 0 ) {
		mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$pl['id'].'" AND `v1` = "priem"');
		//Проверка, в заявке или нет, если в заявке - ничего не делаем
		$st = mysql_fetch_array(mysql_query('SELECT `id`,`zv`,`hpNow`,`hpAll` FROM `stats` WHERE `id` = "'.$pl['id'].'" LIMIT 1'));
		if( $st['zv'] > 0 ) {
			//Ожидаем начала поединка
			if( $st['hpNow'] < $st['hpAll'] ) {
				mysql_query('UPDATE `stats` SET `hpNow` = "'.$st['hpAll'].'",`mpNow` = "'.$st['mpAll'].'" WHERE `id` = "'.$st['id'].'" LIMIT 1');
			}
			$testzv = mysql_fetch_array(mysql_query('SELECT `id`,`usermax`,`time`,`btl_id`,`cancel` FROM `zayvki` WHERE `id` = "'.$st['zv'].'" AND `cancel` = 0 LIMIT 1'));
			if(!isset($testzv['id']) || $testzv['time'] + 600 < time() ) {
				//
				echo '['.($testzv['time'] + 600 - time()).']';
				if((isset($testzv['id']) && ( $testzv['btl_id'] > 0 || $testzv['cancel'] > 0 )) || !isset($testzv['id'])) {
					echo '[inBATTLE]';
					$st['zv'] = 0;
					mysql_query('UPDATE `stats` SET `zv` = "0" WHERE `id` = "'.$st['id'].'" LIMIT 1');
				}
				//
			}
			echo ' в заявке id '.$st['zv'].' ';
		}else{
			mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$pl['id'].'" AND ( `name` LIKE "%травма%" OR `name` LIKE "%ослабление%" )');
			//проверяем комплект
			/*if( $pl['timereg'] < time() - 43200 ) {
				//меняем комплект
				if(changecomplect($pl,$cron_key) == true) {
					mysql_query('UPDATE `users` SET `timereg` = "'.( time() + rand( 0 , 43200 ) ).'" WHERE `id` = "'.$pl['id'].'" LIMIT 1');
				}else{
					echo '[FATAL_ERROR!]';
				}
				echo ' <font color="green"><b>Меняет комплект</b></font> ';
			}else{*/
				//ищем заявку в хаотах
				$zsp = testzv($pl);
				if( $zsp != false ) {
					//Нашли заявку, принимаем!
					if( $zvgo[$zsp['id']] < 2 ) {
						$zvgo[$zsp['id']]++;
						mysql_query('UPDATE `stats` SET `zv` = "'.$zsp['id'].'" WHERE `id` = "'.$pl['id'].'" LIMIT 1');
						if( $st['hpNow'] < $st['hpAll'] ) {
							mysql_query('UPDATE `stats` SET `hpNow` = "'.$st['hpAll'].'",`mpNow` = "'.$st['mpAll'].'" WHERE `id` = "'.$st['id'].'" LIMIT 1');
						}
					}
				}
				echo ' <font color="blue"><b>Ищет заявку ['.$zsp['id'].']</b></font> ';
			//}
		}
	}else{
		/*$st = mysql_fetch_array(mysql_query('SELECT `id`,`zv`,`hpNow`,`hpAll` FROM `stats` WHERE `id` = "'.$pl['id'].'" LIMIT 1'));
		...
		//	mysql_query('UPDATE `stats` SET `zv` = "0" WHERE `id` = "'.$st['id'].'" LIMIT 1');
		//}
		if(!isset($btlgo[$pl['battle']]) && $btlgo[$pl['battle']] < rand(1,3)) {
			$btlgo[$pl['battle']]++;
			$btl = mysql_fetch_array(mysql_query('SELECT `id`,`time_start`,`time_over`,`timeout` FROM `battle` WHERE `id` = "'.$pl['battle'].'" LIMIT 1'));
			if(isset($btl['id'])) {
				echo ' [btl_see] ';
				$log = mysql_fetch_array(mysql_query('SELECT `time` FROM `battle_logs` WHERE `battle` = "'.$pl['battle'].'" ORDER BY `time` DESC LIMIT 1'));
				if(isset($log['time']) && $log['time'] < time() - $btl['timeout']/2) {
					echo '<div style="padding:10px; background-color:#000; color:green;">'.getbattle($cron_key,$pl['id'],$pl['login'],$pl['pass']).'</div>';
					echo ' [reflesh_battle] ';
				}elseif(!isset($log['time']) && $btl['time_start'] < time() - $btl['timeout'] && $btl['time_over'] == 0 ) {
					echo '<div style="padding:10px; background-color:#000; color:green;">'.getbattle($cron_key,$pl['id'],$pl['login'],$pl['pass']).'</div>';
					echo ' [reflesh_battle_timeout] ';
				}
			}else{
				echo ' [btl_lock '.$pl['battle'].' ] ';
			}
		}*/
		
		//В поединке, ничего не делаем, только проверка на возможность заюзать приемы
		echo ' <a href="/logs.php?log='.$pl['battle'].'" target="_blank">в поединке (id'.$pl['battle'].')</a>';
	}
	echo ']<hr>';
}
