...
while($pl = mysql_fetch_array($sp)) {
	if( $rz == $pl['id'] && $rz2 == 0 ) {
		...
		$frm['html'] .= '<a href="http://likebk.com/forum/'.$pl['id'].'/" class="frm-title">&laquo;'.$pl['name'].'&raquo;</a>';
	}
	...
	while($pl2 = mysql_fetch_array($sp2)) {
		...
		if( $rz2 == $pl2['id'] ) {
			...
			$frm['html'] .= '<a href="http://likebk.com/forum/'.$pl['id'].'/'.$pl2['id'].'/" class="frm-title2">'.$pl2['name'].'</a>';
		}
		$frm['px'] += 23;
	}
	$frm['px'] += 23;
