...
if( $url[2] == 'upload' ) {
	...
	if( $u->info['activ'] == 1 ) {
		...
		while($pl = mysql_fetch_array($sp)) {
			if( $url[4] == 'delete' && $url[5] == $pl['id'] ) {
				...
				$html .= '<a title="'.$usrs[$pl['uid']]."\n".date('d.m.Y H:i',$pl['time']).'" oncontextmenu="if(confirm(\'Вы уверены?\')){ top.location=\'http://likebk.com/library/upload/'.htmlspecialchars($url[3],NULL,'cp1251').'/delete/'.$pl['id'].'/\'; }return false;" target="_blank" href="http://likebk.com/ui/'.$pl['img'].'.'.$pl['type'].'"><img src="http://likebk.com/ui/'.$pl['img'].'.'.$pl['type'].'" class="imgo"></a>';
			}
			$i++;
		}
		if( $i == 0 ) {
		 ...
		 if( isset($_FILES['filename']) ) {
				...
				if( $handle->file_src_name_ext == 'png' || $handle->file_src_name_ext == 'jpg' || $handle->file_src_name_ext == 'gif' ) {
					...
					*/
					...
					if ($handle->processed) 
					{
						...
						$html .= 'Файл <a href="http://likebk.com/ui/'.$fname.'.'.$handle->file_src_name_ext.'" target="_blank">http://likebk.com/ui/'.$fname.'.'.$handle->file_src_name_ext.'</a> был успешно загружен.';
						$handle->clean();
					} else {
			 ...
			 $html .= '
			  ...
			  <center>
			  <form action="http://likebk.com/library/upload/" method="post" enctype="multipart/form-data">
			  <input type="file" name="filename"> 
			  <input class="btn2" type="submit" value="Загрузить"><hr>';
			  
			  if( $u->info['admin'] > 0 ) {
				 $html .= '<a href="http://likebk.com/library/upload/all/">[ Все изображения ]</a> ';
			  }
			  $html .= '<a href="http://likebk.com/library/upload/me/">[ Мои изображения ]</a>';
	...
	$i = 1;
	while($pl = mysql_fetch_array($sp)) {
		$html .= '<a target="_blank" href="http://likebk.com/library/'.$pl['url_name'].'/">&gt;&gt; '.$pl['title'].'</a><br>Автор: '.$u->microLogin($pl['uid'],1).' / Дата публикации: '.date('d.m.Y',$pl['time']).'<hr>';
		$i++;
	}
	if( $html == '' ) {
		$html = 'В настоящий момент непроверенных статей нет.<br>
		<br>Если Вы хотите написать свою статью - <a target="_blank" href="http://likebk.com/library/new/">http://likebk.com/library/new/</a><br>
		<br>Более подробная информация - <a href="http://likebk.com/library/public/">http://likebk.com/library/public/</a>';
	}
...
?>
<!-- -->
<script src="http://new.likebk.com/static/ckeditor/ckeditor.js"></script>
<!-- -->
...
?>
		...
		Ссылка для просмотра: <a target="_blank" href="http://likebk.com/library/<?=$mbpage['url_name']?>/">http://likebk.com/library/<?=$mbpage['url_name']?>/</a>
		<hr />
...
<?
	...
	?>
		...
		Ссылка для просмотра: <a target="_blank" href="http://likebk.com/library/id<?=$sid?>/">http://likebk.com/library/id<?=$sid?>/</a>
		<hr />
		Мы ценим проделанную Вами работу и постараемся как можно скорее рассмотреть заявку на добавление данной статьи
		<br /><br /><br /><br /><br /><br /><br /><br />
		, с уважением<br />
...
?>
<form method="post" action="http://likebk.com/library/new/<?=$mbpage['url_name']?>">
<?
if( isset($mbpage['id']) ) {
	...
	if( isset($_POST['save2']) ) {
		...
		if( $u->info['admin'] > 0 ) {
			...
			if( $red500 == true ) {
				echo '<script>top.location.href="http://likebk.com/library/new/'.$mbpage['url_name'].'/"</script>';
			}
		}
	...
	if( isset($pl['id']) ) {
		...
		if($pl['uid'] > 0 || $u->info['admin'] > 0) {
			...
			if($pl['uid'] == $u->info['id'] || $u->info['admin'] > 0) {
				echo ' &nbsp; / &nbsp; <a target="_blank" href="http://likebk.com/library/new/'.$pl['url_name'].'/">Изменить</a>';
				if( $u->info['admin'] > 0 ) {
					echo ' &nbsp; / &nbsp; <a href="http://likebk.com/library/'.$pl['url_name'].'/delete/">Удалить</a>';
				}
			}
			echo '</div></small>';
		}
