		...
		public function finishBattle_new($var1,$var2,$var3) {
			...
			if( $this->info['team_win'] == -1 ) {
				...
				while( $pl = mysql_fetch_array($sp) ) {
					...
					if($this->info['razdel'] == 5 && (date('m') == '12' || (date('m') == '01' && date('d') <= 14))){
						$pl['sneg007'] = 1;
						...
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
					}elseif($this->info['razdel'] == 5 && (date('m') == '06' || (date('m') == '07' && date('d') <= 27))){
						$pl['sneg007'] = 8;
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
						...
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
					}elseif($this->info['dungeon'] > 0 && (date('m') == '06' || (date('m') == '07' && date('d') <= 27))){
						if(rand(0,100) < 11) {
							$pl['sneg007'] = 9;
						}else{
							$pl['sneg007'] = 0;
						...
						while($i<count($this->users)) {
							if( $this->stats[$i]['hpNow'] < 1 && $this->users[$i]['clone'] == 0 && $this->stats[$i]['clone'] == 0 && $this->users[$i]['real'] == 0 ) {
								...
								if( isset($usr_real['id'])) {
									$usrreal = '';
									if( $usr_real['align'] > 0 ) {
										$usrreal .= '<img src=http://img.legbk.net/i/align/align'.$usr_real['align'].'.gif width=12 height=15 >';
									}
									if( $usr_real['clan'] > 0 ) {
										$usrreal .= '<img src=http://img.legbk.net/i/clan/'.$usr_real['clan'].'.gif width=24 height=15 >';
									}
									$usrreal .= '<b>'.$usr_real['login'].'</b>['.$usr_real['level'].']<a target=_blank href=http://legbk.net/inf.php?'.$usr_real['id'].' ><img width=12 hiehgt=11 src=http://img.legbk.net/i/inf_capitalcity.gif ></a>';
								}else{
						...
						while($i<count($this->users)) {
							if( $this->users[$i]['team'] != $this->info['team_win'] ) {
								...
								if( !isset($tr_pl['id']) || $tr_pl['v1'] < 3 || ( $dart['uid1'] == $this->users[$i]['id'] || $dart['uid2'] == $this->users[$i]['id'] ) ) {
									...
									$nelich = 0;
									$nelich2 = '';
									if( $dart['uid1'] == $this->users[$i]['id'] || $dart['uid2'] == $this->users[$i]['id'] ) {
										$nelich = array('id'=>1);
				...
				//обновляем данные
					...
					if($dnr==1){
						...
						if( $dies < 2 || $this->info['dungeon'] == 15 ) {
                            ...
                            $rooms = array(374 => 372, 189 => 188, 360 => 354, 19 => 293, 315 => 275);
...
/*				if($this->stats[$i]['os4'] > 0){
	...
	//Наносим удар между игроками
		...
		public function startAtack($id,$newsf5 = false)
		{
			...
			if( $newsf5 == false ) {
				if( $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod'] > 0 ) {
					...
					mysql_query('UPDATE `battle_act` SET
						`uid1` = "'.$this->atacks[$id]['uid2'].'" , `a1` = "00000" , `b1` = "0" , `out1` = "1" , `tout1` = "'.$this->atacks[$id]['tout2'].'" , `tpo1` = "'.$this->atacks[$id]['tpo2'].'",
					...
					//$usruh = $this->yhod_user($this->atacks[$id]['uid1'],$this->atacks[$id]['uid2'],$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod']);
					...
					//
					$this->startAtack($lstidhod,true);
					//
					$this->atacks[$id] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act` WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1'));
					//echo '<font color=red><b>[UPDATE_ACT-1]</b></font>';
				}elseif( $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['yhod'] > 0 ) {
					//Меняем конструкцию размена
					mysql_query('UPDATE `battle_act` SET
						`uid1` = "'.$this->atacks[$id]['uid1'].'" , `a1` = "00000" , `b1` = "0" , `out1` = "1" , `tout1` = "'.$this->atacks[$id]['tout1'].'" , `tpo1` = "'.$this->atacks[$id]['tpo1'].'",
					...
					$rndb = rand(1,5);
					//
					//$usruh = $this->yhod_user($this->atacks[$id]['uid2'],$this->atacks[$id]['uid1'],$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['yhod']);
					...
					//
					$this->atacks[$id] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act` WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1'));
					//echo '<font color=red><b>[UPDATE_ACT-2]</b></font>';
				}elseif( $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod'] > 0 ) {
					//Меняем конструкцию размена
					mysql_query('UPDATE `battle_act` SET
				...
				while( $i <= 2 ) {
					...
					if(isset($untac['id'])) {						
						...
						$pvr['sp'] = mysql_query('SELECT `a`.* FROM `eff_users` AS `a` WHERE `a`.`uid` = "'.$this->atacks[$id]['uid'.$j].'" AND `a`.`delete` = 0 AND ( `a`.`v1` = "priem"
							...
							AND `a`.`v1` = "priem" AND `a`.`v2` != 201 AND `a`.`v2` != 238 AND `a`.`v2` != 139 AND `a`.`v2` != 211 AND `a`.`v2` != 233 AND `a`.`v2` != 223 AND `a`.`v2` != 222
...
//Добавляем в лог
	...
	/* Расчет урона */
		public function yrn($st1, $st2, $u1, $u2, $level, $level2, $type, $min_yron, $max_yron, $min_bron, $max_bron, $vladenie, $power_yron, $power_krit, $zashita, $ozashita, $proboi, $weapom_damage, $weapom_min, $weapom_max, $za_proc, $zm_proc, $zashitam , $ozashitam ) {
