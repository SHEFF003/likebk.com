	...
	public function dialog_act_update($data) {
		...
		while( $i < count($x) ) {
			...
			if( $var == 'take_item' ) {
				...
				$sp = mysql_query('SELECT * FROM `items_users_res` WHERE `uid` = "'.$u->info['id'].'" AND `item_id` = "'.mysql_real_escape_string($val).'" AND (`delete` = "0" OR `delete` = "1000") AND `inOdet` = "0" AND `inShop` = "0" LIMIT 100');
				while($pl = mysql_fetch_array($sp))	{
					$itms[$pl['item_id']]++;	
				}
				...
				if($itms[$val] >= $k[2]) {
					...
					$xg = round($k[2]);
					$spd1 = mysql_query('SELECT `id` FROM `items_users` WHERE `item_id` = "'.mysql_real_escape_string($val).'" AND `uid` = "'.$u->info['id'].'" AND `inShop` = 0 AND `inTransfer` = 0 AND (`delete` = "0" OR `delete` = "1000") AND `inOdet` = 0 LIMIT '.$xg);
					while( $pld1 = mysql_fetch_array($spd1) ) {
						mysql_query('UPDATE `items_users` SET `delete` = "'.time().'" WHERE `id` = "'.$pld1['id'].'" LIMIT 1');
						$xg--;
					}
					$spd1 = mysql_query('SELECT `id` FROM `items_users_res` WHERE `item_id` = "'.mysql_real_escape_string($val).'" AND `uid` = "'.$u->info['id'].'" AND `inShop` = 0 AND `inTransfer` = 0 AND (`delete` = "0" OR `delete` = "1000") AND `inOdet` = 0 LIMIT '.$xg);
					while( $pld1 = mysql_fetch_array($spd1) ) {
						mysql_query('UPDATE `items_users_res` SET `delete` = "'.time().'" WHERE `id` = "'.$pld1['id'].'" LIMIT 1');
						$xg--;
					}
					...
					if($ta['tr'] != '') {
						...
						while($i < count($x)) {
							...
							if($k[0]=='data') {
								...
								if(!isset($qlst['id'])){
									$qlst = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users_res` WHERE `uid` = "'.$u->info['id'].'" AND `item_id` = "'.$k[1].'" AND `inOdet` = 0 AND `inTransfer` = 0 AND `delete` = 0 AND `inShop` = 0 LIMIT 1'));
									if(!isset($qlst['id'])){
										$go1 = 0;
								...
								if(isset($qlst['id'])){
									$qlst = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users_res` WHERE `uid` = "'.$u->info['id'].'" AND (`item_id` = "'.$k[1].'") AND `inTransfer` = 0 AND `delete` = 0 AND `inShop` = 0 LIMIT 1'));
									if(isset($qlst['id'])){
										$go1 = 0;
										$itmqs = mysql_fetch_array(mysql_query('SELECT `id`,`name` FROM `items_main` WHERE `id` = "'.$k[1].'" LIMIT 1'));
										if(isset($itmqs['id'])) {
								...
								if(isset($qlst['id'])){
									...
									if(isset($qlst['id'])){
										$itmqs = mysql_fetch_array(mysql_query('SELECT `id`,`name` FROM `items_main` WHERE `id` = "'.$k[1].'" LIMIT 1'));
										if(isset($itmqs['id'])) {
											if(mysql_query('UPDATE `items_users_res` SET `delete` = "'.time().'" WHERE `id` = "'.$qlst['id'].'" LIMIT 1')) {
												$txt .= '<br><b><font color=red>Вы отдали &quot;'.$itmqs['name'].'&quot;</font>';
											}
										}
										$pg = $ta['page'];	
					...
					if(isset($act[1]) && $go1 == 1){
						...
						if( $act0[0] == 'fileqst' ) {
							...
							$pg = 3;
								...
								$pg = 6;
						}elseif($act[0] == 'nizini_mod1' && $go == 1) {
							$bag = array();
							
							/*$test_bag = mysql_query('SELECT `id`,`item_id` FROM `items_users` WHERE `uid` = '.$u->info['id'].' AND `item_id` = 9965 OR (`item_id` = 9966) OR (`item_id` = 9967) AND `delete` = 0');
							$bag[0] = 0; $bag[1] = 0; $bag[2] = 0;
							while($pl = mysql_fetch_array($test_bag)) {
								if($pl['item_id'] == 9965) {
									$bag[0]++;
								}elseif($pl['item_id'] == 9966) {
									$bag[1]++;
								}elseif($pl['item_id'] == 9967) {
									$bag[2]++;
								}
							}*/
							
							$bag1 = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users` WHERE `uid` = '.$u->info['id'].' AND `item_id` = 9965 AND `delete` = 0 ORDER BY `id` DESC LIMIT 1'));
							
							$bag2 = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users` WHERE `uid` = '.$u->info['id'].' AND `item_id` = 9966 AND `delete` = 0 ORDER BY `id` DESC LIMIT 1'));
							
							$bag3 = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users` WHERE `uid` = '.$u->info['id'].' AND `item_id` = 9967 AND `delete` = 0 ORDER BY `id` DESC LIMIT 1'));
							
							if(isset($bag1['id']) && (isset($bag2['id'])) && (isset($bag3['id']))) {
								$add_bag = 9968;
								$add = $u->addItem($add_bag,$u->info['id']);
								if($add > 0) {
									//+25 мф крита, или уворота, или антикрита, или против уворота или +25 маны или +25 хп
									$data = array( 0 => "add_m1=25", 1 => "add_m4=25", 2 => "add_m2=25", 3 => "add_m5=25", 4 => "add_hpAll=25", 5 => "add_mpAll=25");
									$data = $data[rand(0,5)];
									$data .= "|add_maxves=1500";
									$txt = $data;
									mysql_query('UPDATE `items_users` SET `data` = "'.$data.'" WHERE `id` = '.$add.' LIMIT 1');
									//удаляем котомки
									$u->deleteItemID(9965, $u->info['id'], 1);
									$u->deleteItemID(9966, $u->info['id'], 1);
									$u->deleteItemID(9967, $u->info['id'], 1);
									
									$txt = 'Вы успешно обменяли 3 Котомки на Коллекционную';
								}
							}else{
								$txt = 'Не достаточно "Котомок" для создания Коллекционной';
							}
							if($txt!=''){
								$txt = '<br><font color="red">'.$txt.'</font>';	
							}
						} elseif($act[0]=='twink' && $go1 == 1) {
							$jt = mysql_fetch_array(mysql_query('SELECT COUNT(`id`) FROM `items_users` WHERE `uid` = '.$u->info['id'].' AND `item_id` = 9979 AND `delete` = 0 LIMIT 1'));
							$jt = $jt[0];
							if($act[1] == 9980) {
								if($jt >= 100) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 100);
									$txt = 'Вы получили чек на 1000кр, за 100 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(100 - $jt).' шт.';
								}
							}elseif($act[1] == 9981) {
								if($jt >= 25) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 25);
									$txt = 'Вы получили Нубохил (0/3), за 25 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(25 - $jt).' шт.';
								}
							}elseif($act[1] == 9982) {
								if($jt >= 50) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 50);
									$txt = 'Вы получили Продлить жизнь, за 50 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(50 - $jt).' шт.';
								}
							}elseif($act[1] == 9983) {
								if($jt >= 10) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 10);
									$txt = 'Вы получили Дуэль нубов, за 10 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(10 - $jt).' шт.';
								}
							}elseif($act[1] == 9984) {
								if($jt >= 70) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 70);
									$txt = 'Вы получили Ярость нуба, за 70 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(70 - $jt).' шт.';
								}
							}elseif($act[1] == 9985) {
								if($jt >= 50) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 50);
									$txt = 'Вы получили Сила нуба, за 50 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(50 - $jt).' шт.';
								}
							}elseif($act[1] == 9986) {
								if($jt >= 50) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 50);
									$txt = 'Вы получили Ловкость нуба, за 50 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(50 - $jt).' шт.';
								}
							}elseif($act[1] == 9987) {
								if($jt >= 50) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 50);
									$txt = 'Вы получили Интуиция нуба, за 50 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(50 - $jt).' шт.';
								}
							}elseif($act[1] == 9988) {
								if($jt >= 50) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 50);
									$txt = 'Вы получили Интеллект нуба, за 50 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(50 - $jt).' шт.';
								}
							}elseif($act[1] == 9992) {
								if($jt >= 25) {
									$u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9979, $u->info['id'], 25);
									$txt = 'Вы получили Нубоману, за 25 жетонов Новичка';
								}else{
									$txt = 'Не достаточно жетонов. Не хватает: '.(25 - $jt).' шт.';
								}
							}
						...
						} elseif($act[0]=='kuznica' && $go1 == 1) {
							$kuz = mysql_query('SELECT `im`.*,`iu`.* FROM `items_users` AS `iu` LEFT JOIN `items_main_data` AS `im` ON (`im`.`items_id` = `iu`.`item_id`) WHERE `im`.`data` LIKE "%kuznica=1%" AND `iu`.`delete` = 0 AND `iu`.`uid` = '.$u->info['id'].'');
							$ik = array();
							while($pl = mysql_fetch_array($kuz)) {
								//считаем ресурсы
								if($pl['items_id'] == 9496) $ik[0]++; //сущность шлема
								if($pl['items_id'] == 9495) $ik[1]++; //сущность наручей
								if($pl['items_id'] == 9494) $ik[2]++; //сущность серег
								if($pl['items_id'] == 9493) $ik[3]++; //сущность амулета
								if($pl['items_id'] == 9492) $ik[4]++; //сущность перчаток
								if($pl['items_id'] == 9491) $ik[5]++; //сущность сапог
								if($pl['items_id'] == 9490) $ik[6]++; //сущность брони
								if($pl['items_id'] == 9489) $ik[7]++; //сущность пояса
								if($pl['items_id'] == 9488) $ik[8]++; //сущность щита
								if($pl['items_id'] == 9487) $ik[9]++; //Кость болотного тролля
								if($pl['items_id'] == 9486) $ik[10]++; //Черное масло
								if($pl['items_id'] == 9485) $ik[11]++; //бурая шкура
								if($pl['items_id'] == 9484) $ik[12]++; //Чешуйчатая шкура 
								if($pl['items_id'] == 9483) $ik[13]++; //Самородок мерцающего металла
								if($pl['items_id'] == 9482) $ik[14]++; //Мерцающий кристалл
								if($pl['items_id'] == 9481) $ik[15]++; //Изменчивые водоросли
								if($pl['items_id'] == 9480) $ik[16]++; //Слезы лунного мерцания
								if($pl['items_id'] == 9479) $ik[17]++; //Древний мох
								if($pl['items_id'] == 9478) $ik[18]++; //Кристальный песок
								
							}
							//получаем предметы
							if($act[1] == 9477) {
								if($ik[0] >= 1 && $ik[9] >= 1 && $ik[16] >= 6 && $ik[10] >= 4 && $ik[11] >= 7 && $ik[13] >= 13 && $ik[17] >= 8) {
									$add = $u->addItem($act[1],$u->info['id']);
									$u->deleteItemID(9496, $u->info['id'], 1);
									$u->deleteItemID(9487, $u->info['id'], 1);
									$u->deleteItemID(9480, $u->info['id'], 6);
									$u->deleteItemID(9486, $u->info['id'], 4);
									$u->deleteItemID(9485, $u->info['id'], 7);
									$u->deleteItemID(9483, $u->info['id'], 13);
									$u->deleteItemID(9479, $u->info['id'], 8);
									$txt = 'Вы создали <b>"Шлем Неотвратимого Возмездия"</b>';
								}else{
									$txt = 'У вас недостаточно ресурсов для создания <b>"Шлем Неотвратимого Возмездия"</b>';
								}
									
							}elseif($act[1] == 9476) {
									if($ik[4] >= 1 && $ik[9] >= 1 && $ik[11] >= 4 && $ik[16] >= 6 && $ik[12] >= 13 && $ik[17] >= 8) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9492, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9485, $u->info['id'], 4);
											$u->deleteItemID(9480, $u->info['id'], 6);
											$u->deleteItemID(9484, $u->info['id'], 13);
											$u->deleteItemID(9479, $u->info['id'], 8);
										$txt = 'Вы создали <b>"Перчатки Неуловимой Мощи"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Перчатки Неуловимой Мощи"</b>';
									}
								}elseif($act[1] == 9474) {
									if($ik[4] >= 1 && $ik[9] >= 1 && $ik[16] >= 2 && $ik[11] >= 5 && $ik[12] >= 6 && $ik[13] >= 6 && $ik[17] >= 8) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9492, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9485, $u->info['id'], 5);
											$u->deleteItemID(9484, $u->info['id'], 6);
											$u->deleteItemID(9483, $u->info['id'], 6);
											$u->deleteItemID(9480, $u->info['id'], 2);
											$u->deleteItemID(9479, $u->info['id'], 8);
										$txt = 'Вы создали <b>"Перчатки Быстрой Расправы"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Перчатки Быстрой Расправы"</b>';
									}
								}elseif($act[1] == 9467) {
									if($ik[5] >= 1 && $ik[9] >= 1 && $ik[11] >= 7 && $ik[10] >= 3 && $ik[12] >= 5 && $ik[17] >= 15) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9491, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9485, $u->info['id'], 7);
											$u->deleteItemID(9486, $u->info['id'], 3);
											$u->deleteItemID(9484, $u->info['id'], 5);
											$u->deleteItemID(9479, $u->info['id'], 15);
										$txt = 'Вы создали <b>"Сандалии Неуловимой Мощи"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Сандалии Неуловимой Мощи"</b>';
									}
								}elseif($act[1] == 9466) {
									if($ik[5] >= 1 && $ik[9] >= 1 && $ik[10] >= 4 && $ik[11] >= 5 && $ik[12] >= 14 && $ik[13] >= 8) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9491, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9486, $u->info['id'], 4);
											$u->deleteItemID(9485, $u->info['id'], 5);
											$u->deleteItemID(9484, $u->info['id'], 14);
											$u->deleteItemID(9483, $u->info['id'], 8);
										$txt = 'Вы создали <b>"Сапоги Быстрой Расправы"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Сапоги Быстрой Расправы"</b>';
									}
								}elseif($act[1] == 9470) {
									if($ik[1] >= 1 && $ik[9] >= 1 && $ik[13] >= 9 && $ik[16] >= 7 && $ik[14] >= 3 && $ik[12] >= 8) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9495, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9483, $u->info['id'], 9);
											$u->deleteItemID(9480, $u->info['id'], 7);
											$u->deleteItemID(9482, $u->info['id'], 3);
											$u->deleteItemID(9484, $u->info['id'], 8);
										$txt = 'Вы создали <b>"Браслеты Неуловимой Мощи"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Браслеты Неуловимой Мощи"</b>';
									}
								}elseif($act[1] == 9471) {
									if($ik[1] >= 1 && $ik[9] >= 1 && $ik[11] >= 5 && $ik[10] >= 3 && $ik[13] >= 6 && $ik[17] >= 7 && $ik[12] >= 8) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9495, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9485, $u->info['id'], 5);
											$u->deleteItemID(9486, $u->info['id'], 3);
											$u->deleteItemID(9483, $u->info['id'], 6);
											$u->deleteItemID(9479, $u->info['id'], 7);
											$u->deleteItemID(9484, $u->info['id'], 8);
										$txt = 'Вы создали <b>"Наручи Неотвратимого Возмездия"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Наручи Неотвратимого Возмездия"</b>';
									}
								}elseif($act[1] == 9468) {
									if($ik[6] >= 1 && $ik[9] >= 1 && $ik[10] >= 10 && $ik[11] >= 12 && $ik[14] >= 8 && $ik[13] >= 20 && $ik[12] >= 10 && $ik[15] >= 10) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9490, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9486, $u->info['id'], 10);
											$u->deleteItemID(9485, $u->info['id'], 12);
											$u->deleteItemID(9482, $u->info['id'], 8);
											$u->deleteItemID(9483, $u->info['id'], 20);
											$u->deleteItemID(9484, $u->info['id'], 10);
											$u->deleteItemID(9481, $u->info['id'], 10);
										$txt = 'Вы создали <b>"Броня Неприступного Утеса"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Броня Неприступного Утеса"</b>';
									}
								}elseif($act[1] == 9475) {
									if($ik[7] >= 1 && $ik[9] >= 1 && $ik[16] >= 4 && $ik[10] >= 3 && $ik[11] >= 4 && $ik[13] >= 5 && $ik[17] >= 10) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9489, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9480, $u->info['id'], 4);
											$u->deleteItemID(9486, $u->info['id'], 3);
											$u->deleteItemID(9485, $u->info['id'], 4);
											$u->deleteItemID(9483, $u->info['id'], 5);
											$u->deleteItemID(9479, $u->info['id'], 10);
										$txt = 'Вы создали <b>"Пояс Неприступного Утеса"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Пояс Неприступного Утеса"</b>';
									}
								}elseif($act[1] == 9472) {
									if($ik[8] >= 1 && $ik[9] >= 1 && $ik[16] >= 7 && $ik[11] >= 7 && $ik[13] >= 11 && $ik[12] >= 6 && $ik[17] >= 6) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9488, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9480, $u->info['id'], 7);
											$u->deleteItemID(9485, $u->info['id'], 7);
											$u->deleteItemID(9483, $u->info['id'], 11);
											$u->deleteItemID(9484, $u->info['id'], 6);
											$u->deleteItemID(9479, $u->info['id'], 6);
										$txt = 'Вы создали <b>"Щит Неприступного Утеса"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Щит Неприступного Утеса"</b>';
									}
								}elseif($act[1] == 9473) {
									if($ik[3] >= 1 && $ik[9] >= 1 && $ik[16] >= 3 && $ik[10] >= 7 && $ik[14] >= 4 && $ik[13] >= 12 && $ik[17] >= 12 && $ik[18] >= 11) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9493, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9480, $u->info['id'], 3);
											$u->deleteItemID(9486, $u->info['id'], 7);
											$u->deleteItemID(9482, $u->info['id'], 4);
											$u->deleteItemID(9483, $u->info['id'], 12);
											$u->deleteItemID(9479, $u->info['id'], 12);
											$u->deleteItemID(9478, $u->info['id'], 11);
										$txt = 'Вы создали <b>"Амулет Быстрой Расправы"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Амулет Быстрой Расправы"</b>';
									}
								}elseif($act[1] == 9469) {
									//9494=1,9487=1,9480=5,9486=5,9482=5,9483=6,9478=8,9481=9  2 9 16 10 14 13 18 15
									if($ik[2] >= 1 && $ik[9] >= 1 && $ik[16] >= 5 && $ik[10] >= 5 && $ik[14] >= 5 && $ik[13] >= 6 && $ik[18] >= 8 && $ik[15] >= 9) {
										$add = $u->addItem($act[1],$u->info['id']);
											$u->deleteItemID(9494, $u->info['id'], 1);
											$u->deleteItemID(9487, $u->info['id'], 1);
											$u->deleteItemID(9480, $u->info['id'], 5);
											$u->deleteItemID(9486, $u->info['id'], 5);
											$u->deleteItemID(9482, $u->info['id'], 5);
											$u->deleteItemID(9483, $u->info['id'], 6);
											$u->deleteItemID(9478, $u->info['id'], 8);
											$u->deleteItemID(9481, $u->info['id'], 9);
										$txt = 'Вы создали <b>"Серьги Неотвратимого Возмездия"</b>';
									}else{
										$txt = 'У вас недостаточно ресурсов для создания <b>"Серьги Неотвратимого Возмездия"</b>';
									}
								}
								if($add > 0) {
									mysql_query('UPDATE `items_users` SET `maidin` = "devilscity" WHERE `id` = '.$add.' LIMIT 1');
								}
							if($txt!=''){
								$txt = '<br><font color="red">'.$txt.'</font>';	
							}
							...
							if(isset($keypr1['id'],$keypr2['id'])) {
								...
								$keypr1 = mysql_fetch_array(mysql_query('SELECT * FROM `items_users_res` WHERE `uid` = "'.$u->info['id'].'" AND `delete` = 0 AND `item_id` = "2553" LIMIT 1'));
								$keypr2 = mysql_fetch_array(mysql_query('SELECT * FROM `items_users_res` WHERE `uid` = "'.$u->info['id'].'" AND `delete` = 0 AND `item_id` = "2554" LIMIT 1'));
								
								$txt = '';
								
								if(isset($keypr1['id'],$keypr2['id'])) {
									mysql_query('DELETE FROM `items_users_res` WHERE `id` = "'.$keypr1['id'].'" OR `id` = "'.$keypr2['id'].'" LIMIT 2');
									$data = '|frompisher='.$d->info['id2'];
									$u->addItem(2555,$u->info['id'],$data);
									$txt = 'Вы успешно получили &quot;Пирамидальный ключ&quot;';
								}
