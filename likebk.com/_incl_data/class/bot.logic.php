...
class botLogic {	
	...
	static function inuser_go_btl( $id , $txt = '' ) {
		if(isset($id['id'])) {
			...
			$val = file_get_contents('http://likebk.com/jx/battle/refresh_bot.php?uid='.$id['id'].'&cron_core='.md5($id['id'].'_brfCOreW@!_'.$id['pass']).'&pass='.$id['pass'].'&'.$txt,false,$ctx);
			echo '['.$val.']<hr>';
			unset( $val );
		}
	}
	...
	static function clear_bot() {
		...
		mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.self::$bot['id'].'" AND `uid` > 0');
	...
	static function inuser_go_main( $id , $txt = '' ) {
		if(isset($id['id'])) {
			...
			file_get_contents('http://likebk.com/main_bot.php?uid='.$id['id'].'&cron_core='.md5($id['id'].'_brfCOreW@!_'.$id['pass']).'&pass='.$id['pass'].'&'.$txt,false,$ctx);
		}
	}
	...
	static function inuser_go_zv( $id , $txt = '' ) {
		/*if(isset($id['id'])) {
			...
			file_get_contents('http://likebk.com/main_bot.php?zayvka=1&r=4&uid='.$id['id'].'&cron_core='.md5($id['id'].'_brfCOreW@!_'.$id['pass']).'&pass='.$id['pass'].'&'.$txt,false,$ctx);
		}*/
	}
	...
	static function actionsCom(){

		global $u;
		if(self::$bot['pass2'] == "saintlucia1"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="1" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia2"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="2" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia3"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="3" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia4"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="4" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia5"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="5" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia6"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="6" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia7"){	
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="7" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia8"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="8" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia9"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="9" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia10"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="10" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia11"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="11" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia12"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="12" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia13"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="13" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia14"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="14" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia15"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="15" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia16"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="16" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "saintlucia17"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" AND class="17" LIMIT 1'));
		}elseif(self::$bot['pass2'] == "-1"){
			$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.(self::$bot['level']).'" ORDER BY RAND() LIMIT 1'));
		}
		echo '[RECOM->???]';
		if(isset($com['id'])) {
...
static function actionsEf(){

		global $u;
		$com = mysql_fetch_array(mysql_query('SELECT * FROM `a_bot_tree` WHERE `level` = "'.self::$bot['level'].'" ORDER BY RAND() LIMIT 1'));

		if(isset($com['id'])) {
			$ef = mysql_query('SELECT `id` FROM `eff_users` WHERE `uid` = "'.self::$bot['id'].'" LIMIT 1');
			while($eft = mysql_fetch_array($ef)){
			/*	if($eft['id_eff'] != 55 || $eft['id_eff'] != 56 || $eft['id_eff'] != 57 || $eft['id_eff'] != 58) {
						mysql_query('UPDATE `eff_users` SET `delete` = "0" WHERE `uid` = "'.self::$bot['id'].'"');
				}else{
					mysql_query('UPDATE `eff_users` SET `delete` = "'.time().'" WHERE `uid` = "'.self::$bot['id'].'"');
				}*/
				if($eft['id_eff'] != 777 || $eft['id_eff'] != 1000){
					mysql_query('DELETE FROM `eff_users` WHERE `id_eff` = "'.$eft['id_eff'].'" AND `uid` = "'.$eft['uid'].'"');
				}
			}
			//mysql_query('UPDATE `eff_users` SET `delete` = "'.time().'" WHERE `uid` = "'.self::$bot['id'].'" AND `delete` = "0"');
			...
			// 		//$it = $u->addItem($com['e'.$i],self::$bot['id']);
			// 		$eff = mysql_fetch_array(mysql_query('SELECT * FROM `eff_main` WHERE `id2` = "'.$com['e'.$i].'" LIMIT 1'));
			// 		if($eff['id2'] != 55 || $eff['id2'] != 56 || $eff['id2'] != 57 || $eff['id2'] != 58) {	
			// 			mysql_query('INSERT INTO `eff_users` (`overType`,`id_eff`,`uid`,`name`,`timeUse`,`data`,`no_Ace`) VALUES ("'.$eff['oneType'].'","'.$eff['id2'].'","'.self::$bot['id'].'","'.$eff['mname'].'","'.(time()+9640000).'","'.$eff['mdata'].'","'.$eff['noAce'].'")');
			// 		}
			// 	}
			// 	$i++;
	...
	static function start2($id){
		...
		if( isset( self::$bot['id'] ) ) {
			...
			if( self::$bot['battle'] == 0 ){
				...
				}elseif(self::$st['hpNow']<self::$st['hpAll'] || self::$st['mpNow'] < self::$st['mpAll'])
				{
					self::$st['hpNow'] = self::$st['hpAll'];
					self::$st['mpNow'] = self::$st['mpAll'];
					mysql_query('UPDATE `stats` SET `zv`= "0",`team`= "0",`hpNow` = "'.self::$st['hpNow'].'",`mpNow` = "'.self::$st['mpNow'].'" WHERE `id` = "'.self::$bot['id'].'" LIMIT 1');
					mysql_query('UPDATE `users` SET `ipreg` = "'.self::new_action().'",`mod_zvanie` = "Стажер" WHERE `id` = "'.self::$bot['id'].'" LIMIT 1');
				}else{
					//Хиляемся дальше
	...
	static function start( $id ) {		
		if(!isset($bot_last_action[$id])) {
			...
			if( isset( self::$bot['id'] ) ) {
				...
				self::_online();
				//self::actionsEf();
				/*if(self::$bot['level'] == 9 && self::$bot['pass2'] != ""){
					self::actionsCom();
				}*/
					...
					if( self::$bot['timeMain'] < time() ) {
						...
						if( self::$bot['ipreg'] == 0 ) {
							...
							if( self::$bot['zv'] == 0 ) {
								...
								/*$ins = mysql_query('INSERT INTO `zayvki` (`bot1`,`bot2`,`time`,`city`,`creator`,`type`,`time_start`,`timeout`,`min_lvl_1`,`max_lvl_1`,`min_lvl_2`,`max_lvl_2`,`tm1max`,`tm2max`,`travmaChance`,`invise`,`razdel`,`comment`,`money`,`withUser`,`tm1`,`tm2`) VALUES (
																		"0",
																		"0",
																		"'.time().'",
																		"'.self::$bot['city'].'",
									...
									if( $zv['creator'] == self::$bot['id'] ) {
										...
										if( isset( $uz2['id'] ) ) {
											...
											if( $pr == 1 ) {
												...
												$us_rom = mysql_fetch_array(mysql_query('SELECT * FROM `users` WHERE `id`="'.$zv['creator'].'"'));
												$expB = 0;
												$btl = array('players'=>'','timeout'=>$zv['timeout'],'type'=>$zv['type'],'invis'=>$zv['invis'],'noinc'=>0,'travmChance'=>0,'typeBattle'=>0,'addExp'=>$expB,'money'=>0);
												$ins = mysql_query('INSERT INTO `battle` (`room`,`time_over`,`city`,`time_start`,`players`,`timeout`,`type`,`invis`,`noinc`,`travmChance`,`typeBattle`,`addExp`,`money`,`team_win`) VALUES (
																					"'.$us_rom['room'].'",
																					"0",
																					"'.self::$bot['city'].'",
																					"'.time().'",
			...
			if( isset( self::$bot['id'] ) ) {
				...
				if( self::$bot['battle'] > 0 ) {
					...
					if( self::$bot['timeMain'] < time() ) {
						...
						//
						echo '['.self::$bot['login'].'] -> '.self::$bot['ipreg'].'';
				...
				if( $pr == 0 ) {
					...
					if( $rz == 4 ) {
						...
						*/	
						...
						/*if( self::$bot['level'] <= 8 ) {
							...
							if( $pl['min_lvl_1'] == self::$bot['level'] && $pl['max_lvl_1'] == self::$bot['level'] ) {							
								$tm1c = mysql_fetch_array(mysql_query('SELECT COUNT(*) FROM `stats` WHERE `zv` = "'.$pl['id'].'" LIMIT 1'));
								if( $tm1c[0] < $pl['usermax'] ) {
									if( rand(0,100) < 50 ) {
										$tm[1] = 1;			
									}
								}
					...
					if($go == 1 && ( $tm[1] != 0 || $tm[2] != 0 )) {
						...
						if( $tm > 0 || $rz == 5 ) {
					    	...
					    	mysql_query('UPDATE `stats` SET `hpNow` = "1000000" , `zv` = "'.$pl['id'].'",`team` = "'.$tm.'" WHERE `id` = "'.self::$bot['id'].'" LIMIT 1');
