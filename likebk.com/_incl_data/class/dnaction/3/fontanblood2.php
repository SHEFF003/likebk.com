<?
if( isset($s[1]) && $s[1] == '3/fontanblood2' ) {
	/*
	...
	*/
	...
	if( $vad['go'] == true ) {
		...
		$itm2 = mysql_fetch_array(mysql_query('SELECT * FROM `items_users` WHERE `item_id` = "6963" AND `uid` = "'.$u->info['id'].'" AND `delete` = 0 AND `inShop` = 0 AND `inTransfer` = 0 LIMIT 1'));
		if(!isset($itm1['id']) || !isset($itm2['id'])) {
			$r = 'Чтобы получить &quot;Мистическое Кольцо Алтаря&quot; вам необходимо принести: Кольцо Алтаря и эссенция Разума.';
		}else{
			mysql_query('DELETE FROM `items_users` WHERE `id` = "'.$itm1['id'].'" OR `id` = "'.$itm2['id'].'"');
			$u->addItem(7025,$u->info['id']);
			$r = 'Вы получили &quot;Мистическое Кольцо Алтаря&quot; в обмен на: Кольцо Алтаря и эссенция Разума.';
		}
	}
