			...
			if( $this->info['team_win'] == -1 ) {
				...
				while( $pl = mysql_fetch_array($sp) ) {
					...
					if($this->info['razdel'] == 5 && (date('m') == '12' || (date('m') == '01' && date('d') <= 14))){
						...
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
					}elseif($this->info['razdel'] == 5 && (date('m') == '08' || (date('m') == '08' && date('d') <= 30))){
						$pl['sneg007'] = 8;
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
						...
						//$txt_prasd = 'Вы получили праздничный предмет! (Снеговик)';
						//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$this->users[$i]['city']."','".$this->users[$i]['room']."','','".$this->users[$i]['login']."','".$txt_prasd."','-1','6','0')");
					}elseif($this->info['dungeon'] > 0 && (date('m') == '08' || (date('m') == '08' && date('d') <= 30))){
						if(rand(0,100) < 11) {
							$pl['sneg007'] = 9;
						}else{
							$pl['sneg007'] = 0;
						...
						while($i<count($this->users)) {
							if( $this->users[$i]['team'] != $this->info['team_win'] ) {
								...
								if( !isset($tr_pl['id']) || $tr_pl['v1'] < 3 || ( $dart['uid1'] == $this->users[$i]['id'] || $dart['uid2'] == $this->users[$i]['id'] ) ) {
									...
									$nelich = 0;
									$nelich2 = '';
									if( /*$dart['uid1'] == $this->users[$i]['id'] || $dart['uid2'] == $this->users[$i]['id']*/ isset($dart['id']) ) {
										$nelich = array('id'=>1);
	...
	//Наносим удар между игроками
		...
		if(isset($this->atacks[$id]) && $this->atacks[$id]['lock'] == 0) {
			...
			if( $newsf5 == false ) {
				...
				/*
				if( $u->info['admin'] > 0  ) {
					echo '['.$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod'].'|'.$this->atacks[$id]['out1'].'|'.$this->atacks[$id]['tpo1'].']<br>';
					echo '['.$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['yhod'].'|'.$this->atacks[$id]['out2'].'|'.$this->atacks[$id]['tpo2'].']<hr>';
				}
				*/
