...
/*
	Прием: Поступь смерти (новая)
	Увеличивает урон за каждое попадание на 1*(лвл) не более 10*(лвл)
*/

if($_COOKIE['login'] != 'Повелитель Багов' && true == false) {
	echo 'Пока-что нельзя использовать...';
}else{
	$pvr = array();
	if( isset($pr_used_this) && isset($pr_moment) && !isset($btl->info['id']) ) {
				...
				while($j < count($at['p'][$a]['atack'])) {
					...
					$at['p'][$a]['atack'][$j][1] == 5 )) {
						if( $pvr['used'] == 0 ) {
							...
							$btl->stats[$btl->uids[$uid]]['effects'][$prv['j_priem']]['data'] = 'add_maxAtack='.($prv['data_re']['step']*$btl->users[$btl->uids[$uid]]['level']).'|step='.$prv['data_re']['step'].'';
		...
		$btl->priemAddLog( $id, 1, 2, $u->info['id'], $u->info['id'],
			'<font color^^^^#'.$prv['color2'].'>Поступь Смерти</font>',
			$prv['text'],
			($btl->hodID + 1)
		);
	}
