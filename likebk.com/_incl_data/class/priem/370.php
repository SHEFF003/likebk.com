...
if( isset($pr_tested_this) ) {
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		$pvr['x4'] = mysql_fetch_array(mysql_query('SELECT `id`,`x`,`hod` FROM `eff_users` WHERE `uid` = "'.$u2.'" AND `v2` = 368 AND `delete` = 0 LIMIT 1'));
		
		//$priem->addPriem($u2,371,'add_notactic=1|add_nousepriem=1',0,77,3,$u1,5,'шокирующийудар',0,0,0,0,1);
					...
					if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {					
						if( $pvr['x5']['x'] > 0 ) {
							...
							if( isset($btl->stats[$btl->uids[$uid1]]['antishock']) && $btl->stats[$btl->uids[$uid1]]['antishock'] > 0 && $pvr['x5']['x'] >= 3) { 
								...
								$at['p'][$a]['atack'][$j]['yron']['plog'][] = '$this->deleffm(370,'.(0+$uid).','.$btl->stats[$btl->uids[$uid]]['u_priem'][$j_id][3].');
