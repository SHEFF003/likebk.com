	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {	
			if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {
				...
				$og = mysql_fetch_array(mysql_query('SELECT `id` FROM `eff_users` WHERE `uid` = "'.$btl->users[$btl->uids[$uid2]]['id'].'" AND `v2` = 423 OR (`v2` = 141) AND `delete` = 0 LIMIT 1 '));
			...
			$btl->priemAddLog( $id, 1, 2, $u2, $u1,
				'<font color^^^^#'.$prv['color2'].'>Отравленный Дротик</font>',
				$prv['text2'],
				($btl->hodID)
			);
				//
