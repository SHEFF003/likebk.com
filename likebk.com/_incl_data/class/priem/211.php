	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {
			...
			while($j < count($at['p'][$a]['atack'])) {
				...
				$at['p'][$a]['atack'][$j][1] == 4 ||
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					if( !isset($btl->stats[$btl->uids[$u1]]['prm_'.$btl->stats[$btl->uids[$u1]]['u_priem'][$j_id][3].'_use']) && $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) && !isset($btl->stats[$btl->uids[$uid]]['um_priem'][$j_id]) ) {
						//
						$btl->stats[$btl->uids[$u1]]['prm_'.$btl->stats[$btl->uids[$u1]]['u_priem'][$j_id][3].'_use'] = true;
						//
						$btl->stats[$btl->uids[$uid]]['um_priem'][$j_id] = true;
						//
