...
if( isset($pvr['mg']['id']) ) {	
	...
	while( $pvr['di'] < 4 ) {
		if( isset($pvr['data']['atgm'][($pvr['dc']-$pvr['di'])]) ) {
			if( $pvr['rd'] < 3 ) {
				$pvr['hp'] += round($pvr['data']['atgm'][($pvr['dc']-$pvr['di'])]/5*$pvr['mg']['hod']);
	...
	*/
	...
	if( $pvr['x5'] >= 2 ) {
		$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
			...
			$prv['text2'].' (Цель полностью защищена)',
			($btl->hodID + 1)
		);
	}else{
		$btl->priemAddLog( $id, 1, 2, $u->info['id'], $this->ue['id'],
		...
		if( !isset($pvr['x5']['id']) ) {
			$this->addPriem($this->ue['id'],191,'add_antishock=1',0,77,5,$u->info['id'],5,'иммунитеткошеломить',0,0,0,0,1);		
		}else{
