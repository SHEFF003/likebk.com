...
if( isset($pr_tested_this) ) {
		...
		if( isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {	
				...
				mysql_query('DELETE FROM `eff_users` WHERE `id` = "'.$btl->stats[$btl->uids[$uid]]['u_priem'][$j_id][3].'" LIMIT 1');
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $uid == $uid1 ) {
			...
			if($btl->users[$btl->uids[$u1]]['bot_id'] > 0 && $btl->users[$btl->uids[$u1]]['dnow'] > 0) {
				$name = "Точный удар";
			}else{
				$name = "Слепая удача";
			}
			...
			while($j < count($at['p'][$a]['atack'])) {
				...
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) && $btl->stats[$btl->uids[$u2]]['enemy_am1'] < 100 ) {
						...
						//
						$at['p'][$a]['atack'][$j]['yron'] = $btl->testYronPriemAttackYK( 2 , $u1 , $u2 , $at['p'][$a]['atack'][$j]['yron'] );
						//
						$at['p'][$a]['atack'][$j]['notactic2'] = true;
						//
						$at['p'][$a]['atack'][$j]['yron']['plog'][] = '$this->deleteEff('.$btl->stats[$btl->uids[$uid]]['u_priem'][$j_id][3].');$this->priemAddLog( '.$id.', '.$a.', '.$b.', '.$u1.', '.$u2.',
							"'.$name.'",
