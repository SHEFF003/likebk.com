		...
		public function mf1Razmen($id,$at,$v) {
			...
			while( $i <= 2 ) {
				...
				while($j < count($at[$a]['atack']) && $j < 8) {
					...
					if( $at[$a]['atack'][$j][2] == $v ) {
						$dmf = 0;
						if( md5($u2) == '22dff57a976c8c584742a3e102aec373' && $this->stats[$this->uids[$u2]]['m4'] > $this->stats[$this->uids[$u1]]['m5'] ) { $dmf = 10; }
						$tyv = $this->mfs( 2 , array( 'mf' => $this->stats[$this->uids[$u2]]['m4'] , 'amf' => $this->stats[$this->uids[$u1]]['m5'] + $witm_data01['sv_m5']  ) );
						if( ( $tyv == 1 || rand(0,100) < $dmf ) && $this->atacks[$id]['out'.$b] == 0 ) {
							//увернулся, гад :)
			...
			while( $i <= 2 ) {
				...
				while($j < count($at[$a]['atack']) && $j < 8) {
					...
					if( $at[$a]['atack'][$j][2] == $v ) {	
						...
						$dmf = 0;
						if( md5($u1) == '356ae2194a594e20dd470442f078acaf' ) { $dmf = 25; }
						if( rand(0,100) > $this->stats[$this->uids[$u2]]['enemy_am1'] && ( rand(0,100) < $dmf || $this->mfs( 1 , array( 'mf' => $this->stats[$this->uids[$u1]]['m1'] + $witm_data01['sv_m1'] , 'amf' => $this->stats[$this->uids[$u2]]['m2']  ) ) == 1 ) ) {
							//кританул, гад :)
							if( $at[$a]['atack'][$j][1] == 3 ) {
								//в блок
								$at[$a]['atack'][$j][1] = 4;
				...
				while($j < count($at[$a]['atack']) && $j < 8 ) {
					...
					if( $at[$a]['atack'][$j][1] == 1 ) {
						...
						if( $this->users[$this->uids[$u1]]['animal2'] > 0 ) {
							...
							if( !isset($at[$a]['atack'][$j]['notactic']) ) {
					...
					$at[$a]['atack'][$j][1] == 5 )) {
						...
						//Травмирование
						if( $at[$a]['atack'][$j][1] == 4 || $at[$a]['atack'][$j][1] == 5 ) {
							if( !isset($at[$a]['atack'][$j]['yron']['travma']) && rand(0,1000) < 25 ) {
	...
	//Удаление эффектов
		public function deleteEff($id) {
			if(date('m') == 6){
				...
				mysql_query('DELETE FROM `еff_users` WHERE `id` = "'.mysql_real_escape_string($id).'" WHERE `time` > 0 LIMIT 1');
			}
		}
	...
	//Наносим удар между игроками
		...
		public function startAtack($id,$newsf5 = false,$nofcast = false)
		{
			...
			/*if( isset($this->stats[$this->uids[$this->atacks[$id]['uid1']]]['short_stats']) ) {
				...
				$hp01 = mysql_fetch_array(mysql_query('SELECT `id`,`hpNow` FROM `stats` WHERE `id` = "'.$this->atacks[$id]['uid1'].'" LIMIT 1'));
				if(isset($hp01['id'])){
					$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = $hp01['hpNow'];
				}else{
					echo 'ERROR_51';
				}
			}
			if( isset($this->stats[$this->uids[$this->atacks[$id]['uid2']]]['short_stats']) ) {
				...
				$hp01 = mysql_fetch_array(mysql_query('SELECT `id`,`hpNow` FROM `stats` WHERE `id` = "'.$this->atacks[$id]['uid2'].'" LIMIT 1'));
				if(isset($hp01['id'])){
					$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = $hp01['hpNow'];
				}else{
					echo 'ERROR_52';
				}
			}*/
		...
		if(isset($this->atacks[$id]) && $this->atacks[$id]['lock'] == 0) {
			...
			//$this->stats[$this->uids[12345]]['yhod'] = 1;
				...
				if( $p1h == true ) {
					...
					if( $outjk == 0 ) {
						...
						if( date('m') < 8 ) {
							...
							$rnda2 = $this->atacks[$id]['a1'];
							$rndb2 = rand(1,5);
							mysql_query('INSERT INTO `battle_act` (
								`tyman`,`battle`,`uid1`,`uid2`,`time`,`time2`,`out2`,`a1`,`b1`,`a2`,`b2`
							) VALUES (
								"1","'.$this->info['id'].'","'.$this->atacks[$id]['uid1'].'","'.$this->atacks[$id]['uid2'].'","'.OK.'","'.OK.'","1",
								"'.$rnda2.'","'.$rndb2.'",
								"'.$rnda.'","'.$rndb.'"
							)');
						}else{
					...
					if( date('m') < 8 ) {
						...
						$rndb2 = rand(1,5);		*/
						$rnda = $this->atacks[$id]['a1'];
						$rndb = $this->atacks[$id]['b2'];
						$rnda2 = $this->atacks[$id]['a2'];
						$rndb2 = $this->atacks[$id]['b1'];	
						...
						mysql_query('INSERT INTO `battle_act` (
							...
							"1","'.$this->info['id'].'","'.$this->atacks[$id]['uid2'].'","'.$usruh.'","'.OK.'","'.OK.'","1",
							"'.$rnda2.'","'.$rndb2.'",
							"'.$rnda.'","'.$rndb.'"
						)');
					}else{
					...
					*///
					...
					//
					if( date('m') < 8 ) {
						$rnda = $this->atacks[$id]['a2'];
						$rndb = $this->atacks[$id]['b1'];
						$rnda2 = $this->atacks[$id]['a1'];
						$rndb2 = $this->atacks[$id]['b2'];
						//
						...
						mysql_query('INSERT INTO `battle_act` (
							...
							"1","'.$this->info['id'].'","'.$this->atacks[$id]['uid1'].'","'.$usruh.'","'.OK.'","'.OK.'","1",
							"'.$rnda2.'","'.$rndb2.'",
							"'.$rnda.'","'.$rndb.'"
						)');
					}else{
...
//Добавляем в лог
			...
			*/
			...
			//Расчет защиты (не более 80%)
				...
				*/
				...
				$min_yrn = 0; //%
				if(md5($u1['id'])=='356ae2194a594e20dd470442f078acaf') {$min_yrn-=rand(5,10);}
	...
	//Проверяем удары, приемы, свитки, зверей
		public function testActions()
		{
			...
			//проверяем удары
				$m = mysql_query('SELECT * FROM `battle_act` WHERE `battle` = "'.$this->info['id'].'" AND (`uid1` = "'.$u->info['id'].'" OR `uid2` = "'.$u->info['id'].'" OR `bot` > 0) ORDER BY `id` ASC LIMIT 10');
				$i = 0;
				$botA = array();
				$botR = array();
	...
	//генерируем команды
		...
		public function addTravm($uid,$type,$lvl,$nelich)
		{
			...
			if(!isset($tsttr1['id'])) {
				...
				$ins = mysql_query('INSERT INTO `eff_users` (`overType`,`timeUse`,`hod`,`name`,`data`,`uid`, `id_eff`, `img2`, `timeAce`, `v1`) VALUES ("0","'.time().'","-1","Иммунитет: Защита от травм","add_notravma=1","'.$uid.'", "263", "cure1.gif","1800", "")');
			}
		}
}
