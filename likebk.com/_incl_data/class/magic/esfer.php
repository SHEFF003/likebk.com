...
if( $itm['magic_inci'] == 'esfer' ) {
	...
	if( $u->stats['hpNow'] < 1 ) {
		...
		if( $btl->info['status'] > 0 ) {
			$u->error = '<font color=red><b>Закрыть статусный бой невозможно!</b></font>';	
		}elseif( $btl->info['noinc'] > 0 ) {
			$u->error = '<font color=red><b>Бой был изолирован ранее</b></font>';	
		}else{			
			$btl->priemAddLog( $id, 1, 2, $u->info['id'], $u->info['enemy'],
				'',
				'{tm1} {u1} изолировал бой от внешнего физического мира... В бой нельзя вмешаться и живым из боя не выйти! ',
				($btl->hodID)
			);			
			$u->error = '<font color=red><b>Мерцающая сфера отделила всех вас от остального мира...  </b></font>';		
			mysql_query('UPDATE `battle` SET `noinc` = 1 , `noatack` = 1 WHERE `id` = '.$btl->info['id'].' LIMIT 1');
