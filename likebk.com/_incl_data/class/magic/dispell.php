...
if( $itm['magic_inci'] == 'dispell' ) {
	...
	*/
	...
	if($u->info['hpNow'] > 0) {
		...
		$pvr['nego'] = '';
		$pvr['nego'] .= ' `id_eff` = 320 OR `id_eff` = 319 OR `id_eff` = 318 OR `id_eff` = 317 OR `id_eff` = 316 OR `id_eff` = 424 OR `id_eff` = 501 OR `id_eff` = 503 OR `id_eff` = 504 ';
		...
		$pvr['sp'] = mysql_query('SELECT `a`.* FROM `eff_users` AS `a` LEFT JOIN `priems` AS `b` ON `b`.`id` = `a`.`v2` WHERE `a`.`uid` = "'.$u->info['id'].'" AND `a`.`delete` = 0 AND (`a`.`v1` != "priem" OR `a`.`v2` = "292") AND ( `a`.`v1` = "priem" '.$pvr['no'].' OR ( `a`.`v1` = 0 AND ('.$pvr['nego'].') ) ) AND ( `b`.`neg` > 0 OR `a`.`v2` = 191 OR `a`.`v2` = 0 ) ORDER BY `a`.`id` DESC LIMIT 1');
		...
		$u->error = '<font color=red><b>Не удалось использовать, вы мертвы</b></font>';
	}
}
?>