...
if( $itm['magic_inci'] == 'podvig' ) {
	...
	if( $u->info['align'] != 2 ) {
		if( $itm['item_id'] == 10179 ) {
			$u->error = 'Успешно использован свиток Право на Подвиг (Хижина Искателей Приключений)';
			mysql_query('DELETE FROM `hip_day` WHERE `uid` = "'.$u->info['id'].'" AND `time` = "'.date('d.m.Y').'" LIMIT 1');
			...
			$dngcity = array(
				...
				6038 => array('abandonedplain','Гора Легиона'),
				6039 => array('suncity','Грибница'),
				6040 => array('sandcity','Пещера Мглы'),
				10179 => array('hip','Хижина Искателей Приключений')
			);
			//
