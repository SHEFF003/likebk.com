...
if($st['usefromfile']=='bloodring2' && $u->info['battle'] > 0 && $u->info['hpNow'] >= 1)
{
	if($btl->info['team_win'] != -1 ) {
		...
		$u->error = 'Не хватает '.(5-ceil($u->info['tactic6'])).' <img width=8 height=8 src=http://img.likebk.com/i/micro/hp.gif> для &quot;Поглотить Кровь&quot;';
	}else{
		...
		if($st['td_cast'] > 17) {
			$u->error = 'Использование кольца возможно не более 18 раз в сутки!';
		}else{
