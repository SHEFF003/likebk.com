...
if($st['usefromfile']=='pirogi' && $u->info['battle'] > 0 && $u->info['hpNow'] >= 1)
{
	if($btl->info['team_win'] != -1 ) {
		...
		if(isset($bu['id'])) {
			...
			if( $itm['item_id'] == 10150 ) { //Отвар ПГ
				//$txt = ' В результате персонаж &quot;{u2}&quot; был подвержен действию &quot;<b>Отвар когтей ПГ (общий)</b>&quot;.';
				mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `name` = "Отвар когтей ПГ (общий)"');
				mysql_query('INSERT INTO `eff_users` (
				`hod`, `v2`, `img2`, `id_eff`, `uid`, `name`, `data`, `overType`, `timeUse`, `v1`, `user_use`
				) VALUES (
				"5", "426", "none.gif", 22, "'.$u->info['id'].'", "Отвар когтей ПГ (общий)", "add_za=200|add_zm=200", 0, "77", "priem", "'.$u->info['id'].'"
				)');
				echo '<div><font color="red"><b>Успешно использовано...</b></font></div>';
				if($u->info['sex']==1) {
			...
			}elseif( $itm['item_id'] == 6752 ) { //Страницы с заклятиями
				//
				/*
