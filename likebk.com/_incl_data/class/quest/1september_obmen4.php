...
/*
	...
	4751 - оранжевый
*/
$test = mysql_fetch_array(mysql_query('SELECT `id` FROM `items_users` WHERE `uid` = "'.$u->info['id'].'" AND (`item_id` = 6740 OR `item_id` = 6741 OR `item_id` = 6742 OR `item_id` = 6743 OR `item_id` = 6744) LIMIT 1'));
if(isset($test['id'])) {
	$txt .= '<br><b><font color=red>Вы уже получили одну из книг, нельзя получить еще одну в этом году...</b></font>';
}else{
	$pvr = array();
	$pvr['ch'] = 10; //шанс
	$pvr['tr'] = array( array(6745,1) , array(6747,1) , array(6748,1) );
	...
	if( isset($pvr['bad_itm']) && $pvr['bad_itm'] > 0 ) {
		...
		if( rand(0,100) < $pvr['ch'] ) {
			...
			$pvr['itm'] = $u->addItem(6743,$u->info['id']);
			//
