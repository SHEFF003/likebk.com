...
function lookGift(e,id,nm,img,txt,from)
{
	...
	{
		gf.style.top = mY+'px';
		gf.innerHTML = '<b><span style="float:left;">'+nm+'</span> <span style="float:right;">&nbsp; <a href="javascript:void(0);" onClick="closeGift();">X</a></span></b><br><div align="center" style="padding:5px;background-color:#f5f6ec;"><img src="http://<? echo $c['img']; ?>/i/items/'+img+'"></div>'+txt+'<div>Подарок от <a target="_blank" href="inf.php?login='+from+'">'+from+'</a></div>';
      ...
      <tr>
        ...
        <td valign="top"><table style="margin-top:18px;" cellspacing="0" cellpadding="0" width="100%">
          <TD valign=top>Сила: <? echo statInfo('s1'); ?><BR>
            ...
            Побед: <? if($inf['level']<4){ echo number_format($inf['win'], 0, ",", " "); }else{ echo '<a title="Персонаж учавствует в рейтинге" href="http://top.likebk.com/?user='.$inf['id'].'&rnd='.$code.'" target="_blank">'.number_format($inf['win'], 0, ",", " ").'</a>'; } ?><BR>
              ...
              <?
				 ...
				 if($inf['marry']!=0)
				 {
					...
					if(isset($marry['id']))
					{
						...
						echo '<a href="inf.php?'.$marry['id'].'"><img src="http://'.$c['img'].'/i/i_marry.gif" onMouseOver="top.hi(this,\''.$mrtxt.' <b>'.$marry['login'].'</b>\',event,0,0,1,0,\'\');" onMouseOut="top.hic();" onMouseDown="top.hic();"></a>';
					}
				 } 
				...
				if(($u->info['align']>=1.1 && $u->info['align']<=1.99 && $inf['admin']<1) || ($u->info['align']>=3.05 && $u->info['align']<=3.99 && $inf['admin']<1) || $u->info['admin']>0) 
				{
					...
					} elseif ($u->info['admin']==0 && (($u->info['admin']==0 && (floor($u->info['align'])==1 && $inf['align']>=3.01 && $inf['align']<=3.99) || (floor($u->info['align'])==3 && $inf['align']>=1.1 && $inf['align']<=1.99)) || ($u->info['admin']==0 && $inf['admin']>0)))  
					{
						...
						if(!isset($_GET['mod_inf'])) {
							echo '<a href="inf.php?'.$inf['id'].'&mod_inf">Показать личноое дело</a>';
						}else{
							...
							echo '<a href="inf.php?'.$inf['id'].'">Скрыть личноое дело</a>';
						}
						echo '</small><br>';
						...
						if(($u->info['align']>=1.4 && $u->info['align']<=1.99 && $u->info['align']!=1.6 && $u->info['align']!=1.75 && $inf['admin']<1) || ($u->info['align']>=3.05 && $u->info['align']<=3.99 && $u->info['align']!=3.06 && $inf['admin']<1) || $u->info['admin']>0) 
						{
							...
							if(isset($pr1['id']))
							{
								...
								if(isset($pr['id']))
								{
									$pr['login'] = 'Персонажа пригласили: <b>'.$pr['login'].'</b> ['.$pr['level'].'] <a href="inf.php?'.$pr['id'].'" target="_blank"><img src="http://'.$c['img'].'/i/inf_capitalcity.gif" title="Инф. о '.$pr['login'].'"></a><br>';
								}else{
