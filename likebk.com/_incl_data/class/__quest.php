...
class quests {	
	...
	{ // Отсеиваем одноразовые квесты
		...
		if($ins['sum'] == 'delete'){
		    ...
		    $ok=false;
		}elseif(isset($ins) && $ins['sum'] >= 2 && $rep['rep'.$quest['city']] <= 24999 && $rep['rep'.$quest['city']] >= 10000 && false == true) {
		    $ok=false; 
		} else $result[] = $quest;
		unset($ins,$t,$ok);
	    } 
	...
	public function startq_dn($id, $val=NULL)
	{
		...
		/*if( $u->info['id'] == 165800424 ) {
			$pl = mysql_fetch_array(mysql_query('SELECT * FROM `quests` WHERE `delete` = "0" AND `name` LIKE "%уша%повели%" LIMIT 1'));
		}*/
		if(isset($pl['id']))
		{
			if($val==NULL)
		...
		//Условия задания которые должны удовлетворять текущим характеристикам
			...
			if(isset($d['tk_itm'])) {
				...
				while($i < count($ex)) {
					...
					$x2r = mysql_fetch_array(mysql_query('SELECT COUNT(*) FROM `items_users_res` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 LIMIT 1'));
					
					$x2[0] = (int)$x2[0] + (int)$x2r[0]; 
					if( $x2[0] >= $ex2[1] ) {
  ...
  <div title="Выполнение задания" class="hpborder" style="position:absolute; top:-10px; width:120px; height:9px; z-index:13;"><img src="http://img.likebk.com/1x1.gif" height="9" width="1"></div>
  <div class="hp_3 senohp" style="height:9px; width:120px; position:absolute; top:-10px; z-index:11;" id="lhp-1234500000'.$pl['id'].'"><img src="http://img.likebk.com/1x1.gif" height="9" width="1"></div>
  <div title="Выполнение задания" class="hp_none" style="position:absolute; top:-10px; width:120px; height:10px; z-index:10;"><img src="http://img.likebk.com/1x1.gif" height="10"></div>
				...
				if(isset($d['tr_botitm']))
				{
					...
					while($i<count($e)) {
						...
						while($j<count($e3))
						{
							...
							$t .= '<img src="http://img.likebk.com/i/items/'.$qi['img'].'" style="max-height:12px"> <a href="items_info.php?id='.$qi['id'].'" target="_blank">'.$qi['name'].'</a>, ';
							$j++;
						}
						$t = trim($t,', ');
						$i++;
		...
		//Награда за квест
			...
			if(isset($d['add_eff']))
			{
				...
				while($x4 < count($x3)) {
					...
					if(isset($x6['id2'])) {
						$x5 .= '<img width="40" height="25" src="http://img.likebk.com/i/eff/'.$x6['img'].'" title="'.$x6['mname'].'
Время действия: '.$u->timeOut($x7[1]).'"> ';
			...
			//Собрать ресурсы
				if(isset($d['tk_itm'])) {
					...
					while($i < count($ex)) {
						...
						$x2r = mysql_fetch_array(mysql_query('SELECT COUNT(*) as count FROM `items_users_res` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 LIMIT 1'));
						$x2[0] = (int)$x2[0] + (int)$x2r[0]; 
						if( $x2[0] >= $ex2[1] ) {
							if( $x2[0] <= ($ex2[1] + round($ex2[1]*0.15)) ){
								$x2[0] = $x2[0];
						...
						$bot2 = mysql_fetch_array(mysql_query('SELECT `id`,`name`,`img` FROM `items_main` WHERE `id` = "'.mysql_real_escape_string($ex2[0]).'" LIMIT 1'));
						$x .= '&nbsp; &nbsp; &bull; <a target=_blank href=http://lib.likebk.com/items_info.php?id='.$bot2['id'].' >'.$bot2['name'].'</a> ['.$x2[0].'/'.$ex2[1].']<br>';
						$i++;
					}
					if($x!='')
					{
	...
	public function testquest(){
		...
		/* Квесты для новичков */
		if($u->info['fnq'] < 38) { //Генерируем квесты для ньюбов
			require('noob_quest.php');				
		}
	}
		...
		if($u->info['battle']==0 && $u->room['name']!='Башня Смерти') { 
			...
			if(isset($d['tk_itm'])) {
				...
				while( $i < count($ex) ) {
					...
					$x2 = mysql_fetch_array(mysql_query('SELECT COUNT(*) as count FROM `items_users` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 LIMIT 1'));
					$x2r = mysql_fetch_array(mysql_query('SELECT COUNT(*) as count FROM `items_users_res` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 LIMIT 1'));
					$x2['count'] =  (int)$x2['count'] + (int)$x2r['count'];
					if( $x2['count'] >= $ex2[1] ) {
	...
	public function questSuccesEnd( $quest , $action ) {
		...
		if( isset($quest['id']) ) {
			...
			if( isset($d['tk_itm']) OR isset($d['tkill_itm']) ) {
				...
				while( $i < count($ex) ) {
					$ex2 = explode('=',$ex[$i]);
					$x2 = mysql_fetch_array(mysql_query('SELECT COUNT(*) as count  FROM `items_users` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 AND `inShop` != 1 LIMIT 1'));
					$x2r = mysql_fetch_array(mysql_query('SELECT COUNT(*) as count  FROM `items_users_res` WHERE `item_id` = "'.mysql_real_escape_string($ex2[0]).'" AND `uid` = "'.$u->info['id'].'" AND (`delete` = 1000 OR `delete` = 0) AND `inTransfer` = 0 AND `inShop` = 0 AND `inShop` != 1 LIMIT 1'));
					$x2['count'] = (int)$x2['count'] + (int)$x2r['count']; 
					if( $x2['count'] >= $ex2[1] ) {
						if( $x2['count'] <= ($ex2[1] + round($ex2[1]*0.15)) ){
							$ex2[1] = $x2['count'];
			...
			if( isset($d['add_rep']) ) {
					...
					if($quest['kin'] == 0){
						...
						while( $ii < count($ex) ) {
							...
							while( $i < $d['all_kill'] ) {
								...
								//
								$delbot[] = 'DELETE FROM `actions` WHERE `uid` = "'.$u->info['id'].'" AND `time` > '.$action['time'].' AND `vars` = "win_bot_'.$ex2[0].'" LIMIT '.$d['all_kill'];
								//
								$i++;
							}
							$ii++;
			    ...
			    if( $quest['city'] != '' ) {
					...
					} elseif($rep['rep'.$quest['city']] < 24999 && $rep['rep'.$quest['city']] + $d['add_rep'] >= 24999 && $quest['kin'] != 2 && true == false) {
						$rep['rep'.$quest['city']] = 24999;
					} else {*/
					
					if($quest['id'] == 334 || $quest['id'] == 335) {
						$d['add_rep'] = 400;
					}elseif($quest['id'] == 336 || $quest['id'] == 337) {
						$d['add_rep'] = 600;
					}
					
						$rep['nagrada'] += $d['add_rep'];
						$u->rep['nagrada'] += $d['add_rep'];
						$rep['rep'.$quest['city']] += $d['add_rep'];
					//}
					mysql_query('UPDATE `rep` SET `rep'.$quest['city'].'` = "'.$rep['rep'.$quest['city']].'", `nagrada` = "'.$rep['nagrada'].'" WHERE `id` = "'.$rep['id'].'" LIMIT 1');
					
					
					if($quest['name'] == "Тело Кейси") {
						
						//Задорный скелет
						$bot670 = $u->testAction('`uid` = "'.$u->info['id'].'" AND `vars` = "win_bot_670" LIMIT 1',1);
						
						//Одинокий призрак
						$bot686 = $u->testAction('`uid` = "'.$u->info['id'].'" AND `vars` = "win_bot_686" LIMIT 1',1);
						
						if(isset($bot670['id'])) {
							
							$eff489 = mysql_fetch_array(mysql_query('SELECT * FROM `eff_users` WHERE `id_eff` = 489 AND `uid` = "'.$u->info['id'].'" AND `delete` = 0 LIMIT 1'));
							
							if(isset($eff489['id'])) {
								mysql_query('UPDATE `eff_users` SET `timeUse` = "'.time().'" WHERE `id` = "'.$eff489['id'].'" LIMIT 1');
								//
								mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `id_eff` = 490');
							}else{
								mysql_query('INSERT INTO `eff_users` (`id_eff`,`uid`,`name`,`data`,`overType`,`timeUse`,`user_use`,`img2`,`x`,`hod`,`file_finish`) VALUES ("489","'.$u->info['id'].'","Благодарность Задорного Скелета","add_hpProc=15","85","'.time().'","","","1","-1","")');
							}
							
						}elseif(isset($bot686['id'])) {
							
							$eff490 = mysql_fetch_array(mysql_query('SELECT * FROM `eff_users` WHERE `id_eff` = 490 AND `uid` = "'.$u->info['id'].'" AND `delete` = 0 LIMIT 1'));
							
							if(isset($eff490['id'])) {
								mysql_query('UPDATE `eff_users` SET `timeUse` = "'.time().'" WHERE `id` = "'.$eff490['id'].'" LIMIT 1');
								//
								mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `id_eff` = 489');
							}else{
								mysql_query('INSERT INTO `eff_users` (`id_eff`,`uid`,`name`,`data`,`overType`,`timeUse`,`user_use`,`img2`,`x`,`hod`,`file_finish`) VALUES ("490","'.$u->info['id'].'","Благодарность Одинокого Призрака","add_m2=175|add_m4=175","85","'.time().'","","","1","-1","")');
							}
			...
			$i = 0;
			while( $i < count($delbot) ) {
				if(isset($delbot[$i])) {
					mysql_query($delbot[$i]);
				}
				$i++;
			}
