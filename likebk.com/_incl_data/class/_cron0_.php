...
{	
	...
	public function minMana($uid,$mp,$tp)
	{
		...
		$mp -= round($mp/100*$btl->stats[$btl->uids[$uid]]['min_use_mp2']);
	...
	*/
	...
	public function magatackfiz( $u1, $u2, $yron, $type, $krit , $ymelki ) {
		...
		//
		$prm = array(
			'y' => $btl->stats[$btl->uids[$u1]]['mg'.$btl->mname[$ymelki]], //умелки
			'yv' => 0, //умения, значение коф.
			'max_krit' => 0 //вероятность крита
		);
		//
		...
		if( isset($btl->stats[$btl->uids[$u1]]['id']) && floor($btl->stats[$btl->uids[$u1]]['hpNow']) < 1 ) {
			...
			if( $btl->stats[$btl->uids[$u1]]['spasenie'] > 0 ) {
				...
				if(!isset($testdie['id'])) {
					...
					$mas['text'] = '{tm1} <b>'.$btl->stats[$btl->uids[$u1]]['login'].'</b> убит...<b title='.$btl->stats[$btl->uids[$u1]]['hpNow'].'-'.$btl->stats[$btl->uids[$u1]]['mpNow'].' >'.$btl->stats[$btl->uids[$u1]]['login'].'</b> был спасен.';
					$btl->add_log($mas);
					$btl->dietest = true; 
				}
				//
	...
	public function testMagicCast( $user , $enemy ) {
		...
		if( $u->info['dnow'] == 0 && $enemy != $u->info['id'] && $enemy != $user && $btl->users[$btl->uids[$enemy]]['id'] > 0 && $btl->users[$btl->uids[$user]]['id'] > 0 ) {
			if( $btl->stats[$btl->uids[$enemy]]['items_price2'] < floor($btl->stats[$btl->uids[$user]]['items_price2']/2) || $btl->users[$btl->uids[$enemy]]['level'] < $btl->users[$btl->uids[$user]]['level'] ) {
				//Цена кр комплекта ниже более 2 раз
				if( $u->info['team'] != $btl->users[$btl->uids[$user]]['team'] ) {
					$r = 1;
					echo '<div align=center><b><font color=red>Нельзя кастовать через слабых в сильных!</font></b></div>';
				}
			}
		}
	...
	public function pruse($id)
	{
		...
		if( $u->stats['hpNow'] < 1 || $u->info['hpNow'] < 1) {
			echo 'Вы погибли и не можете использовать приемы...';
				...
				}elseif(isset($a['id']))
				{
					...
					if(isset($id['id']))
					{
						...
						if($b>0 && $b!=false)
						{
							...
							mysql_query('UPDATE `users` SET `login` = "'.$a['name'].' (зверь '.$u->info['login'].')",`obraz` = "'.$a['obraz'].'.gif",`battle` = "'.$btl->info['id'].'",`animal2` = "'.time().'" WHERE `id` = "'.$b['id'].'" LIMIT 1');
			...
			if($p[(int)$id]>0 && $pz[(int)$id]<=0 && $u->info['hpNow']>=1)
			{
				...
				if(isset($pl['id']))
				{
					...
					if(isset($_POST['useon']) && $_POST['useon']!='' && $_POST['useon']!='none')
					{
						...
						if(isset($this->ue['id']) && $this->ue['inUser']>0) {
							...
							if(isset($ue2['id'])) {
								...
								die('Нет подходящей цели! ('.htmlspecialchars($_POST['useon'],NULL,'cp1251').')');
							}
							unset($ue2);
						}
					...
					{					
						...
						if(file_exists('../../_incl_data/class/new_priem/'.$pl['id'].'.php')) {
							//
							...
							//$u->lock('reflesh.usepriem');
							require('../../_incl_data/class/new_priem/'.$pl['id'].'.php');
							//$u->unlock();
						...
						{*/
							...
							//$this->addEffPr($pl,$id);
							/*echo 'test3->';
							if(file_exists('../../_incl_data/class'.$pl['id'].'.php')) {
								require('../../_incl_data/class'.$pl['id'].'.php');
							}else{
								echo 'useSkill'.$pl['id'].'';
							}*/
							/*echo 'test3';
	...
	public function uppz($pl,$id)
	{
		...
		if(isset($tr['tr_mpNow']))
		{
			...
			$tr['tr_mpNow'] = round($tr['tr_mpNow']/100*(100-$u->stats['min_use_mp2']));
	...
	public function seeMy($t)
	{
		...
		if( $u->info['inTurnir'] == 0 || true == true ) {
			...
			{			
				if($p[$i]>0)
				{
					...
					if( ( ($pz[$i]>0 || $notr>0) && $t==2 ) || (isset($u->stats['nopriems']) && $pl['nosh'] == 0) || $u->stats['notuse_last_pr'] == $pl['id'] || ($u->stats['hpNow'] < 1 && $u->info['battle'] > 0))
					{
						...
						$cl = '';
						$cli2  = ' class="nopriemuse" title="Нельзя использовать (#1)" ';
					}
			...
			if($u->info['animal']>0 && $t==2)
			{
				...
				{
					$cl2 = '';
					$pr .= '<a onMouseOver="top.hi(this,\'<b>Выпустить зверя</b><Br>Ваш зверь вмешивается в поединок. Можно применять один раз за бой.\',event,3,0,1,1,\'width:240px\');" onMouseOut="top.hic();" onMouseDown="top.hic();" href="javascript:void(0);" onClick="usepriem(100500,1);"><img style="margin-top:1px; '.$cl2.' margin-left:2px;" src="http://img.likebk.com/i/eff/pet_unleash.gif" width="40" height="25" /></a>';
				}else{
					$cl2 = '" title="Нельзя использовать (#2)" class="nopriemuse';
					$pr .= '<img onMouseOver="top.hi(this,\'<b>Выпустить зверя</b><Br>Ваш зверь вмешивается в поединок. Можно применять один раз за бой.\',event,3,0,1,1,\'width:240px\');" onMouseOut="top.hic();" onMouseDown="top.hic();" style="margin-top:1px; margin-left:2px;'.$cl2.'" src="http://img.likebk.com/i/eff/pet_unleash.gif" width="40" height="25" />';
	...
	public function testpriem($pl,$t = 1,$o = 0)
	{
		...
		if(isset($tr['tr_mpNow']))
		{
			...
			{
				if($btl->stats[$btl->uids[$u->info['id']]]['mpNow'] < round(round($tr['tr_mpNow']/100*(100-$btl->stats[$btl->uids[$u->info['id']]]['min_use_mp']))/100*(100-$btl->stats[$btl->uids[$u->info['id']]]['min_use_mp2'])))
				{
					$notr++;
				}
			}else{
	...
	public function priemInfo($pl,$t,$id = false)
	{
		...
		if(isset($tr['tr_mpNow']) && $tr['tr_mpNow']>0)
		{
			...
			$tr['tr_mpNow'] -= round($tr['tr_mpNow']/100*$u->stats['min_use_mp2']);
	...
	//Мощность / подавление / сопротивление и т.д.
		public function testPower($s1,$s2,$y,$t,$t2)
		{
			...
			if($t2==2)
			{
				...
				$fx = array(
					'b' => $y, //базовый урон
					'm' => round( $s1['pm'.$t] * 1.65 - $s2['antpm'.$t] ), //мощь
