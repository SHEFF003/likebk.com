	...
	//JS информация о игроке
		public function myInfo($id,$t)
		{
			...
			if(isset($this->users[$this->uids[$id]]) || $u->info['id'] == $id)
			{
				...
				$blag = mysql_fetch_array(mysql_query('SELECT * FROM `eff_users` WHERE `id_eff`="555" AND `uid`="'.$id.'" LIMIT 1'));
				if($blag){
				...
				if($tl <= 1) {					
					...
					if($this->info['izlom'] == 0) {
						...
						if($this->info['id']>0)
						{
							...
							$trl = mysql_query('SELECT `u`.`no_ip`,`u`.`id`,`u`.`notrhod`,`u`.`login`,`u`.`login2`,`u`.`sex`,`u`.`online`,`u`.`admin`,`u`.`align`,`u`.`clan`,`u`.`level`,`u`.`battle`,`u`.`obraz`,`u`.`win`,`u`.`win_p`,`u`.`lose_p`,`u`.`lose`,`u`.`nich`,`u`.`animal`,`u`.`animal2`,`st`.`stats`,`st`.`hpNow`,`st`.`mpNow`,`st`.`exp`,`st`.`dnow`,`st`.`team`,`st`.`battle_yron`,`st`.`battle_exp`,`st`.`enemy`,`st`.`battle_text`,`st`.`upLevel`,`st`.`timeGo`,`st`.`timeGoL`,`st`.`bot`,`st`.`lider`,`st`.`btl_cof`,`st`.`tactic1`,`st`.`tactic2`,`st`.`tactic3`,`st`.`tactic4`,`st`.`tactic5`,`st`.`tactic6`,`st`.`tactic7`,`st`.`x`,`st`.`y`,`st`.`battleEnd`,`st`.`priemslot`,`st`.`priems`,`st`.`priems_z`,`st`.`bet`,`st`.`clone`,`st`.`atack`,`st`.`bbexp`,`st`.`res_x`,`st`.`res_y`,`st`.`res_s`,`st`.`id`,`st`.`last_hp`,`st`.`last_pr`,`u`.`sex`,`u`.`money`,`u`.`money3`,`u`.`bot_id` FROM `users` AS `u` LEFT JOIN `stats` AS `st` ON (`u`.`id` = `st`.`id`) WHERE `u`.`battle` = "'.$this->info['id'].'" AND `st`.`hpNow` > 0 ORDER BY `st`.`bot` DESC');
							$ir = 0; $bi = 0;
							$this->users = NULL;
							$this->stats = NULL;
							$this->uids = NULL;
					...
					{					
						...
						if($i > 0) {
							...
							mysql_query('DELETE FROM `items_users` WHERE '.$dlt2.' AND `uid` > 0 LIMIT '.($i*100));
...
/*				if($this->stats[$i]['os4'] > 0){
						...
						if($u->info['enemy'] > 0)
						{
							...
							if(!isset($this->ga[$u->info['enemy']][$u->info['id']]))
							{
								if($this->stats[$this->uids[$u->info['id']]]['hpNow']>=1 && $this->stats[$this->uids[$u->info['enemy']]]['hpNow']>=1)
								{
									...
									$b = $this->uAtc['b'];	
									mysql_query('DELETE FROM `battle_act2` WHERE `battle` = "'.$this->info['id'].'" AND ((`uid2` = "'.$u->info['id'].'" AND `uid1` = "'.$u->info['enemy'].'") OR (`uid1` = "'.$u->info['id'].'" AND `uid2` = "'.$u->info['enemy'].'")) LIMIT 2');						
									$d = mysql_query('INSERT INTO `battle_act2` (`battle`,`time`,`uid1`,`uid2`,`a1`,`b1`) VALUES ("'.$this->info['id'].'","'.time().'","'.$u->info['id'].'","'.$u->info['enemy'].'","'.$a.'","'.$b.'")');
									if(!$d)
									{
										$this->e = 'Не удалось нанести удар по противнику...';
									}else{
										$this->ga[$u->info['id']][$u->info['enemy']] = mysql_insert_id();
										mysql_query('UPDATE `stats` SET `upd` = "'.md5(microtime()).'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
									}
								}								
							}else{
		...
		public function newRazmen($id,$at) {
			...
			*/
			...
			if( $this->stats[$this->uids[$uid1]]['yhod'] > 0 && $this->atacks[$id]['out1'] > 0 && $this->atacks[$id]['b1'] == 0) { // маг бьет вторым				
				...
				mysql_query('UPDATE `battle_act2` SET `out1` = 0 , `a1` = "'.$this->atacks[$id]['a1'].'" , `b1` = "'.$this->atacks[$id]['b1'].'" WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1');
				*/
			}
			if( $this->stats[$this->uids[$uid2]]['yhod'] > 0 && $this->atacks[$id]['out2'] > 0 && $this->atacks[$id]['b2'] == 0) { //маг бьет первым
				...
				mysql_query('UPDATE `battle_act2` SET `out2` = 0 , `a2` = "'.$this->atacks[$id]['a2'].'" , `b2` = "'.$this->atacks[$id]['b2'].'" WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1');
			}
	...
	//Добавляем размены в лог
		public function addlogRazmen($id,$at,$ne5) {
			...
			while($i <= 2) {
				...
				if( !isset($at[$a]['atack']) ) {
					...
					while($j < count($at[$a]['atack']) && $j < 8) {	
						...
						}elseif( (!isset($this->stats[$this->uids[$u2]]['notravma']) || $this->stats[$this->uids[$u2]]['notravma'] == 0 )  && isset($at[$a]['atack'][$j]['yron']['travma']) && $at[$a]['atack'][$j]['yron']['travma'][0] > 0 && floor($at[$a]['atack'][$j]['yron']['hp']) <= 0 ) {
			...
			if( $ne5 == false ) {
				...
				if( floor($this->stats[$this->uids[$u1]]['hpNow']) < 1 ) {
					$tst = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_act2ions` WHERE `btl` = "'.$this->info['id'].'" AND `uid` = "'.$u1.'" AND `vars` = "priem222" LIMIT 1'));
					//Игрок 1 погиб
				...
				if( floor($this->stats[$this->uids[$u2]]['hpNow']) < 1 && $u1 != $u2 ) {
					$tst = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_act2ions` WHERE `btl` = "'.$this->info['id'].'" AND `uid` = "'.$u2.'" AND `vars` = "priem222" LIMIT 1'));
					//Игрок 2 погиб
			...
			if( $dies[1] > 0 || $dies[2] > 0 ) {
				...
				if( $dies[1] == 1 ) {
					...
					if( $ne5 == false ) {
						mysql_query('DELETE FROM `battle_act2` WHERE `uid1` = "'.$u1.'"');
					}
					...
					if( $this->stats[$this->uids[$u1]]['spasenie'] > 0 && !isset($tst['id']) ) {
						...
						if(isset($this->stats[$this->uids[$u1]]['login'])) {
							if($this->users[$this->uids[$u1]]['room'] == 9) {
								if($this->users[$this->uids[$u1]]['align'] >= 1 && $this->users[$this->uids[$u1]]['align'] < 2) {
									mysql_query('UPDATE `achiev` SET `a20` = `a20` + 1 WHERE `uid` = '.$u2.' AND `a20lvl` < 3 LIMIT 1');
								}elseif($this->users[$this->uids[$u1]]['align'] >= 3 && $this->users[$this->uids[$u1]]['align'] < 4) {
									mysql_query('UPDATE `achiev` SET `a21` = `a21` + 1 WHERE `uid` = '.$u2.' AND `a21lvl` < 3 LIMIT 1');
								}elseif($this->users[$this->uids[$u1]]['align'] == 7 ) {
				...
				if( $dies[2] == 1 ) {
					...
					if( $ne5 == false ) {
						mysql_query('DELETE FROM `battle_act2` WHERE `uid2` = "'.$u2.'"');
					}
					if( $this->stats[$this->uids[$u2]]['spasenie'] > 0 && !isset($tst['id']) ) {
						//Свиток спасения
						...
						if(isset($this->stats[$this->uids[$u2]]['login'])) {
							if($this->users[$this->uids[$u2]]['room'] == 9) {
								if($this->users[$this->uids[$u2]]['align'] >= 1 && $this->users[$this->uids[$u2]]['align'] < 2) {
									mysql_query('UPDATE `achiev` SET `a20` = `a20` + 1 WHERE `uid` = '.$u1.' AND `a20lvl` < 3 LIMIT 1');
								}elseif($this->users[$this->uids[$u2]]['align'] >= 3 && $this->users[$this->uids[$u2]]['align'] < 4) {
									mysql_query('UPDATE `achiev` SET `a21` = `a21` + 1 WHERE `uid` = '.$u1.' AND `a21lvl` < 3 LIMIT 1');
								}elseif($this->users[$this->uids[$u2]]['align'] == 7) {
	...
	//Наносим удар между игроками
		...
		if(isset($this->atacks[$id]) && $this->atacks[$id]['lock'] == 0) {
			...
			if( $newsf5 == false ) {
				...
				if( $p1h == true ) {
					...
					if( $outjk == 0 ) {
						...
						$rndb = rand(1,5);	
						mysql_query('INSERT INTO `battle_act2` (
						...
						$this->atacks[$lstidhod] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act2` WHERE `id` = "'.$lstidhod.'" LIMIT 1'));
						//
					...
					mysql_query('UPDATE `battle_act2` SET
					...
					*///
					...
					//
					$this->atacks[$id] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act2` WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1'));
			...
			//$stopfx = false;
			mysql_query('DELETE FROM `battle_act2` WHERE `id` = "'.$id.'" LIMIT 1');
			if( $stopfx == true ) {
...
*/
				...
				*/				
						...
						if(isset($eff[$i])) {
							if($eff[$i]['timeUse']==77 && $eff[$i]['hod']>-1) {
								...
								if( strripos( $eff[$i]['data'] , 'minprocmanahod') == true ) {									
									...
									mysql_query('UPDATE `stats` SET `mpNow` = "'.$this->stats[$this->uids[$pvr['uid']]]['mpNow'].'" WHERE `id` = "'.$pvr['uid'].'" LIMIT 1');
				...
				/*mysql_query('DELETE FROM `battle_act2` WHERE ( `uid1` = "'.$this->atacks[$id]['uid1'].'" AND `uid2` = "'.$this->atacks[$id]['uid2'].'" ) OR
...
//Добавляем в лог
			...
			*/
			...
			//Расчет защиты (не более 80%)
				...
				*/
				...
				$defd = mysql_fetch_array(mysql_query('SELECT SUM(`vals`) FROM `battle_act2ions` WHERE `btl` = "'.$this->info['id'].'" AND `vars` = "use_defteam'.$u2['team'].'" LIMIT 1'));
				$defd = 0 + $defd[0];
				$powd = mysql_fetch_array(mysql_query('SELECT SUM(`vals`) FROM `battle_act2ions` WHERE `btl` = "'.$this->info['id'].'" AND `vars` = "use_powteam'.$u1['team'].'" LIMIT 1'));
				$powd = 0 + $powd[0];
				$defd = $defd - $powd;
	...
	//авто-смена противника
		public function autoSmena()
		{
			...
			if($j > 0) {
				if( isset($msh[$ms]) ) {
					//echo '<div><small>'.$this->smena($ms,true,true).'</small></div>';
					$this->smena($ms,true,true);
				}else{
					//echo '<div><small>'.$this->smena($ms,true).'</small></div>';
					$this->smena($ms,true);
				}
			}else{
				if( $u->info['enemy'] < 0 ) {
	...
	//Действия бота (атака)
		public function botAtack($uid,$pl,$tp)
		{
			...
			//$pl - удар 
			$test_atack = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_act2` WHERE `battle` = "'.$this->info['id'].'" AND ((
			...
			if($tp==1 && !isset($test_atack['id']))
			{
				...
				//{
					...
					$b = rand(1,5);
					//mysql_query('LOCK TABLES battle_act2 WRITE');
					
					//mysql_query('DELETE FROM `battle_act2` WHERE `battle` = "'.$this->info['id'].'" AND ((`uid2` = "'.$pl.'" AND `uid1` = "'.$uid.'") OR (`uid1` = "'.$pl.'" AND `uid2` = "'.$uid.'")) LIMIT 2');
					
					$d = mysql_query('INSERT INTO `battle_act2` (`bot`,`battle`,`time`,`uid1`,`uid2`,`a1`,`b1`) VALUES ("1","'.$this->info['id'].'","'.time().'","'.$pl.'","'.$uid.'","'.$a.'","'.$b.'")');
	...
	//Проверяем удары, приемы, свитки, зверей
		public function testActions()
		{
			...
			//проверяем удары
				$m = mysql_query('SELECT * FROM `battle_act2` WHERE `battle` = "'.$this->info['id'].'" AND (`uid1` = "'.$u->info['id'].'" OR `uid2` = "'.$u->info['id'].'" OR `bot` > 0) ORDER BY `id` ASC LIMIT 10');
				$i = 0;
				$botA = array();
				$botR = array();
				while($pl = mysql_fetch_array($m))
				{
					/*$ts1 = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_act2` WHERE `uid1` = "'.$pl['uid1'].'" AND `uid2` = "'.$pl['uid2'].'" AND `id` != "'.$pl['id'].'" LIMIT 1'));
					if(isset($ts1['id'])) {
						//mysql_query('DELETE FROM `battle_act2` WHERE `uid1` = "'.$pl['uid1'].'" AND `uid2` = "'.$pl['uid2'].'" AND `id` != "'.$pl['id'].'"');
					}
					$ts2 = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_act2` WHERE `uid2` = "'.$pl['uid1'].'" AND `uid1` = "'.$pl['uid2'].'" AND `id` != "'.$pl['id'].'" LIMIT 1'));
					if(isset($ts2['id'])) {
						//mysql_query('DELETE FROM `battle_act2` WHERE `uid2` = "'.$pl['uid1'].'" AND `uid1` = "'.$pl['uid2'].'" AND `id` != "'.$pl['id'].'"');
					}*/
					unset($ts1,$ts2);
					
					if($this->stats[$this->uids[$pl['uid1']]]['hpNow']<=0 || $this->stats[$this->uids[$pl['uid2']]]['hpNow']<=0)
					{
						mysql_query('DELETE FROM `battle_act2` WHERE `id` = "'.$pl['id'].'" LIMIT 1');
	...
	//проверка блока
		public function testZonb($uid,$uid2,$id)
		{
			...
			if( $this->stats[$this->uids[$uid2]]['yhod'] > 0 && $this->atacks[$id]['out2'] > 0 && $this->atacks[$id]['b2'] == 0) {
				$this->atacks[$id]['b2'] = rand(1,5);
				$this->atacks[$id]['a2'] = ''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).'';
				mysql_query('UPDATE `battle_act2` SET `a2` = "'.$this->atacks[$id]['a2'].'" , `b2` = "'.$this->atacks[$id]['b2'].'" WHERE `id` = "'.$this->atacks[$id]['id'].'" LIMIT 1');
			}
		...
		public function genTeams($you)
		{
			...
			while($i<count($this->uids))
			{
				if($this->stats[$i]['hpNow']>0)
				{
					...
					{
						$a1mc = '';
						$ac = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act2` WHERE `id` = "'.$this->ga[$this->users[$i]['id']][$you].'" LIMIT 1'));
					...
					{
						$a1mc = '';
						$ac = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act2` WHERE `id` = "'.$this->ag[$this->users[$i]['id']][$you].'" LIMIT 1'));
						if(isset($ac) && $ac['time']+$this->info['timeout']-15<time())
						{
							$a1mc = 'color:green;';
						}
