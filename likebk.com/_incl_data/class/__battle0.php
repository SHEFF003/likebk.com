...
class battleClass {	
	...
	public $prm = array(
		...
		*/
		...
		193 => array( 'name'	=> 'Усиленные удары', 'act' => 2 , 'type_of' => 3 ), // type_of => 3
	...
	public $e,                 //Ошибка (текст)
		   $cached = true,	   //Кэширование данных
		...
		public function testExp($y,$s1,$s2,$id1,$id2)
		{
			...
			*/
			...
			if( $this->users[$this->uids[$id2]]['clone'] == 0 ) {
				$procitm = 100;			
				$itmodet = mysql_fetch_array(mysql_query('SELECT COUNT(`id`) FROM `items_users` WHERE `inOdet` > 0 AND `inOdet` < 18 AND `uid` = "'.$s2['id'].'" LIMIT 1'));
				$itmodet = $itmodet[0];
		...
		public function myInfo($id,$t)
		{
			...
			if(isset($this->users[$this->uids[$id]]) || $u->info['id'] == $id)
			{
				if($u->info['id'] == $id || ($u->info['enemy'] == $id && $id > 0)) {
					...
					$this->stats[$this->uids[$id]] = $u->getStats($this->users[$this->uids[$id]],0,0,false,$this->cached);
				...
				if( $ur['invis_time'] > time() && $u->info['id'] != $ur['id'] ) {
					if( $ur['invis2'] > 0 ) {
						$ur = mysql_fetch_array(mysql_query('SELECT `u`.*,`st`.* FROM `users` AS `u` LEFT JOIN `stats` AS `st` ON (`u`.`id` = `st`.`id`) WHERE `u`.`id` = "'.$ur['invis2'].'" LIMIT 1'));
						if(isset($ur['id'])) {
							//$id = $ur['id'];
							$hp1 = $st['hpNow'];
							$hp1all = $st['hpAll'];
							$st = $u->getStats($ur,0,0,false,$this->cached);
							$st['hpNow'] = $hp1;
							$st['hpAll'] = $hp1all;
							$itm = $st['items'];
							$eff = $st['effects'];
	...
	//Проверяем завершение боя
		public $btlstatus = array(
			1 => array(
				array('Обычный бой',0,0,0),
				array('Великая битва',100,50000,10),
				array('Величайшая битва',200,150000,20),
				array('Историческая битва',500,250000,30),
				array('Эпохальная битва',1000,350000,50),
				array('Судный день',1500,500000,100)
			),
			2 => array(
				array('Кровавый бой',0,0,0),
				array('Кровавая битва',200,50000,10),
				array('Кровавая резня',400,150000,20),
				array('Кровавая сеча',1000,250000,30),
				array('Кровавое побоище',2000,350000,50),
				array('Судный день',3000,500000,100)
			)
		);
		public function testFinish()
		{
			...
			}elseif($this->info['team_win'] == -1)
			{
				...
				if($tl<=1) {
					...
					$tmHpNow = array();
					$tmNow = array();
					$sp = mysql_query('SELECT `u`.`login`,`u`.`invis`,`u`.`invis2`,`u`.`win_zv`,`u`.`win_time`,`u`.`invis_time`,`u`.`id`,`u`.`battle`,`s`.`team`,`s`.`hpNow` FROM `users` AS `u` LEFT JOIN `stats` AS `s` ON `s`.`id` = `u`.`id` WHERE `u`.`battle` = "'.$this->info['id'].'"');
					...
					if($this->info['izlom'] == 0) {
						...
						if($this->info['id']>0)
						{
							...
							while($pl = mysql_fetch_array($trl))
							{
								...
								if( $pl['invis_time'] ) {
									$ius = mysql_fetch_array(mysql_query('SELECT `id`,`login`,`level` FROM `users` WHERE `id` = "'.$pl['invis2'].'" LIMIT 1'));
									if(isset($ius['id'])) {
										$pl['login2'] = $ius['login'];
									}
								}
								...
								//записываем статы
								$this->stats[$ir] = $u->getStats($pl,0,0,false,$this->cached);
								$ir++;
							}
						}
						...
						if($this->info['id']>0)
						{
							...
							$pl = mysql_fetch_array($trl);
								...
								//записываем статы
								$this->stats[count($this->stats)] = $u->getStats($pl,0,0,false,$this->cached);
						}
					}
			...
			*/	
					...
					if( $u->info['inTurnir'] > 0 ) {
						...
						while($i<count($this->users)) {
							if( $this->stats[$i]['hpNow'] < 1 && $this->users[$i]['clone'] == 0 && $this->stats[$i]['clone'] == 0 && $this->users[$i]['real'] == 0 ) {
								...
								$spik = mysql_query('SELECT `id`,`item_id` FROM `items_users` WHERE `uid` = "'.$this->users[$i]['id'].'" AND `delete` ="0" LIMIT 100');
							while( $plik = mysql_fetch_array($spik) ) {
								...
								mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.$this->users[$i]['id'].'" AND `uid` > 0');
				...
				/*if($this->users[$i]['room'] == 363) {
					...
					if( $this->info['type'] == 99 || $this->info['type'] == 199 ) {
						...
						while($i<count($this->users)) {
							...
							mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$this->users[$i]['id'].'" AND `id_eff` = 263 AND `delete` = 0');
			...
			{				
				if($this->users[$i]['bot']==2) {
					...
					mysql_query('DELETE FROM `users` WHERE `id` = "'.$this->users[$i]['id'].'" LIMIT 1');
					mysql_query('DELETE FROM `stats` WHERE `id` = "'.$this->users[$i]['id'].'" LIMIT 1');
					mysql_query('DELETE FROM `items_users` WHERE `uid` = "'.$this->users[$i]['id'].'" AND `uid` > 0 LIMIT 100');
					mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$this->users[$i]['id'].'" LIMIT 100');
				}
									...
									if( isset($tbot['id2']) ) {
										...
										while($tii<count($itms)){
											...
											if($itmz[0]>0) {
												...
												if( $itmz[0] >= 6042 && $itmz[0] <= 6045 ) {
													$rnd = 20;
												}
												
												if( round($itmz[1],5) * 100000 >= rand(1,10000000) && rand(0,100) <= $rnd ) {
													$tou = 0; //какому юзеру предназначено
													/* выделяем случайного юзера из команды */
													$itmnm = mysql_fetch_array(mysql_query('SELECT `id`,`name` FROM `items_main` WHERE `id` = "'.$itmz[0].'" LIMIT 1'));
													$itmnm = $itmnm['name'];
													$noadd = 0;
													if( $itmnm['id'] == 8301 ) {
														$test = mysql_fetch_array(mysql_query('SELECT `id` FROM `izum` WHERE `uid` = "'.$u->info['id'].'" AND `time` = "'.date('d.m.Y').'" LIMIT 1'));
														if(!isset($test['id'])) {
															mysql_query('INSERT INTO `izum` (`uid`,`time`) VALUES ("'.$u->info['id'].'","'.date('d.m.Y').'")');
														}else{
															$noadd = 1;
														}
													}
...
/*			if($this->stats[$i]['os4'] > 0){
			...
			if( $u->info['id'] == 12345 ) {
				...
				//Поломка предметов
					...
					if($this->info['razdel'] == 5) {
						mysql_query('UPDATE `ng_quests` SET `haot` = `haot` + 1 WHERE `uid` = '.$this->users[$i]['id'].' AND `etap` = 3 LIMIT 1');
					}
					if($this->info['razdel'] <= 2 && $u->info['dnow'] == 0) {
						mysql_query('UPDATE `ng_quests` SET `fiz` = `fiz` + 1 WHERE `uid` = '.$this->users[$i]['id'].' AND `etap` = 4 LIMIT 1');
					}
					...
					if( md5($this->users[$i]['id']) == '97ff7a37204564b002d49b38ae170880' ) {
						$lime = array(
							12 => 600000,
							13 => 400000
						);
					...
					if($this->users[$i]['clan'] > 0) {
						...
						mysql_query('UPDATE `clan` SET `exp` = `exp` + "'.round($this->users[$i]['battle_exp']/100*$cpr).'", `exp1` = `exp1` + "'.round($this->users[$i]['battle_exp']/100*$cpr).'", `exp2` = `exp2` + "'.round($this->users[$i]['battle_exp']/100*$cpr).'", `exp3` = `exp3` + "'.round($this->users[$i]['battle_exp']/100*$cpr).'" WHERE `id` = "'.$this->users[$i]['clan'].'" LIMIT 1');
					}
...
/*				if($this->stats[$i]['os4'] > 0){
						...
						if($u->info['enemy'] > 0)
						{
							...
							if(!isset($this->ga[$u->info['enemy']][$u->info['id']]))
							{
								if($this->stats[$this->uids[$u->info['id']]]['hpNow']>=1 && $this->stats[$this->uids[$u->info['enemy']]]['hpNow']>=1)
								{
									...
									$d = mysql_query('INSERT INTO `battle_act` (`battle`,`time`,`uid1`,`uid2`,`a1`,`b1`,`bot`) VALUES ("'.$this->info['id'].'","'.time().'","'.$u->info['id'].'","'.$u->info['enemy'].'","'.$a.'","'.$b.'","0")');
									if(!$d)
									{
		...
		public function testPogB($uid,$yr,$pliid,$test = 0)
		{
			...
			{			
				...
				if( $this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['id'] == $pliid || $test == 1 ) {
					//
					$jd = $u->lookStats($this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data']);
					$j['y'] = $jd['add_pog2'];
					//
					$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data'] = str_replace('add_pog2='.$j['y'],'add_pog2=$',$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data']);
					$dt3 = $u->lookStats($this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data']);
					...
					$this->stats[$this->uids[$uid]]['set_pog'][$i]['y'] = $j['y'];
					$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data'] = str_replace('add_pog2=$','add_pog2='.$j['y'],$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data']);
					//echo '['.$uid.'='.$j['id'].'!'.$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['id'].'!'.$j['y'].'-'.$yr.'-'.$yr2.']';
					$upd = mysql_query('UPDATE `eff_users` SET `data` = "'.$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['data'].'" WHERE `id` = "'.$this->stats[$this->uids[$uid]]['effects'][$j['id']-1]['id'].'" LIMIT 1');
					if($upd) {
						//echo '['.$j['y'].'->'.$yr2.']';
	...
	//Игрок1 наносит удар Игрок2
		public function usersTestAtack($id,$uid1,$uid2,$mgf) {
			...
			//Проверка блоков
				...
				if( $this->atacks[$id]['out' . $a ] == 0 || $this->atacks[$id]['tyman'] > 0 ) {
				...
				/*if( $u->info['admin'] > 0 ) {
					//echo '<hr>['.$this->atacks[$id]['tyman'].':('.$uid1.','.$this->atacks[$id]['uid1'].'-'.$uid2.','.$this->atacks[$id]['uid2'].'):'.$this->atacks[$id]['b1'].','.$this->atacks[$id]['b2'].']';
					print_r($atack);
					echo '<br>';
					print_r($block);
					echo '<hr>';
						...
						if( $tyv == 1 && $this->atacks[$id]['out'.$b] == 0 ) {
							...
							//if( !isset($this->stats[$this->uids[$u1]]['nopryh']) || $this->stats[$this->uids[$u1]]['nopryh'] <= 0 ) {
								if( isset($this->stats[$this->uids[$u1]]['nopryh['.$j.']']) ) {
									...
									if( rand(1,100) > $this->stats[$this->uids[$u1]]['nopryh'] ) {
										$at[$a]['atack'][$j][1] = 2;
									}
								}
							/*}else{
			...
			while( $i <= 2 ) {
				...
				while($j < count($at[$a]['atack']) && $j < 8) {
					...
					if( $at[$a]['atack'][$j][2] == $v ) {	
						...
						if( rand(1,100) > $this->stats[$this->uids[$u2]]['enemy_am1'] && $this->mfs( 1 , array( 'mf' => $this->stats[$this->uids[$u1]]['m1'] + $witm_data01['sv_m1'] , 'amf' => $this->stats[$this->uids[$u2]]['m2']  ) ) == 1 ) {
							//кританул, гад :)
							if( $at[$a]['atack'][$j][1] == 3 ) {
								//в блок
								$at[$a]['atack'][$j][1] = 4;
			...
			while($i <= 2) {
				...
				if( !isset($at[$a]['atack']) ) {
					...
					while($j < count($at[$a]['atack']) && $j < 8) {	
						...
						$atkgd = 0;
						//
						/*if( $this->info['id'] == 67259 ) {
							print_r($at[$a]['atack'][$j]);
						}*/
						if( $at[$a]['atack'][$j][1] == 1 ) {
							//u1 ударил обычным ударом u2
						...
						if( (!isset($this->stats[$this->uids[$u2]]['notravma']) || $this->stats[$this->uids[$u2]]['notravma'] == 0 )  && isset($at[$a]['atack'][$j]['yron']['travma']) && $at[$a]['atack'][$j]['yron']['travma'][0] > 0 && floor($at[$a]['atack'][$j]['yron']['hp']) <= 0 ) {
							if( $this->users[$this->uids[$u2]]['timereg'] <= time() - 86400 * 3 ) {
						...
						if( isset($at[$a]['atack'][$j]['yron']['hp']) ) { 
							...
							if( $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod'] > 0 && $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['yhod'] > 0 ) {
								unset($this->stats[$this->uids[$this->atacks[$id]['uid1']]]['yhod'],$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['yhod']);
								mysql_query('DELETE FROM `eff_users` WHERE ( `uid` = "'.$this->atacks[$id]['uid1'].'" OR `uid` = "'.$this->atacks[$id]['uid2'].'" ) AND ( `v2` = 213 OR `v2` = 273 OR `v2` = 286 OR `v2` = 287 OR `v2` = 288 )');
							}
							$mas['text'] .= ' <span title=x'.$at[$a]['atack'][$j]['yron']['w_type'].'x > ['.floor($at[$a]['atack'][$j]['yron']['hp']).'/'.floor($at[$a]['atack'][$j]['yron']['hpAll']).']</span>';
							//ФИКС НР
							$this->users[$this->uids[$u2]]['hpNow'] = floor($at[$a]['atack'][$j]['yron']['hp']);
							$this->stats[$this->uids[$u2]]['hpNow'] = floor($at[$a]['atack'][$j]['yron']['hp']);
						}
						//
						if( $mas['text'] != '' ) {
							$mas['text'] = '{tm1} ' . $mas['text'];
			...
			if( $ne5 == false ) {
				...
				if( floor($this->stats[$this->uids[$u1]]['hpNow']) < 1 ) {
					...
					if( $this->stats[$this->uids[$u1]]['spasenie'] > 0 && !isset($tst['id']) ) {
						$this->dus[$u1]++;
						if( !isset($this->dus[$u1]) ) {
							$mas['text'] = '{tm1} <b>'.$this->users[$this->uids[$u1]]['login2'].'</b> убит...<b>'.$this->stats[$this->uids[$u1]]['login'].'</b> был спасен. ';
							$this->add_log($mas);
						}
						$this->dietest = true;
						if( $this->info['dungeon'] > 0 && $this->info['razdel'] != 1 ) {
							mysql_query('INSERT INTO `rating` (`iid`,`type`,`time`,`val`) VALUES ("'.$this->stats[$this->uids[$u1]]['id'].'","1","'.time().'","-0.25")');
							mysql_query('INSERT INTO `rating` (`iid`,`type`,`time`,`val`) VALUES ("'.$this->stats[$this->uids[$u2]]['id'].'","1","'.time().'","0.5")');
						}
					}
				}
				if( floor($this->stats[$this->uids[$u2]]['hpNow']) < 1 && $u1 != $u2 ) {
					...
					if( $this->stats[$this->uids[$u2]]['spasenie'] > 0 && !isset($tst['id']) ) {
						$this->dus[$u2]++;
						if( !isset($this->dus[$u2]) ) {
							$mas['text'] = '{tm1} <b>'.$this->users[$this->uids[$u2]]['login2'].'</b> убит...<b>'.$this->stats[$this->uids[$u2]]['login'].'</b> был спасен. ';
							$this->add_log($mas);
						}
						$this->dietest = true; 
						if( $this->info['dungeon'] > 0 && $this->info['razdel'] != 1 ) {
							mysql_query('INSERT INTO `rating` (`iid`,`type`,`time`,`val`) VALUES ("'.$this->stats[$this->uids[$u2]]['id'].'","1","'.time().'","-0.25")');
							mysql_query('INSERT INTO `rating` (`iid`,`type`,`time`,`val`) VALUES ("'.$this->stats[$this->uids[$u1]]['id'].'","1","'.time().'","0.5")');
						}
					}
				}
			}
			if( $dies[1] > 0 || $dies[2] > 0 ) {
				...
				if( $dies[1] == 1 ) {
					...
					if( $this->stats[$this->uids[$u1]]['spasenie'] > 0 && !isset($tst['id']) ) {
						...
						if(isset($this->stats[$this->uids[$u1]]['login'])) {
							...
							if( !isset($this->dus[$u1]) ) {
								$this->dus[$u1]++;
								$mas['text'] = '{tm1} <b>'.$this->users[$this->uids[$u1]]['login2'].'</b> погиб.';
								$this->add_log($mas);
								mysql_query('UPDATE `battle` SET `testfinish` = "'.time().'" WHERE `id` = "'.$this->info['id'].'" LIMIT 1');
							}
						}
					}
				}
				if( $dies[2] == 1 ) {
					...
					if( $this->stats[$this->uids[$u2]]['spasenie'] > 0 && !isset($tst['id']) ) {
						...
						if(isset($this->users[$this->uids[$u2]]['login'])) {
							mysql_query('UPDATE `stats` SET `last_hp` = 0 , `hpNow` = "0",`mpNow` = "0" WHERE `id` = "'.$u2.'" LIMIT 1');
							if( !isset($this->dus[$u2]) ) {
								$this->dus[$u2]++;
								$mas['text'] = '{tm1} <b>'.$this->users[$this->uids[$u2]]['login2'].'</b> погиб.';
								$this->add_log($mas);
								mysql_query('UPDATE `battle` SET `testfinish` = "'.time().'" WHERE `id` = "'.$this->info['id'].'" LIMIT 1');
							}
						}
	...
	//Расчитываем ед. урона
		public function yronGetrazmen22( $uid1,$uid2,$wp,$zona,$yhod ) {
			global $u;
			$ditest = array();
			//$oldst1 = $this->stats[$this->uids[$uid1]];
			//$oldst2 = $this->stats[$this->uids[$uid2]];	
			...
			*/
			...
			if( $this->stats[$this->uids[$uid1]]['100proboi1'] > 0 ) {
				$proboi  = 100;
				$r['pb'] = 1;
			}
	...
	//Добавляем новую статистику игрока
		public function addstatuser($id) {
			if( $id > 0 ) {
				...
				$usrd = array(
					'id'	=> $this->users[$id]['id'],
					'login' => $this->users[$id]['login'],
					'level' => $this->users[$id]['level'],
					'clan' => $this->users[$id]['clan'],
					'align' => $this->users[$id]['align'],
					'dealer' => $this->users[$id]['dealer']					
				);
								
				if( $this->users[$id]['invis_time'] != 0 ) {
					if( $this->users[$id]['invis2'] > 0 ) {
						$ius = mysql_fetch_array(mysql_query('SELECT `id`,`login`,`level`,`clan`,`align`,`dealer` FROM `users` WHERE `id` = "'.$this->users[$id]['invis2'].'" LIMIT 1'));
						if(isset($ius['id'])) {
							$usrd['id']	 = 	$ius['id'];
							$usrd['login']	 = 	$ius['login'];
							$usrd['level']	 = 	$ius['level'];
							$usrd['clan']	 = 	$ius['clan'];
							$usrd['align']	 = 	$ius['align'];
							$usrd['dealer']	 = 	$ius['dealer'];
							$invisid = $this->users[$id]['id'];
						}
					}
				}
				
				mysql_query('INSERT INTO `battle_users`
					( `battle`,`uid`,`time_enter`,`login`,`level`,`align`,`clan`,`hpAll`,`hp`,`team`,`invis` )
				VALUES
				(
					"'.$this->info['id'].'",
					"'.$usrd['id'].'",
	...
	//Проверяем приемы
		public function priemsRazmen($id, $at, $newsf5) {
			if( $at == 'fast' ) {
				$uid1 = $id[0];
				$uid2 = $id[1];
			}else{
	...
	//Удаление эффектов
		public function deleteEff($id) {
			mysql_query('DELETE FROM `eff_users` WHERE `id` = "'.mysql_real_escape_string($id).'" LIMIT 1');
		}
		
	//Приемы которые используются моментально
		public function priemsRazmenMoment( $id, $at, $newsf5 ) {
				...
				//if(!isset($this->stats[$this->uids[$u2]]['nopryh']) || $this->stats[$this->uids[$u2]]['nopryh'] == 0) {
					...
					while( $j <= count( $this->stats[$this->uids[$u1]]['u_priem'] ) ) {
						if( $this->stats[$this->uids[$u1]]['u_priem'][$j][2] > 0  && $this->stats[$this->uids[$u1]]['u_priem'][$j][5] == 1 ) {							
							if(file_exists('../../_incl_data/class/new_priem/'.$this->stats[$this->uids[$u1]]['u_priem'][$j][1].'.php')) {
								$pr_used_this = $u1;
								$pr_moment = true;
								require('../../_incl_data/class/new_priem/'.$this->stats[$this->uids[$u1]]['u_priem'][$j][1].'.php');
				...
				while( $j <= count( $this->stats[$this->uids[$u1]]['u_priem'] ) ) {
					if( $this->stats[$this->uids[$u1]]['u_priem'][$j][2] > 0 && $this->stats[$this->uids[$u1]]['u_priem'][$j][5] == 3 ) {
						if(file_exists('../../_incl_data/class/new_priem/'.$this->stats[$this->uids[$u1]]['u_priem'][$j][1].'.php')) {
							$pr_used_this = $u1;
							$pr_moment = true;
							require('new_priem/'.$this->stats[$this->uids[$u1]]['u_priem'][$j][1].'.php');
							if( isset($fx_priem) ) {
								$at = $fx_priem($id,$at,$u1,$j);
							}
							unset(${'fx_priem'});
				...
				if( $this->stats[$this->uids[$u2]]['yhod'] > 0 ) {
					...
					while( $j <= count( $this->stats[$this->uids[$u2]]['u_priem'] ) ) {
						if( $this->stats[$this->uids[$u2]]['u_priem'][$j][2] > 0 && $this->stats[$this->uids[$u2]]['u_priem'][$j][4] == 4 ) {
							if(file_exists('../../_incl_data/class/new_priem/'.$this->stats[$this->uids[$u2]]['u_priem'][$j][1].'.php')) {
								//if( $this->stats[$this->uids[$u1]]['yhod'] > 0 && !isset($this->nohodpriem[$this->stats[$this->uids[$u1]]['u_priem'][$j][1]]) ) {
										
								//}else{
									$pr_used_this = $u2;
									require('new_priem/'.$this->stats[$this->uids[$u2]]['u_priem'][$j][1].'.php');
									if( isset($fx_priem) ) {
		...
		public function testYronPriemAttack( $pid , $u1 , $u2 , $hp ) {
			...
			*/
			...
			while( $j <= count($eff) ) {
				if( isset($eff[$j]) && $eff[$j]['id_eff'] == 22 && $eff[$j]['v1'] == 'priem' && $eff[$j]['v2'] > 0 ) {
					/*
						Полная защита исправление полная защита правка
					*/
					// id прием $eff[$j]['v2']
					if( $eff[$j]['v2'] == 140 ) { //все удары
						...
						$hp['m_y'] = 1;
						$hp['m_k'] = 2;	
					}elseif( $eff[$j]['v2'] == 45 || $eff[$j]['v2'] == 211 ) { //только первый удар
						//Приемы от которых урон = 1 , то есть выдаем 0
						if( !isset($this->stats[$this->uids[$u2]]['effects'][$j]['used_typa']) ) {
							$hp['y'] = -1;
							$hp['r'] = 1;
							$hp['k'] = 2;
							$hp['m_y'] = 1;
							$hp['m_k'] = 2;
							$this->stats[$this->uids[$u2]]['effects'][$j]['used_typa']++;
						}
					}
				}
				$j++;
	...
	//Наносим удар между игроками
		...
		public $count_uid = 0;
		public function startAtack($id,$newsf5 = false,$nofcast = false,$uid3 = 0)
		{
			...
			//if( $u->info['id'] == $this->atacks[$id]['uid1'] || $u->info['id'] == $this->atacks[$id]['uid2'] ) {
				$this->count_uid++;
			//}
			//		
			...
			*/
			...
			$hpstart = array( 
				1	=>	$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] ,
				2	=>	$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow']
			);
		...
		if(isset($this->atacks[$id]) && $this->atacks[$id]['lock'] == 0) {
			...
			if( $newsf5 == false ) {
				...
				if( $p1h == true ) {
					...
					if( $outjk == 0 ) {
						...
						$rndb = $this->atacks[$id]['b2'];
						$rnda2 = rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5);
						$rndb2 = rand(1,5);	
						...
						//
						$this->startAtack($lstidhod,true,true,$this->atacks[$id]['uid1']); $this->hodID--;
					}else{
						$prv['color2'] = '000000';	
					...
					$rndb = $this->atacks[$id]['b2'];
					$rnda2 = rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5);
					/*if( $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['mageme'] > 0 ) {
						$rndb2 = -3;
						$rndb = -3;
					}else{*/
						$rndb2 = rand(1,5);	
					...
					$this->atacks[$lstidhod] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act` WHERE `id` = "'.$lstidhod.'" LIMIT 1'));
					//
					$this->startAtack($lstidhod,true,false,$this->atacks[$id]['uid1']); $this->hodID--;
					//7579 $this->teamsTake();
					//Туманный образ
					/*$prv['color2'] = '000000';
					$prv['text'] = $this->addlt(1 , 17 , $this->users[$this->uids[$uidtum1]]['sex'] , NULL);	
					...
					*///
					...
					//
					$rnda = rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5);
					$rndb = $this->atacks[$id]['b1'];
					$rnda2 = rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5).''.rand(1,5);
					//if( $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['mageme'] > 0 ) {
					//	$rndb2 = -3;
					//	$rndb = -3;
					//}else{
						$rndb2 = rand(1,5);	
					...
					$this->atacks[$lstidhod] = mysql_fetch_array(mysql_query('SELECT * FROM `battle_act` WHERE `id` = "'.$lstidhod.'" LIMIT 1'));
					//
					$this->startAtack($lstidhod,true,true,$this->atacks[$id]['uid2']); $this->hodID--;
					//7684 $this->teamsTake();
					//
					/*$prv['color2'] = '000000';
					$prv['text'] = $this->addlt(1 , 17 , $this->users[$this->uids[$uidtum1]]['sex'] , NULL);	
			...
			if( $stopfx == true ) {
				...
				//
				if( $rest > 0 ) {
					$this->stats[$this->uids[$this->atacks[$id]['uid1']]] = $u->getStats($this->atacks[$id]['uid1'],0,0,false,$this->cached,true);
					$this->stats[$this->uids[$this->atacks[$id]['uid2']]] = $u->getStats($this->atacks[$id]['uid2'],0,0,false,$this->cached,true);
				}
				//
				...
				while( $i <= 2 && $newsf5 == false ) {
					$untac = mysql_fetch_array(mysql_query('SELECT `id` FROM `eff_users` WHERE `v1` = "priem" AND `v2` = "217" AND `uid` = "'.$this->atacks[$id]['uid'.$i].'" AND `delete` = 0 LIMIT 1'));
					//$untac2 = mysql_fetch_array(mysql_query('SELECT `id` FROM `eff_users` WHERE `v1` = "priem" AND `v2` = "211" AND `uid` = "'.$this->atacks[$id]['uid'.$j].'" AND `delete` = 0 LIMIT 1'));
					if(isset($untac['id'])) {
						//1
						$pvr['sp'] = mysql_query('SELECT `a`.* FROM `eff_users` AS `a` WHERE `a`.`uid` = "'.$this->atacks[$id]['uid'.$j].'" AND `a`.`delete` = 0 AND `a`.`v1` = "priem"
				...
				*/
				...
				if( $this->atacks[$id]['out1'] == 0 ) {
					$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['mpNow'] += floor($this->stats[$this->uids[$this->atacks[$id]['uid1']]]['mpAll']*0.03 + $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hod_minmana'] );
					$this->users[$this->uids[$this->atacks[$id]['uid1']]]['mpNow'] = $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['mpNow'];
				}
				if( $this->atacks[$id]['out2'] == 0 ) {
					$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['mpNow'] += floor($this->stats[$this->uids[$this->atacks[$id]['uid2']]]['mpAll']*0.03 + $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hod_minmana'] );
					$this->users[$this->uids[$this->atacks[$id]['uid2']]]['mpNow'] = $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['mpNow'];
				}
				
				$this->atacks[$id]['newsf5'] = $newsf5;
				$this->atacks[$id]['stopfx'] = $stopfx;
				...
				$this->priemsRazmen($id,$at,$newsf5);
				$this->priemsRazmenMoment($id,$at,$newsf5);
				$this->priemsRazmen($id,$at,$newsf5);
				...
				*/
				...
				//if( $this->info['id'] != 67933 ) {
					/*if( isset($this->stats[$this->uids[$this->atacks[$id]['uid2']]]['noobrech']) ) {
						echo '';
						$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['nopryh'] = 0;
					}*/
				...
				$at = $this->priemsRestartRazmen($id,$at); //Повторная проверка приемов (если требуется) , удаление
				
				unset($this->stats[$this->uids[$this->atacks[$id]['uid2']]]['noobrech']);
				...
				//	sleep(5);
					$hpfinish = array(
						1	=> $hpstart[1] - $this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'],
						2	=> $hpstart[2] - $this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow']
					);
					
					$sp1 = mysql_query('SELECT `hpNow`,`id` FROM `stats` WHERE `id` = "'.$this->atacks[$id]['uid1'].'" OR `id` = "'.$this->atacks[$id]['uid2'].'" LIMIT 1');
					while( $pl1 = mysql_fetch_array($sp1) ) {
						if( $pl1['id'] == $this->atacks[$id]['uid1'] ) {
							$this->users[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = $pl1['hpNow'];
							$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = $pl1['hpNow'];
						}elseif( $pl1['id'] == $this->atacks[$id]['uid2'] ) {
							$this->users[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = $pl1['hpNow'];
							$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = $pl1['hpNow'];
						}
					}
					//print_r($hpfinish);
					if( $hpfinish[1] != 0 || $hpfinish[2] != 0 ) {
						$txt2 = '<span><font color="red"><b>БАГ-8218 СООБЩИТЕ АДМИНУ ['.$hpfinish[1].']['.$hpfinish[2].']</b></font></span>';
						$this->priemAddLogFast( $this->atacks[$id]['uid'.$j], 0, "Баг-8218",
							'{tm1} ' . $txt2,
						1, time() );
					}
					//Смотрим разницу НР и обновляем
				...
				*/
				...
				mysql_query('UPDATE `users` SET `notrhod` = "-1" WHERE `id` = "'.$this->atacks[$id]['uid1'].'" OR `id` = "'.$uid3.'" OR `id` = "'.$this->atacks[$id]['uid2'].'" LIMIT 3');
...
*/
				...
				if( $nofcast == false ) {
					mysql_query('DELETE FROM `battle_heal` WHERE `uid` = "'.$this->atacks[$id]['uid2'].'" OR `uid` = "'.$this->atacks[$id]['uid1'].'" OR `uid` = "'.$uid3.'"');
				}
				...
				*/
				...
				if( $this->info['id'] == 67933 ) {
					$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = 1000;
					$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = 1000;
				}
					
				if( $this->dus[$this->atacks[$id]['uid1']] > 0 ) {
					$this->users[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = 0;
					$this->stats[$this->uids[$this->atacks[$id]['uid1']]]['hpNow'] = 0;
				}
				
				if( $this->dus[$this->atacks[$id]['uid2']] > 0 ) {
					$this->users[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = 0;
					$this->stats[$this->uids[$this->atacks[$id]['uid2']]]['hpNow'] = 0;
				}
			...
			if( $this->laterScript  != '' ) {
				...
				if( $this->info['razdel'] != 5 && $this->info['izlom'] == 0 && $this->info['dungeon'] == 0 && $u->info['inTurnirnew'] == 0 && $u->info['inTurnir'] == 0 ) {
					
					if($this->info['status'] == 0 && $this->info['time_start'] > time()-60) {
						$test = mysql_fetch_array(mysql_query('SELECT `id` FROM `users` WHERE `battle` = "'.$this->info['id'].'" AND `id` IN (SELECT `id` FROM `stats` WHERE `lider` > 0) LIMIT 1'));
						if(!isset($test['id'])) {
					...
					$bojk = mysql_fetch_array(mysql_query('SELECT SUM(`battle_yron`) FROM `stats` WHERE `id` IN (SELECT `id` FROM `users` WHERE `battle` = "'.$this->info['id'].'") LIMIT 1'));
					$bojk = $bojk[0];
					$bojku = 0;
					if( $bojk >= 900000 ) {
						$bojku = 5;
					...
					if( $bojku > $this->info['status'] ) {
						...
						$this->info['timeout'] = 120;
						mysql_query('UPDATE `battle` SET `timeout` = "120", `status` = "'.$this->info['status'].'" WHERE `id` = "'.$this->info['id'].'" LIMIT 1');
					}
...
//Добавляем в лог
			...
			*/
			...
			//Расчет защиты (не более 80%)
				...
				*/
				...
				$min_yrn = 0; //%
				if(md5($u1['id'])=='632e788ac6af1d6f467b7cb973f24ce1') {$min_yrn-=rand(0,20);}
	...
	//$this->delPriem($pd[$k2][1][$k],${'p'.$k2},1,${'p'.$k2jn});
		...
		public function delPriem($pl,$u1,$t = 1,$u2 = false,$rznm = 'Очиститься Кровью',$k2nm,$yrn,$yrnt)
		{
			...
			{	
				...
				$this->stats[$this->uids[$pl['uid']]] = $u->getStats($pl['uid'],0,0,false,$this->cached);
			}else{
		...
		public function botAtack($uid,$pl,$tp)
		{
			...
			if($tp==1 && !isset($test_atack['id']))
			{
				...
				//{
					...
					$d = mysql_query('INSERT INTO `battle_act` (`bot`,`battle`,`time`,`uid1`,`uid2`,`a1`,`b1`) VALUES ("1","'.$this->info['id'].'","'.time().'","'.$pl.'","'.$uid.'","'.$a.'","'.$b.'")');
	...
	//Проверяем удары, приемы, свитки, зверей
		public function testActions()
		{
			...
			//проверяем удары
				$m = mysql_query('SELECT * FROM `battle_act` WHERE `battle` = "'.$this->info['id'].'" AND (`bot` > 0 OR `uid1` = "'.$u->info['id'].'" OR `uid2` = "'.$u->info['id'].'") ORDER BY `id` ASC LIMIT 10');
				$i = 0;
				$botA = array();
				$botR = array();
								...
								if(isset($this->users[$j]) && $this->stats[$j]['hpNow']>=1 && $this->stats[$this->uids[$bot]]['hpNow']>=1 && !isset($this->ga[$bot][$this->users[$j]['id']]) && !isset($this->ag[$bot][$this->users[$j]['id']]) && $this->users[$j]['id']!=$bot && $this->users[$j]['team']!=$this->users[$this->uids[$bot]]['team'])
								{
									...
									if(isset($this->ga[$bot][$this->users[$j]['id']]) && $this->users[$j]['bot']>0)
									{
										if($this->users[$j]['timeGo'] < time() && $this->users[$this->uids[$bot]]['timeGo']<time())
										{
											...
											$this->users[$this->uids[$bot]]['timeGo'] = time()+$psd;
											mysql_query('UPDATE `stats` SET `timeGo` = "'.(time()+$psd).'" WHERE `id` = "'.$this->users[$j]['id'].'" LIMIT 1');
										}
										...
										if($this->users[$this->uids[$bot]]['timeGo']<time() && $this->users[$j]['timeGo']<time())
										{
											...
											$this->users[$this->uids[$bot]]['timeGo'] = time()+$psd;
											mysql_query('UPDATE `stats` SET `timeGo` = "'.(time()+$psd).'" WHERE `id` = "'.$this->users[$this->uids[$bot]]['id'].'" LIMIT 1');
										}
									}
								}
							}
	...
	//выделяем пользователей
		public function teamsTake()
		{
			...
			if($this->info['id']>0)
			{
				...
				`st`.*
				FROM `users` AS `u` LEFT JOIN `stats` AS `st` ON (`u`.`id` = `st`.`id`) WHERE `u`.`battle` = "'.$this->info['id'].'"' . $ogr);
				...
				{	
					...
					if( $this->info['start2'] == 0 )
					{
						...
						if($this->users[$i]['level'] <= 7) {
							$this->users[$i]['tactic7'] = round(10/$this->stats[$i]['hpAll']*$this->stats[$i]['hpNow']);
						} elseif($this->users[$i]['level'] == 8) {
							$this->users[$i]['tactic7'] = round(20/$this->stats[$i]['hpAll']*$this->stats[$i]['hpNow']);
						} elseif($this->users[$i]['level'] == 9) {
							$this->users[$i]['tactic7'] = round(30/$this->stats[$i]['hpAll']*$this->stats[$i]['hpNow']);
						} elseif($this->users[$i]['level'] >= 10) {
							$this->users[$i]['tactic7'] = round((40+$this->stats[$i]['s7'])/$this->stats[$i]['hpAll']*$this->stats[$i]['hpNow']);
						}
						...
						if($this->users[$i]['animal'] > 0) {
							...
							if(isset($a['id'])) {
								if($a['eda']>=1) {
									...
									$mas1 = array('time'=>time(),'battle'=>$this->info['id'],'id_hod'=>3,'text'=>'','vars'=>$vLog,'zona1'=>'','zonb1'=>'','zona2'=>'','zonb2'=>'','type'=>'1');
					...
					if( $pl['invis2'] == 0 ) {
						...
						$mybu = mysql_fetch_array(mysql_query('SELECT `id` FROM `battle_users` WHERE `battle` = "'.$this->info['id'].'" AND `uid` = "'.mysql_real_escape_string($pl['invis2']).'" AND `invis` = "'.mysql_real_escape_string($pl['id']).'" LIMIT 1'));
					}
					if(!isset($mybu['id'])) {
				...
				if($this->info['start1']==0)
				{
					...
					if(isset($tststrt['id'])) {
						...
						if($upd)
						{
							...
							$vLog = 'time1='.time().'||';
							$mass = array('time'=>time(),'battle'=>$this->info['id'],'id_hod'=>2,'text'=>'test','vars'=>$vLog,'zona1'=>'','zonb1'=>'','zona2'=>'','zonb2'=>'','type'=>'1');
							/*if( rand(0,2) == 1 ) {
								$this->get_comment();
							}*/
	...
	//проверка блока
		public function testZonb($uid,$uid2,$id)
		{
			...
			if($zba[1]<=0 && $zba[1] != -3){ $zba[1] = 1; }
			if($zba[2]<=0 && $zba[2] != -3){ $zba[2] = 1; }
	...
	//генерируем команды
		...
		public function genTeams($you)
		{
			...
			while($i<count($tms))
			{
				...
				if( $teams[$tms[$i]] != '' && $u->info['team'] == $tms[$i] ) {
