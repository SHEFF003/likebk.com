...
if( isset($pr_used_this) && isset($pr_moment) ) {
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {
			...
			mysql_query('UPDATE `eff_users` SET `data` = "'.$pvr['redata'].'" WHERE `uid` = "'.$btl->stats[$btl->uids[$u1]]['effects'][$prv['j_priem']]['uid'].'" AND `bj` LIKE "%хваткальда%" LIMIT 1');
			//
		}
		// -- конец приема
		return $at;
	...
	if( isset($pvr['mg']['id']) ) {	
		...
		if( !isset($pvr['x5']['id']) ) {
			...
			//Добавляем прием
			//$this->addEffPr($pl,$id);
			mysql_query('UPDATE `eff_users` SET `delete` = "'.time().'" WHERE `id` = "'.$pvr['mg']['id'].'" LIMIT 1');
