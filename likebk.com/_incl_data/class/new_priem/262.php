...
*/
$pvr = array();
$pvr['mg'] = mysql_fetch_array(mysql_query('SELECT * FROM `eff_users` WHERE `uid` = "'.$btl->users[$btl->uids[$this->ue['id']]]['id'].'" AND `v2` = "260" AND `v1` NOT LIKE "%Спасение%" AND `user_use` = "'.$u->info['id'].'" ORDER BY `id` DESC LIMIT 1'));
if( isset($pvr['mg']['id']) ) {	
	//Действие при клике
	...
	if( $pvr['data']['add_mg2static_points'][0] < 1 ) {
		...
		$pvr['no'] .= ' AND `a`.`v2` != 188';
		$pvr['no'] .= ' AND `a`.`v2` != 139';
		$pvr['no'] .= ' AND `a`.`bj` != "спасение"';
		//$pvr['no'] .= ' AND `a`.`name` NOT LIKE "%Магический Барьер%"';
		
		$pvr['sp'] = mysql_query('SELECT `a`.* FROM `eff_users` AS `a` LEFT JOIN `priems` AS `b` ON `b`.`id` = `a`.`v2` WHERE `a`.`uid` = "'.$this->ue['id'].'" AND `a`.`delete` = 0 AND `a`.`v1` = "priem" '.$pvr['no'].' AND `b`.`neg` = 0 ORDER BY RAND() LIMIT 1');
		$pvr['pl'] = mysql_fetch_array($pvr['sp']);
		$pvr['pl']['priem'] = mysql_fetch_array(mysql_query('SELECT * FROM `priems` WHERE `id` = "'.$pvr['pl']['v2'].'" AND `id` != 233 LIMIT 1'));
