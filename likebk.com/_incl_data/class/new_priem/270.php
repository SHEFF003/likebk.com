...
if( isset($pr_momental_this_seven)) {
	...
	$fx_moment = function(  $uid, $enemy, $j_id, $yron, $profil, $inlog ) {
		//if(!isset($btl->stats[$btl->uids[$uid]]['um_priem'][$j_id])) {
			...
			//
			$btl->users[$btl->uids[$uid]]['mpNow'] = $pvr['mpNow'];
			$btl->stats[$btl->uids[$uid]]['mpNow'] = $pvr['mpNow'];
			$btl->stats[$btl->uids[$uid]]['mpNow_update'] += $pvr['mp'];
			//	
			if( $btl->stats[$btl->uids[$u1]]['id'] == $u->info['id'] ) {
				$u->info['mpNow'] = $btl->stats[$btl->uids[$uid]]['mpNow'];
				$u->stats['mpNow'] = $btl->stats[$btl->uids[$uid]]['mpNow'];
			}			
			mysql_query('UPDATE `stats` SET `mpNow` = "'.$btl->stats[$btl->uids[$uid]]['mpNow'].'" WHERE `id` = "'.$uid.'" LIMIT 1');
			//
			$btl->priemAddLogFast( $uid, 0, "Духи Льда",
				'{tm1} {u1} восстановил ману заклятием &quot;{pr}&quot;. <font color=green><b>+'.ceil($pvr['mpSee']).'</b></font> ['.$pvr['mpNow'].'/'.$pvr['mpAll'].'] (Мана) [Проверка приемов]',
			$pvr['test1'], time() );
			if( $yron < 0 ) {
				$yron = 1;
			}
				...
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {
						...
						*/
						...
						//
						$btl->stats[$btl->uids[$u1]]['mpNow_update'] += $pvr['mp'];
						$btl->users[$btl->uids[$u1]]['mpNow'] = $pvr['mpNow'];
						$btl->stats[$btl->uids[$u1]]['mpNow'] = $pvr['mpNow'];	
						$btl->stats[$btl->uids[$u1]]['mpNow_update'] = $pvr['mpNow'];	
						if( $btl->stats[$btl->uids[$u1]]['id'] == $u->info['id'] ) {
							$u->info['mpNow'] = $btl->stats[$btl->uids[$u1]]['mpNow'];
							$u->stats['mpNow'] = $btl->stats[$btl->uids[$u1]]['mpNow'];
						}
						...
						$at['p'][$a]['atack'][$j]['yron']['plog'][] = '$this->priemAddLog( '.$id.', '.$a.', '.$b.', '.$u1.', '.$u2.',
							"Духи Льда",
							"{tm1} {u1} восстановил ману заклятием &quot;{pr}&quot;. <font color=green><b>+'.ceil($pvr['mpSee']).'</b></font> ['.$pvr['mpNow'].'/'.$pvr['mpAll'].'] (Мана) [Размен]",
						'.($btl->hodID + 1).' );';						
						//
