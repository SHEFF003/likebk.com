...
/*
	Прием: Ошеломить
	Противник наносит на 50% меньше урона 1 ход
*/
$pvr = array();
if( isset($pr_tested_this) ) {
	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		$pvr['x4'] = mysql_fetch_array(mysql_query('SELECT `id`,`x`,`hod` FROM `eff_users` WHERE `uid` = "'.$u2.'" AND `v2` = 230 AND `delete` = 0 LIMIT 1'));
				...
				$at['p'][$a]['atack'][$j][1] == 5 )) {
					...
					if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$uid][$j_id]) ) {					
						if( $pvr['x5']['x'] > 0 ) {
							...
							if( isset($btl->stats[$btl->uids[$uid1]]['antishock']) && $btl->stats[$btl->uids[$uid1]]['antishock'] > 0 ) { 
								$at['p'][$a]['atack'][$j]['yron']['plog'][] = '$this->priemAddLog( '.$id.', '.$a.', '.$b.', '.$u1.', '.$u2.',
									"Ошеломить",
									"{tm1} '.$btl->addlt($a , 17 , $btl->users[$btl->uids[$u1]]['sex'] , NULL).' (Цель полностью защищена от ошеломления)",
								'.($btl->hodID + 1).' );';
								if( isset($pvr['x5']['id']) ) {
								...
								$priem->addPriem($u2,230,'add_yrnfizproc=-50|add_yrnmagproc=-50',0,77,(1-$pvr['x5']['x']),$u1,5,'ошеломить',0,0,0,0,1);
								if( !isset($pvr['x5']['id']) ) {
