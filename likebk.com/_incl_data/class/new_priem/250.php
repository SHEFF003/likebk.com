...
if( $pvr['x5']['x'] < 5 ) {
	...
	//
	if(!isset($pvr['x5']['x'])) {
		$pvr['x52'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$this->ue['id'].'" AND `v2` = 250 AND `delete` = 0 LIMIT 1'));
		mysql_query('UPDATE `eff_users` SET `hod` = 10 WHERE `id` = "'.$pvr['x52']['id'].'" LIMIT 1');
	}else{
		$pvr['x5'] = mysql_fetch_array(mysql_query('SELECT `id`,`x` FROM `eff_users` WHERE `uid` = "'.$this->ue['id'].'" AND `v2` = 250 AND `delete` = 0 LIMIT 1'));
		mysql_query('UPDATE `eff_users` SET `hod` = 10 WHERE `id` = "'.$pvr['x5']['id'].'" LIMIT 1');
	}
	//
	$prv['effx'] = '';
	if( $pvr['x5']['x'] > 0 ) {
	...
	$prv['effx'] = ' (Уязвимость физическому урона (%): +'.($xpp*3).', Уязвимость магии стихий (%): +'.($xpp*3).', Эффект лечения (%): -'.($xpp*7).')';
	//
	$prv['color2'] = '000000';
