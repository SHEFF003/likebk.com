	...
	$fx_priem = function(  $id , $at , $uid, $j_id ) {
		...
		if( $a > 0 ) {
			...
			if( $pvr['used'] == 0 && !isset($at['p'][$a]['priems']['kill'][$u1][$j_id]) ) {
				...
				if($btl->users[$btl->uids[$u1]]['bot_id'] == 297) {
					$name = 'Пристальный Взгляд';
				}else{
					$name = 'Разгадать тактику';
				}
				...
				if(isset($pvr['nouse']['id'])) {
					...
					1, time() );					
					//
					$pvr['sp'] = mysql_query('SELECT * FROM `eff_users` WHERE `uid` = "'.$u2.'" AND `delete` = 0 AND `v2` != "324" LIMIT 1');
