...
if($u->room['name']!='Рулетка')
{
	...
	*/
				...
				if($good==1)
				{
					...
					mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$u->info['city']."','".$u->info['room']."','','','<small>[<b><font color=0066ff>Крупье</font></b>] Игрок <b>".mysql_real_escape_string($u->info['login'])."</b> сделал ставку: ".(0+((int)$_GET['coin'])).".00 екр. на ".$stvka.", игра №".$gid['id']."</small>','".time()."','6','0')");
	...
	if($add==true)
	{
			...
			if($end)
			{
				...
				while($pl2 = mysql_fetch_array($sp2))
				{
					...
					if($upd)
					{
						...
						if($wn > 0 || $wn2 > 0) {
							$u->addDelo(1,$pl2['uid'],'&quot;<font color=red>Casino.'.$u->info['city'].'</font>&quot;: Выиграл '.$nmn.' еекр.',time(),$u->info['city'],'Casino.'.$u->info['city'].'',0,0);
		...
		if(count($u_w[0]) > 0) {
			//mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$pl['city']."','225','Крупье','','Игрок <b>".$pl['login']."</b> выиграл <b>".$nmn.".00</b> екр.','".time()."','6','0')");
			$i = 0;
			while($i < count($u_w[0])) {
				if($u_w[1][$u_w[0][$i]] > 0) {
				...
				FROM `users` AS `u` WHERE `u`.`id`="'.mysql_real_escape_string($u_w[0][$i]).'" LIMIT 1'));
					...
					mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','".$u->info['city']."','389','','','<small>[<b><font color=0066ff>Крупье</font></b>] Игрок <b>".mysql_real_escape_string($infu['login'])."</b> выиграл <b>".$u_w[1][$u_w[0][$i]].".00</b> еекр., ставка: ".(0+$u_w[3][$u_w[0][$i]]).".00 еекр., игра №".$u_w[2][$i]."</small>','".time()."','6','0')");
				}
				$i++;
			}
		}
