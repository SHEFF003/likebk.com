...
if(!defined('GAME'))
{
	...
	if(isset($_GET['ignore'])) {
		...
		$_POST['x'] = 4;
		$_POST['y'] = 6;
		$frd = mysql_fetch_array(mysql_query("SELECT id,login FROM `users` WHERE `id` != 12345 AND `login` = '".mysql_real_escape_string($_POST['friendadd'])."' ORDER BY `id` ASC LIMIT 1;"));
	...
	$friend = mysql_fetch_array(mysql_query("SELECT * FROM `friends` WHERE `user` = '".mysql_real_escape_string($u->info['id'])."' LIMIT 1;"));
        if($u->info['level']<1){header("Location: index.html"); exit();}
...
if($_POST['sd4'] && $_POST['friendadd']){
	...
	if(preg_match("/__/",$_POST['friendadd']) || preg_match("/--/",$_POST['friendadd'])){
		...
		$frd = mysql_fetch_array(mysql_query("SELECT `id`,`login` FROM `users` WHERE `id` != 12345 AND `login` = '".mysql_real_escape_string($_POST['friendadd'])."' LIMIT 1;"));
	}
...
if($_GET['friendremove']){
	...
	if(preg_match("/__/",$_GET['friendremove']) || preg_match("/--/",$_GET['friendremove'])){
		...
		$frd = mysql_fetch_array(mysql_query("SELECT id FROM `users` WHERE `id` != 12345 AND `login` = '".mysql_real_escape_string($_GET['friendremove'])."' LIMIT 1;"));
	}
	if(!$frd['id']){echo"<font color=red>Персонаж не найден.</font>";}
	else{
...
if($_POST['friendedit']){
	...
	if(preg_match("/__/",$_POST['friendedit']) || preg_match("/--/",$_POST['friendedit'])){
		...
		$frd = mysql_fetch_array(mysql_query("SELECT id FROM `users` WHERE `id` != 12345 AND `login` = '".mysql_real_escape_string($_POST['friendedit'])."' LIMIT 1;"));
	}
	$_POST['comment']=htmlspecialchars($_POST['comment'],NULL,'cp1251');
	if(!$frd['id']){echo"<font color=red>Персонаж не найден.</font>";}
...
?>
<SCRIPT src='http://img.likebk.com/js/sl2.21.js'></SCRIPT>
<SCRIPT src='http://img.likebk.com/commoninf.js'></SCRIPT>
<SCRIPT>
var nlevel=0;
var from = Array('+', ' ', '#');
var to = Array('%2B', '+', '%23');
...
{   var s = '<table width=250 cellspacing=1 cellpadding=0 bgcolor=CCC3AA><tr><td align=center><B>'+title+'</td><td width=20 align=right valign=top style="cursor: pointer" onclick="closehint3();"><BIG><B>x</td></tr><tr><td colspan=2>';
    ...
    s += '<INPUT type=image SRC=http://img.likebk.com/i/b__ok.gif WIDTH=25 HEIGHT=18 ALT="Сохранить" style="border:0; vertical-align: middle"></TD></TR></TABLE><INPUT TYPE=hidden name=sd4 value=""></TD></TR></TABLE></form></td></tr></table>';
...
s += '<INPUT type=image SRC=http://img.likebk.com/i/b__ok.gif WIDTH=25 HEIGHT=18 ALT="Добавить контакт" style="border:0; vertical-align: middle"></TD></TR></TABLE><INPUT TYPE=hidden name=sd4 value="1"></TD></TR></TABLE></form></td></tr></table>';
...
if (online!="") {
if (city!="") {
s+='<IMG style="filter:gray()" SRC=http://img.likebk.com/i/lock.gif WIDTH=20 HEIGHT=15 ALT="В другом городе">';
} else {
s+='<a href="javascript:void(0)" onclick="top.chat.addto(\''+login+'\',\'private\')"><IMG SRC=http://img.likebk.com/i/lock.gif WIDTH=20 HEIGHT=15 ALT="Приватно"'+(battle!=0?' style="filter: invert()"':'')+'></a>';
}
if (city!="") {
s+='<img src="http://forum.img.likebk.com/city/'+city+'.gif" width=17 height=15>';
}
...
s+='<a href="javascript:void(0)" onclick="top.chat.addto(\''+login+'\',\'to\')">'+login+'</a>['+level+']<a href=/inf.php?'+id+' target=_blank><IMG SRC=http://img.likebk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT="Информация о персонаже"></a>';
...
s+='<FONT color=gray><b>'+login+'</b>['+level+']<a href=/inf.php?'+id+' target=_blank><IMG SRC=http://img.likebk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT="Информация о персонаже"></a></td><td bgcolor=efeded nowrap>Нет в клубе';
} else {
if (nlevel==1) {
nlevel=2; //s="<BR>"+s;
}
...
s+='<FONT color=gray><i>'+login+'</i> <a href=/inf.php?login='+mlogin+' target=_blank><IMG SRC=http://img.likebk.com/i/inf_dis.gif WIDTH=12 HEIGHT=11 ALT="Информация о персонаже"></a></td><td bgcolor=efeded nowrap>нет в этом городе';
}
s+='</FONT>';
}
document.write(s+'<BR>');
...
var s='';
s+='<a href="javascript:void(0)" onclick="top.chat.addto(\''+login+'\',\'private\')"><IMG SRC=http://img.likebk.com/i/lock.gif WIDTH=20 HEIGHT=15 ALT="Приватно"></a>';
s+=' <IMG SRC=http://img.likebk.com/i/align/align'+align+'.gif WIDTH=12 HEIGHT=15>';
if (klan!='') {
s+='<A HREF="/encicl/klan/'+klan+'.html" target=_blank><IMG SRC="http://img.likebk.com/i/clan/'+klan+'.gif" WIDTH=24 HEIGHT=15 ALT=""></A>'
}
s+='<a href="javascript:void(0)" onclick="top.chat.addto(\''+login+'\',\'to\')">'+login+'</a>['+level+']<a href=/inf.php?'+id+' target=_blank><IMG SRC=http://img.likebk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT="Информация о персонаже"></a>';
document.write(s+'<BR>');
}
...
{
var s="";
if (align!="0") s+="<A HREF='"+getalignurl(align)+"' target=_blank><IMG SRC='http://img.likebk.com/i/align/align"+align+".gif' WIDTH=12 HEIGHT=15 ALT=\""+getalign(align)+"\"></A>";
if (klan) s+="<A HREF='claninf.php?"+klan+"' target=_blank><IMG SRC='http://img.likebk.com/i/clan/"+klan+".gif' WIDTH=24 HEIGHT=15 ALT=''></A>";
s+="<B>"+name+"</B>";
if (level!=-1) s+=" ["+level+"]";
if (id!=-1 && !img) s+="<A HREF='/inf.php?"+id+"' target='_blank'><IMG SRC=http://img.likebk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT='Инф. о "+name+"'></A>";
if (img) s+="<A HREF='http://capitalcity.combats.com/encicl/obraz_"+(sex?"w":"m")+"1.html?l="+img+"' target='_blank'><IMG SRC=http://img.likebk.com/i/inf.gif WIDTH=12 HEIGHT=11 ALT='Образ "+name+"'></A>";
return s;
}
</SCRIPT>
</HEAD>
...
<?
					...
					FROM `users` WHERE `id` != 12345 AND `id` = '".mysql_real_escape_string($row['notinlist'])."';"));
					if(!isset($us['id'])) {
						mysql_query('DELETE FROM `friends` WHERE
						`user` = "'.mysql_real_escape_string($row['notinlist']).'" OR
						`friend` = "'.mysql_real_escape_string($row['notinlist']).'" OR
						`enemy` = "'.mysql_real_escape_string($row['notinlist']).'" OR
						`ignor` = "'.mysql_real_escape_string($row['notinlist']).'" OR
						`notinlist` = "'.mysql_real_escape_string($row['notinlist']).'"');
					}
//function w(login,id,align,klan,level,online, city, battle)
...
<?
					...
					(select `name_mini` from `clan` WHERE `id` = users.`clan`) as `klan`,
					(select `name` from `room` WHERE `id` = users.`room`) as `room`
					FROM `users` WHERE `id` != 12345 AND `id` = '".mysql_real_escape_string($row['enemy'])."';"));
					if(!isset($us['id'])) {
						mysql_query('DELETE FROM `friends` WHERE
						`user` = "'.mysql_real_escape_string($row['enemy']).'" OR
						`friend` = "'.mysql_real_escape_string($row['enemy']).'" OR
						`enemy` = "'.mysql_real_escape_string($row['enemy']).'" OR
						`ignor` = "'.mysql_real_escape_string($row['enemy']).'" OR
						`notinlist` = "'.mysql_real_escape_string($row['enemy']).'"
						');
					}
if($u->info['city']==$us['city']){$us['city']='';}
	$n++;
...
<?
					...
					(select `name_mini` from `clan` WHERE `id` = users.`clan`) as `klan`,
					(select `name` from `room` WHERE `id` = users.`room`) as `room`
					FROM `users` WHERE `id` != 12345 AND `id` = '".mysql_real_escape_string($row['friend'])."' ORDER BY online DESC, login ASC;"));
					if(!isset($us['id'])) {
						mysql_query('DELETE FROM `friends` WHERE
						`user` = "'.mysql_real_escape_string($row['friend']).'" OR
						`friend` = "'.mysql_real_escape_string($row['friend']).'" OR
						`enemy` = "'.mysql_real_escape_string($row['friend']).'" OR
						`ignor` = "'.mysql_real_escape_string($row['friend']).'" OR
						`notinlist` = "'.mysql_real_escape_string($row['friend']).'"
						');
					}
if($u->info['city']==$us['city']){$us['city']='';}
	$i++;
...
<TD bgcolor=efeded nowrap><SCRIPT>w('<?=$us['login']?>','<?=$us['id']?>','<?=$us['align']?>','<?=$us['klan']?>','<?=$us['level']?>','<?=$rrm?>','<?=$us['city']?>','<?=$us['battle']?>');</SCRIPT></TD>
<TD bgcolor=efeded width="40%"><small><FONT class=dsc><i><?=$row['comment']?></i></FONT></small><TD>
<TD width="1%"><INPUT type=image SRC=http://img.likebk.com/i/b__ok.gif WIDTH=25 HEIGHT=18 ALT="Редактировать" style="float: right" onclick='editcontact("Редактирование контакта", "main.php?friends", "friendedit", "<?=$us['login']?>", "<SCRIPT>drwfl(\"<?=$us['login']?>\",<?=$us['id']?>,\"<?=$us['level']?>\",<?=$us['align']?>,\"<?=str_replace('"','&quot;',str_replace("'","`",$us['klan']))?>\")</SCRIPT>", "0", new Array( "Друзья","Враги","Не в группе","Игнорирование" ), "", new Array(  ), "");'></TD>
</TR>
<?
}
}	$i = 0;
...
if(isset($_GET['clearig'])) {
	mysql_query('DELETE FROM `friends` WHERE `user` = "'.$u->info['id'].'" AND `ignor` > 0');
}
					...
					(select `name_mini` from `clan` WHERE `id` = users.`clan`) as `klan`,
					(select `name` from `room` WHERE `id` = users.`room`) as `room`
					FROM `users` WHERE `id` != 12345 AND `id` = '".mysql_real_escape_string($row['ignor'])."' ORDER BY online DESC, login ASC;"));
					if(!isset($us['id'])) {
						mysql_query('DELETE FROM `friends` WHERE
						`user` = "'.mysql_real_escape_string($row['ignor']).'" OR
						`friend` = "'.mysql_real_escape_string($row['ignor']).'" OR
						`enemy` = "'.mysql_real_escape_string($row['ignor']).'" OR
						`ignor` = "'.mysql_real_escape_string($row['ignor']).'" OR
						`notinlist` = "'.mysql_real_escape_string($row['ignor']).'"
						');
					}
if($u->info['city']==$us['city']){$us['city']='';}
	$i++;
...
?>
<TR>
<TD colspan="4" nowrap align="center" style="height: 40px" valign="bottom"><h4>Игнорирование</h4> <a href="/main.php?friends&clearig">Очистить список игнора</a></TD>
</TR>
<? } ?>
<TR valign="top">
<?
...
<TD style="width: 40%; vertical-align: top; "><TABLE cellspacing=0 cellpadding=2>
<TR>
<TD style="vertical-align: top; text-align: right; ">
	<INPUT type='button' value='Обновить' onclick='location="main.php?friends=1"'>
...
$data=mysql_query("SELECT `id`, `login`, `level`, `align`, `clan` FROM `users` WHERE `id` != 12345 AND `align` != 1.6 AND `align` != 1.4 AND `align` != 1.59 AND `align` != 1.999 AND `align` != 3.06 AND `align` != 3.059 AND (`inUser` > 0 OR `online` > '".(time()-120)."') AND ((align>1 and align<2 and align!=1.2) or (align>3 and align<4)) AND `city` = '".mysql_real_escape_string($u->info['city'])."' order by align asc;");	
while ($row = mysql_fetch_array($data)) {
	if(!isset($dataum[$row['login']])) {
		$dataum[$row['login']] = true;
...
<?
$chk=mysql_fetch_array(mysql_query("SELECT `id` FROM `users` WHERE `id` != 12345 AND `online` > '".(time()-120)."' AND ((align>1 and align<2 and align!=1.2) or (align>3 and align<4)) AND `city` = '".mysql_real_escape_string($u->info['city'])."' order by align asc;"));	
