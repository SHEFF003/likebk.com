...
if(isset($_GET['delete']) && $_GET['delete'] == $an['id']) {
	...
	if($u->info['money'] >= 30){
		if($n == 1) {
			mysql_query('UPDATE `users` SET `money` = `money` - 30 WHERE `id` = "'.$an['uid'].'" LIMIT 1');
			...
			$an['rename'] = 1;
			$err = '<font color="red">Вы успешно переименовали питомца в &quot;'.$nl.'&quot;</font>';
		}else{
			$err = '<font color="red">Эта кличка не подходит</font>';
		}
	}else{
		$err = 'У вас недостаточно средств на счете';
	}
}
...
if($an['level'] > 0){
	$eda = 100 - $an['eda'];
	if($eda > 0){
		$korm_price = $eda*(($an['level']+1)*0.01);
	}
}elseif($an['level'] == 0){
	$eda = 100 - $an['eda'];
	if($eda > 0){
...
if(isset($_GET['korm'])){
	if($u->info['money'] >= $korm_price){
		$up = mysql_query('UPDATE `users` SET `money` = `money` - '.$korm_price.' WHERE `id` = "'.$an['uid'].'" LIMIT 1');
		if($up){
			mysql_query('UPDATE `users_animal` SET `eda` = `eda` + '.$eda.' WHERE `id` = "'.$an['id'].'" LIMIT 1');
			$err = 'Ваш питомец был успешно накормлен...';
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    <td width="350" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="150" align="center" valign="top"><b><?=$an['name']?></b> [<?=$an['level']?>]<br><img src="http://img.likebk.com/i/obraz/<?=$an['sex']?>/<?=$an['obraz']?>.gif" width="120" height="220"></td>
  ...
  <div>
	    <div style="float:left"><? 
	      //if($an['rename'] == 0) { ?>
	    	<input type="button" onclick="top.anren();" value="Кличка" /><? 
	      //} ?> 
	    	<input type="button" onclick="if(confirm('Выгнать зверя?')){top.frames['main'].location='main.php?pet=1&delete=<?=$an['id']?>&rnd=<?=$code?>'}" value="Выгнать" />
	    	<input class="btnnew" type="button" onclick="top.frames['main'].location='main.php?obrazanimal&rnd=<? echo $code; ?>'" value="Образ животного" />
	    	<?php if($an['eda'] < 100){?>
	    	<br>Покормить зверя за <?= $korm_price;?> кр.<br>
	    	<input type="button" onclick="if(confirm('Покормить зверя за <?= $korm_price;?> кр.?')){top.frames['main'].location='main.php?pet=1&korm=<?=$an['id']?>&rnd=<?=$code?>'}" value="Покормить" />
	    	<?}?>
	    	</div>
	    	<div style="float:right"><input type="button" onclick="top.frames['main'].location='main.php?pet=1&rnd=<?=$code?>'" value="Обновить" /> <input type="button" onclick="top.frames['main'].location='main.php?rnd=<?=$code?>'" value="Вернуться" />
	    </div>
    </div>
    <div><br />
    <?
