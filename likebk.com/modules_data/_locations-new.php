		...
		if( 
			...
			if($u->room['noatack'] == 0) {
				...
				if(isset($ua['id']) && $ua['online'] > time()-520) {
					...
					) && true == false && isset($itmart['id']) && !isset($ua['id']) ) {
					...
					) && true == false && isset($itmart['id']) && isset($uabt['id']) ) {
						echo 'Нельзя нападать в артефактах во время комендантского часа!';
					}elseif( $ua['level'] < 2 ) {
...
if(isset($_GET['loc']))
{
	...
	}elseif( (!isset($zadej) || $zadej == 0) && $u->aves['now']>=$u->aves['max'] && $u->room['name']!='Общежитие' && $u->room['name']!='Общ. Этаж 1' && $u->room['name']!='Общ. Этаж 2' && $u->room['name']!='Общ. Этаж 3')
	{
