...
?>
<h4><a href="javascript:void(0)" onClick="top.chat.addto('<?=$ckln?>','private')"><IMG border=0 SRC=http://img.likebk.com/i/lock.gif WIDTH=20 HEIGHT=15 ALT="Приватно"></a> Соклановцы</H4>
		<table bgcolor=#eeeeee>
			<tr>
			<td align=left>
<?
$res1 = mysql_query("SELECT `login`,`id`,`align`,`level`,`mod_zvanie`,`online`,`clan_prava` FROM `users` WHERE `align` > '".mysql_real_escape_string(floor($u->info['align']))."' AND `align` < '".mysql_real_escape_string(ceil($u->info['align']))."' ORDER BY `online` DESC");
while($data = mysql_fetch_array($res1)) {
				if ($data['online']>time()-120) {
					echo '<A href="javascript:void(0)" onClick="top.chat.addto(\''.$data['login'].'\',\'private\')"><img src="http://img.likebk.com/i/lock.gif" width=20 height=15></A>
					<img title="'.$res['name'].'" src="http://img.likebk.com/i/align/align'.$data['align'].'.gif"><b>'.$data['login'].'</b> ['.$data['level'].']<a href="inf.php?'.$data['id'].'" target="_blank"><img title="Инф. о '.$data['login'].'" src="http://img.likebk.com/i/inf_capitalcity.gif"></a>';
					...
					elseif ($data['online']<time()-120) {
							echo '<img src="http://img.likebk.com/i/offline.gif" width=20 height=15>
							<img src="http://img.likebk.com/i/align/align'.$data['align'].'.gif"><font color=grey><b>'.$data['login'].'</b> ['.$data['level'].']<a href="inf.php?'.$data['id'].'" target="_blank"><img title="Инф. о '.$data['login'].'" src="http://img.likebk.com/inf_dis.gif"></a>';
							if ($data['clan_prava'] == 'glava') {
								echo ' - <b>Глава клана</b>';
							} else {
...
//echo '<img title="Свет" src="http://img.likebk.com/i/align/align'.$data['align'].'.gif"><img title="'.$res['name'].'" src="http://img.likebk.com/i/clan/'.$res['name_mini'].'.gif"><b>'.$data['login'].'</b> ['.$data['level'].']<a href="inf.php?'.$data['id'].'" target="_blank"><img title="Инф. о '.$data['login'].'" src="http://img.likebk.com/i/inf_capitalcity.gif"></a> - '.$data['mod_zvanie'].'<br>';
}
?>
			</td>
			</tr>
