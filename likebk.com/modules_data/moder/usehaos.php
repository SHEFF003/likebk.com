...
{
	$tm = (int)$_POST['time'];
	$tmban = array(1=>'один день',3=>'три дня',7=>'одна неделя',14=>'две недели',30=>'один месяц',60=>'два месяца',1=>'бессрочно');
	if($tm!=1 && $tm!=3 && $tm!=7 && $tm!=14 && $tm!=30 && $tm!=60 && ($tm!=1 || ($p['haosInf']==0 && $tm==1)))
	{
		...
		if(isset($uu['id']))
		{
			...
			}elseif(floor($uu['align'])==$a && $uu['align']>$u->info['align'] && $u->info['admin']==0)
			{
				...
				$upd = mysql_query('UPDATE `users` SET `align` = "2",`clan` = "0",`haos` = "'.mysql_real_escape_string($th).'" WHERE `login` = "'.$uu['login'].'"');
				if($upd)
				{
					$sx = '';
					if($u->info['sex']==1)
