...
{
	$tm = (int)$_POST['time'];
	if($tm!=5 && $tm!=15 && $tm!=30 && $tm!=60 && $tm!=180 && $tm!=360 && $tm!=720 && $tm!=1440 && $tm!=4320 && $tm!=10080 && $tm!=30240)
	{
		...
		if(isset($uu['id']))
		{
			...
			}elseif(floor($uu['align'])==$a && $uu['align']>$u->info['align'] && $u->info['admin']==0 && true==false)
			{
				...
				if(isset($lastTime[0]) && $lastTime[0]>(time()+300)){
					...
					// Ќаложение молчани€
					$upd = mysql_query('UPDATE `users` SET `molch1` = "'.mysql_real_escape_string(time()+round($tm)*60).'" WHERE `login` = "'.$uu['login'].'"');
					if($upd)
					{
						$sx = '';
						if($u->info['sex']==1)
