...
if($u->room['file']=='dragopsh') {
		...
		if(isset($var['obj']['id'])) {
			if( $var['obj']['use'] == 0 ) {
				if( $var['obj']['type'] == 1 ) {
					...
					$var['text'] = '<img width=40 height=25 src=http://img.likebk.com/i/items/event_sunduk.gif> <b>'.$u->info['login'].'</b> открыл'.$var['sex'].' сундук...и забрал &quot;'.$var['itm']['name'].'&quot;';
					mysql_query("INSERT INTO `chat` (`dn`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`,`typeTime`,`new`) VALUES ('".$u->info['dnow']."','".$u->info['city']."','".$u->info['room']."','','','".$var['text']."','".time()."','6','0','1','1')");								
					$errors .= '<img width="40" height="25" src="http://img.likebk.com/i/items/event_sunduk.gif"> '.$u->info['login'].' открыл'.$var['sex'].' сундук...и забрал &quot;'.$var['itm']['name'].'&quot;';
					...
					$var['text'] = '<img width=40 height=25 src=http://img.likebk.com/i/items/event_pandbox.gif> <b>'.$u->info['login'].'</b> открыл'.$var['sex'].' Ящик Пандоры...';
					mysql_query("INSERT INTO `chat` (`dn`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`,`typeTime`,`new`) VALUES ('".$u->info['dnow']."','".$u->info['city']."','".$u->info['room']."','','','".$var['text']."','".time()."','6','0','1','1')");								
					$errors .= '<img width="40" height="25" src="http://img.likebk.com/i/items/event_pandbox.gif"> '.$u->info['login'].' открыл'.$var['sex'].' Ящик Пандоры...';
					$var['obj']['use'] = $u->info['id'];
					...
					$var['text'] = '<img width=40 height=25 src=http://img.likebk.com/i/items/event_heal.gif> <b>'.$u->info['login'].'</b> пополнил'.$var['sex'].' здоровье, Уровень Жизни +'.$var['hpp'].'% (+'.$var['hp'].'HP)';
					mysql_query('UPDATE `stats` SET `hpNow` = "'.$u->stats['hpNow'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
					mysql_query("INSERT INTO `chat` (`dn`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`,`typeTime`,`new`) VALUES ('".$u->info['dnow']."','".$u->info['city']."','".$u->info['room']."','','','".$var['text']."','".time()."','6','0','1','1')");								
					$errors .= '<img width="40" height="25" src="http://img.likebk.com/i/items/event_heal.gif"> '.$u->info['login'].' пополнил'.$var['sex'].' здоровье, Уровень Жизни +'.$var['hpp'].'% (+'.$var['hp'].'HP)';
					$var['obj']['use'] = $u->info['id'];
				}				
				if( $var['obj']['use'] > 0 ) {
	...
	if(isset($objs[$u->info['x']][$u->info['y']]['id'])) {
		...
		if( $mitm['type'] == 6 ) {
			...
			//Сундук
			if( $mitm['use'] == 0 ) {
				$itms .= '<a title="Открыть" href="main.php?useobj='.$mitm['id'].'"><img src="http://img.likebk.com/i/items/use_sunduk_on.gif" width="60" height="60"></a>';
			}else{
				$itms .= '<img title="Сундук был открыт" src="http://img.likebk.com/i/items/use_sunduk_off.gif" width="60" height="60">';
			}
		}elseif( $mitm['type'] == 3 ) {
			//хилка
			if( $mitm['use'] == 0 ) {
				$itms .= '<a title="Выпить" href="main.php?useobj='.$mitm['id'].'"><img src="http://img.likebk.com/i/items/use_heal_on.gif" width="60" height="60"></a>';
			}else{
				$itms .= '<img title="Эликсир был выпит" src="http://img.likebk.com/i/items/openHeal.gif" width="60" height="60">';
			}
		}elseif( $mitm['type'] == 4 ) {
			if( $mitm['use'] == 0 ) {
				//Ловушка
				...
				$var['sex'] = ''; if($u->info['sex'] == 1) { $var['sex'] = 'а'; }
				$var['text'] = '<img width=40 height=25 src=http://img.likebk.com/i/items/event_timer_trap.gif> <b>'.$u->info['login'].'</b> угодил'.$var['sex'].' в ловушку...';
				
				$varos['trap1'] = mysql_fetch_array(mysql_query('SELECT `vals`,`time` FROM `laba_act` WHERE `uid` = "'.$u->info['id'].'" AND `lib` = "'.$lab['id'].'" AND `time` > "'.time().'" AND `vars` = "trap1" ORDER BY `time` DESC LIMIT 1'));
	
				mysql_query("INSERT INTO `chat` (`dn`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`,`typeTime`,`new`) VALUES ('".$u->info['dnow']."','".$u->info['city']."','".$u->info['room']."','','','".$var['text']."','".time()."','6','0','1','1')");								
				$errors .= '<img width="40" height="25" src="http://img.likebk.com/i/items/event_timer_trap.gif"> '.$u->info['login'].' угодил'.$var['sex'].' в ловушку... Время перехода +3 секунды (Длительность: +'.$var['time'].' мин.)';
			}
		}elseif( $mitm['type'] == 5 ) {
			//Пандора
			if( $mitm['use'] == 0 ) {
				$itms .= '<a title="Открыть" href="main.php?useobj='.$mitm['id'].'"><img src="http://img.likebk.com/i/items/panbox_on.gif" width="60" height="60"></a>';
			}else{
				$itms .= '<img title="Сундук был открыт" src="http://img.likebk.com/i/items/panbox_off.gif" width="60" height="60">';
			}
		}
		unset($var);
	}
	...
	while( $pl = mysql_fetch_array($sp) ) {
		$itms .= ' <a href="main.php?takeitm='.$pl['id'].'"><img src="http://img.likebk.com/i/items/'.$pl['img'].'" title="Поднять &quot;'.$pl['name'].'&quot;"></a>';
	}
	...
	if( $itms != '' ) {
		...
		mysql_query('UPDATE `users` SET `room` = "369" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		mysql_query('UPDATE `stats` SET `dnow` = "0",`x`="0",`y`="0" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		$r = '<img src=http://img.likebk.com/i/items/paper100.gif width=40 height=25 /> Вы получили награду &quot;Чек на предъявителя (50кр.)&quot;';
		...
		$r = '<img src=http://img.likebk.com/i/items/lmap.gif width=40 height=25 /> Вы прошли лабиринт &quot;Подземелья Драконов&quot; и сохранили предметы из лабиринта!';
		mysql_query("INSERT INTO `chat` (`typeTime`,`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES ('1','1','".$u->info['city']."','".$u->info['room']."','','".$u->info['login']."','".$r."','".time()."','6','0')");
		//Выбрасываем предметы которые из подземелья		
		die('<script>location.href="main.php";</script>');
	}
	...
	if( isset($varos['trap1']['vals']) && $varos['trap1']['vals'] > 0 ) {
		$effed .= '<div><img width=40 height=25 src=http://img.likebk.com/i/items/event_timer_trap.gif> - Время перехода +3 секунды (Осталось: '.$u->timeOut($varos['trap1']['time']-time()).')</div>';
	}
	unset($varos);
?>
<style>
.ddp0 { 
	...
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/o.gif");
}
.ddp1 { 
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/m.gif");
}
.ddpStart {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/os.gif");
}
.ddpExit {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/of.gif");
}
.ddp1s {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/s.gif");
}
.ddp1m {
	display:inline-block;
	width:15px;
...
.ddp1h {
	...
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/h.gif");
}
.ddp1l {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/b.gif");
}
.ddp1p {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/p.gif");
}
.ddp1me {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/u.gif");
}
.ddp1ee1 {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/e1.gif");
}
.ddp1ee2 {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/e2.gif");
}
.ddp1ee3 {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/e3.gif");
}
.ddp1ee4 {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/e4.gif");
}
.ddp1ee5 {
	display:inline-block;
	width:15px;
	height:15px;
	background-image:url("http://img.likebk.com/drgn/bg/e5.gif");
}
</style>
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#E2E0E0">
  <tbody>
    ...
    <tr>
      ...
      <td height="409" width="300" valign="top" align="center"><table width="100%" height="396" border="0" cellpadding="0" cellspacing="0" style="background-position: top right; background-repeat: no-repeat; width: 300px; height: 410px; background: url('http://img.likebk.com/drgn/navbg_big.gif'); ">
        <tbody>
          <tr>
            <td height="34"><table align="center" height="25" border="0" style="background:url(http://img.likebk.com/drgn/bg/ramka_s2.gif); background-repeat:no-repeat; background-position:left;">
              <tbody>
                <tr valign="middle">
                  <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
            ...
            <td height="102" valign="top" align="center"><table width="100%" height="102" border="0" cellpadding="0" cellspacing="0">
              <tbody>
                <tr>
                  ...
                  <td width="103" style="background:url(http://img.likebk.com/drgn/in_nav_bg.gif); width:103px; height:102px; background-repeat: no-repeat;"><table width="103" height="102" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                      <tr>
                        <td width="26" height="26"></td>
                        <td width="12"></td>
                        <td width="26" height="26"><a href="http://likebk.com/main.php?goto=2"><img src="http://img.likebk.com/drgn/arr1.gif" border="0" title="Вверх" alt="Вверх" /></a></td>
                        <td width="13"></td>
                        <td width="26"></td>
                      </tr>
                      <tr>
                      ...
                      <tr>
                        <td width="26" height="26"><a href="http://likebk.com/main.php?goto=1"><img src="http://img.likebk.com/drgn/arr4.gif" border="0" title="Влево" alt="Влево" /></a></td>
                        <td></td>
                        <td width="26" height="26"	><a href="http://likebk.com/main.php?refresh"><img src="http://img.likebk.com/drgn/refresh.gif" border="0" title="Обновить" alt="Обновить" /></a></td>
                        <td></td>
                        <td width="26" height="26"><a href="http://likebk.com/main.php?goto=3"><img src="http://img.likebk.com/drgn/arr2.gif" border="0" title="Вправо" alt="Вправо" /></a></td>
                      </tr>
                      <tr>
                        <td></td>
                        <td></td>
                      ...
                      <tr>
                        <td></td>
                        <td></td>
                        <td width="26" height="26"><a href="http://likebk.com/main.php?goto=4"><img src="http://img.likebk.com/drgn/arr3.gif" border="0" title="Вниз" alt="Вниз" /></a></td>
                        <td></td>
                        <td></td>
                      </tr>
                    </tbody>
