...
$t = mysql_fetch_array(mysql_query('SELECT * FROM `bs_turnirs` WHERE `level` = "'.((int)($r+6)).'" LIMIT 1'));
if(isset($t['id']) ) {
  ...
  if(isset($_POST['coin']) && $t['time_start'] > time() ) {
	  if($u->info['level'] > 8) {
		...
		if((round((int)$tz['money'], 2) + $_POST['coin']) > 100 || $tz_all[0] > 100) {
		  $error = 'Вам нельзя делать ставку выше 100 кр.';
		  ...
		  if(isset($tz['id'])) {
			...
			$error = 'Участие в турнире доступно с 9-го уровня!';
		}
   }elseif(isset($_POST['coin'])){
	 $error = 'Вы попоздали на турнир...';  
   }
}
...
<?php /*if($u->info['admin'] != 1 && $u->info['id'] != "16564"){?>
<center><h4><font style="padding-right: 180px;" color=red>Раздел в разработке!!</font></h4></center><br>
<?php exit();}*/ ?>
<center><h4><font color=green>Поздравляем! Вы успешно допускаетесь к турниру!</font></h4></center><br>
<P align=right><INPUT TYPE="button" onClick="location = 'main.php?rnd=<?=$code;?>';" value="Обновить"> &nbsp; </P>
...
if($t['status'] == 0) {
?>
Начало турнира: <span class=date><?=date('d.m.Y H:i',$t['time_start']);?></span><BR>
...
?>
<? if(!isset($tz['id'])) { ?>
Сколько ставите кредитов? (минимальная ставка <b><?=$t['min_money'];?>.00 кр.</B> у вас в наличии <b><? echo floor($u->info['money']); ?> кр.</b>)<BR><input type="text" name="coin" value="<?=$t['min_money'];?>" size="8"> <input
type="submit" value="Подать заявку" name="docoin"><BR>
Чем выше ваша ставка, тем больше шансов принять участие в турнире.<BR>
<a href="http://likebk.com/i/old.png" target="_blank">Открыть карту Башни Смерти</a>
<? }else{ ?>
...
<input type="text" name="coin" value="<?=$t['min_money'];?>" size="8" id="coin" />
...
$notowerlog = false;
$log = mysql_fetch_array(mysql_query('SELECT `id`,`id_bs`,`m` FROM `bs_logs` WHERE `id_bs` = "'.mysql_real_escape_string($t['bsid']).'" ORDER BY `id` ASC LIMIT 1'));
if(!isset($log['id'])) {
	$notowerlog = true;
	$r = '<div>Скорее всего Архивариус снова потерял пергамент с хрониками турниров ...</div>';
} else {
	$sp = mysql_query('SELECT * FROM `bs_logs` WHERE `id_bs` = "'.$log['id_bs'].'" ORDER BY `id` ASC');
	while( $pl = mysql_fetch_array($sp) ) {
		$datesb = '';
		if( $pl['type'] == 2 ) {
			$datesb = '2';
	...
	$liveusers = '';
	$sp = mysql_query('SELECT `id`,`login`,`clan`,`align`,`level` FROM `users` WHERE `room` = 363 AND `inTurnir` = "'.$t['id'].'"');
	$c_user = 0;
	while( $pl = mysql_fetch_array($sp) ) {
		...
		if( isset($pl['id']) ) {
			$alc = '';
			if( $pl['align'] > 0 ) {
				$alc .= '<img width=12 height=15 src=http://img.likebk.com/i/align/align'.$pl['align'].'.gif >';
			}
			if( $pl['clan'] > 0 ) {
				$alc .= '<img width=24 height=15 src=http://img.likebk.com/i/clan/'.$pl['clan'].'.gif >';
			}
			$liveusers .= ', <b>'.$alc.$pl['login'].'</b> ['.$pl['level'].']<a href=/inf.php?'.$pl['id'].' target=_blank><img src=http://img.likebk.com/i/inf_capitalcity.gif width=12 height=11 ></a>';
		}
		$c_user++;
	}
	$r .= '<br><br>Всего живых участников на данный момент: <b>'.$c_user.'</b> ('.ltrim($liveusers,', ').')';
	unset($liveusers,$alc);
}
if( $notowerlog == false ) {?>
...
$i = 1;
while ($pl = mysql_fetch_array($sp)) {
	$wuser = '<b>'.$pl['wlogin'].'</b> ['.$pl['wlevel'].']<a href=/inf.php?'.$pl['wuid'].' target=_blank ><img width=12 height=11 src=http://img.likebk.com/i/inf_capitalcity.gif ></a>';
	if( $pl['wclan'] > 0 ) {
		$wuser = '<img width=24 height=15 src=http://img.likebk.com/i/clan/'.$pl['wclan'].'.gif>'.$wuser;
	}
	if( $pl['walign'] > 0 ) {
		$wuser = '<img width=12 height=15 src=http://img.likebk.com/i/align/align'.$pl['walign'].'.gif>'.$wuser;
	}
