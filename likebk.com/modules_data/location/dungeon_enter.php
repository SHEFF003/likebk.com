		...
		if($ins){
			...
			while( $pu = mysql_fetch_array($su) ) {
				...
				while($pl2 = mysql_fetch_array($sp)){
					...
					/*if( $u->info['id'] == 164492395 ) {
						die('...');
						$pl = mysql_fetch_array(mysql_query('SELECT * FROM `quests` WHERE `id` = "'.(str_replace('start_quest','',$pl2['vars'])).'" AND `name` LIKE "%Повели%" AND `line` = "'.$dungeon['id'].'" LIMIT 1')); 
					}*/
					if( isset($pl['id']) ) {
			...
			if( $upd1 ){
				...
				if( floor(date('m')) == 2 && floor(date('d')) >= 1 && false == true ) {
					if( floor(date('m')) == 2 && floor(date('d')) >= 17 ) {
	...
	if(isset($_GET['cave_sale'])){
		...
		if($_GET['cave_sale'] == 1){
			...
			if(($u->rep['nagrada']) >= $sumcav){
				...
				if($re){
					$u->rep['nagrada'] -= $sumcav;
					$r = mysql_query('UPDATE `rep` SET `nagrada` = "'.$u->rep['nagrada'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
...
<?php if(isset($_GET['cave_sale'])){
	...
	switch($_GET['obmen']) {
		...
		break;
			...
			$rsot1 = array(10000,20000,30000,40000);
	...
	switch($_GET['buy1']) {
		case 1:
			...
			$limit = 30;
			if($u->rep['add_stats'] == $limit) {
				echo '<font color=red><b>Вы достигли лимита!</font></b>';
			}elseif($u->rep['nagrada'] < $price) {
				echo '<font color=red><b>У вас не хватает: '.($price - $u->rep['nagrada']).' награды</font></b>';
			}else{
				$u->info['ability'] += 1;
				$u->rep['add_stats'] += 1;
				$u->rep['nagrada'] -= $price;
				mysql_query('UPDATE `rep` SET `add_stats` = "'.$u->rep['add_stats'].'", `nagrada` = "'.$u->rep['nagrada'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		...
		case 2:
			...
			if($u->rep['add_skills'] == $limit) {
				...
				$u->rep['add_skills'] += 1;
				$u->rep['nagrada'] -= $price;
				mysql_query('UPDATE `rep` SET `add_skills` = "'.$u->rep['add_skills'].'", `nagrada` = "'.$u->rep['nagrada'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		...
		case 3: 
			...
			if($u->rep['nagrada'] < $price) {
				echo '<font color=red><b>У вас не хватает: '.($price - $u->rep['nagrada']).' награды</font></b>';
			}else{
				$u->info['money'] += $add_money;
				$u->rep['add_money'] += $add_money;
				$u->rep['nagrada'] -= $price;
				mysql_query('UPDATE `users` SET `money` = '.$u->info['money'].' WHERE `id` = '.$u->info['id'].' LIMIT 1');
				mysql_query('UPDATE `rep` SET `add_money` = '.$u->rep['add_money'].', `nagrada` = '.$u->rep['nagrada'].' WHERE `id` = '.$u->rep['id'].' LIMIT 1');
				echo '<font color=red><b>Вы успешно обменяли 100 награды на 10 кредитов!</font></b>';
			}
		break;
		...
		case 4:
			...
			if($u->rep['add_skills2'] == $limit) {
				...
				$u->rep['add_skills2'] += 1;
				$u->rep['nagrada'] -= $price;
				mysql_query('UPDATE `rep` SET `add_skills2` = "'.$u->rep['add_skills2'].'", `nagrada` = "'.$u->rep['nagrada'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		...
		case 5:
			...
			if($u->rep['add_skills2'] == $limit) {
				...
				$u->rep['add_slot'] += 1;
				$u->rep['nagrada'] -= $price;
				mysql_query('UPDATE `rep` SET `add_slot` = "'.$u->rep['add_slot'].'", `nagrada` = '.$u->rep['nagrada'].' WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
	...
	/*if($u->info['admin'] == 0 && isset( $_GET['buy1'] ) || isset($_GET['addslot1']) ) {
		$rt = 1;
		if( $_GET['buy1'] == 1 ) {
			//покупаем статы
			$price = 2000+($u->rep['add_stats']*100);
	...
	//echo ( isset( $rt ) && $rt == 1 ? ($u->rep['allrep']-$u->rep['allnurep'])-$cur_price['price'] : ($u->rep['allrep']-$u->rep['allnurep']) );
	?>
	<fieldset>
		
        <legend style="font-weight:bold; color:#8F0000;">Награда: <b> <?=$u->rep['nagrada']?> ед.</b></legend>
        <table>
			<tr>
	...
	<fieldset style='margin-top:15px;'>
		...
		<fieldset>
	        ...
	        <table>
				<tr>
					...
					<td style='padding-left: 10px'><input type='button' value='Купить'  onclick="if (confirm('Вы уверены, что хотите купить вещь за 150 ед.')) {location='main.php?rz=1&cave_sale=1'}"/></td>
				</tr>
				<tr>
					<td>Вещи 7-8 ур.</td>
					<td style='padding-left: 10px'>за <span class="prc_color">300ед.</span></td>
					<td style='padding-left: 10px'><input type='button' value='Купить' onclick="if (confirm('Вы уверены, что хотите купить вещь за 300 ед.')) {location='main.php?rz=1&cave_sale=2'}"/></td>
				</tr>
				<tr>
					<td>Вещи 9-10 ур.</td>
					<td style='padding-left: 10px'>за <span class="prc_color">400ед.</span></td>
					<td style='padding-left: 10px'><input type='button' value='Купить' onclick="if (confirm('Вы уверены, что хотите купить вещь за 400 ед.')) {location='main.php?rz=1&cave_sale=3'}"/></td>
				</tr>
	        </table>
		</fieldset>
        <fieldset>
        ...
        <fieldset>
	        ...
	        <input type='button' value='Купить слот за <?=$rsot?> ед.' onClick="location='main.php?rz=1&buy1=5'"/>
		</fieldset>
        <? }?>
		 <fieldset>
