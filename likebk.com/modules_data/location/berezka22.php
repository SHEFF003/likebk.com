...
/*			if( isset($po['art']) ) {
	...
	<td bgcolor="#D3D3D3" nowrap><a href="#" id="greyText" class="menutop" onclick="location='main.php?loc=1.180.0.13&rnd=<? echo $code; ?>';" title="<? thisInfRm('1.180.0.13',1); ?>">Магазин "Березка"</a></td>
	</tr>
	...
	$mas = array(
			...
			while( $i < count($mas) ) {
				if( $_GET['otdel'] == $i ) {
					echo '<div style="background-color: #C7C7C7"><a href="?otdel='.$i.'">'.$mas[$i].'</a></div>';
				}else{
					echo '<div style="background-color: #e2e0e0"><a href="?otdel='.$i.'">'.$mas[$i].'</a></div>';
				}
				$i++;
			}

  /*echo '<div style="background-color: #C7C7C7"><a href="?otdel=1">Усиление: Серьги</a></div>
