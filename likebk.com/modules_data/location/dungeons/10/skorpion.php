...
if(isset($file) && $file[0] == 'dungeons/10/skorpion.php') {
	$test = mysql_fetch_array(mysql_query('SELECT COUNT(*) FROM `dungeon_actions` WHERE `vars` = "obj_act'.$obj['id'].'" AND `dn` = "'.$u->info['dnow'].'" AND `uid` = "'.$u->info['id'].'" LIMIT 1'));
	if($test[0] > 0) {
		$btl_id = 0;
			$expB = 0;
			...
			if( $btl_id > 0 ) { //Добавляем ботов 
				$botw = mysql_query('SELECT * FROM `dungeon_bots` WHERE `dn` = '.$u->info['dnow'].' AND `id_bot` = 711 OR `id_bot` = 712 AND `delete` = 0');
				...
				while($pl = mysql_fetch_array($botw)) {
					...
					if($pl['dn'] == $u->info['dnow']) {	
						...
						$values = '"9","'.$test['login'].'",'.$test['level'].',"bot_gribnica","'.$u->info['city'].'","'.$u->info['city'].'","'.$test['login'].'",'.$test['sex'].',"","","'.time().'","'.$test['obraz'].'",'.$pl['id_bot'].',"0"';
					...
					mysql_query('INSERT INTO `dungeon_actions` (`dn`,`time`,`x`,`y`,`uid`,`vars`,`vals`) VALUES ("'.$u->info['dnow'].'","'.time().'","'.$u->info['x'].'","'.$u->info['y'].'","'.$u->info['id'].'","skorpion_atack","")');
			}
