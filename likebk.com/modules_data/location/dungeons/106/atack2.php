...
if(isset($file) && $file[0] == 'dungeons/106/atack2.php') {
	$test = mysql_fetch_array(mysql_query('SELECT COUNT(*) FROM `dungeon_actions` WHERE `vars` = "act_niz2" AND `dn` = "'.$u->info['dnow'].'" AND `uid` = "'.$u->info['id'].'" LIMIT 1'));
	if($test[0] == 0) {
		$btl_id = 0;
			$expB = 0;
			...
			if( $btl_id > 0 ) { //ƒобавл€ем ботов 
				$botw = mysql_query('SELECT * FROM `dungeon_bots` WHERE `dn` = '.$u->info['dnow'].' AND `x` = 2 AND `y` = -13 OR (`x` = 2 AND `y` = -11 OR `x` = 1 AND `y` = -12 OR `x` = 3 AND `y` = -12) AND `delete` = 0');
				while($pl = mysql_fetch_array($botw)) {
					...
					mysql_query('INSERT INTO `dungeon_actions` (`dn`,`x`,`y`,`time`,`uid`,`vars`,`vals`) VALUES ("'.$u->info['dnow'].'","'.$u->info['x'].'","'.$u->info['y'].'","'.time().'","'.$u->info['id'].'","act_niz2","2")');
			}
