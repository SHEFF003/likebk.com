...
if($u->room['file']=='shop_')
{
	...
	if(isset($_POST['itemgift']) && $u->info['id'] != 35447902)
	{
		...
		if( $to['twink'] > 0 || $u->info['twink'] > 0 ) {
			$re = '<div align="left">Твинкам нельзя дарить подарки...</div>';
		}elseif(isset($to['id']))
		{
			...
			}elseif($u->info['level']<4)
			{
				...
				if( $_POST['itemgift'] > 1000000000000 ) {
					$itm_l = mysql_fetch_array(mysql_query('SELECT * FROM `users_gifts` WHERE `x` > 0 AND `uid` = "'.$u->info['id'].'" AND `id` = "'.mysql_real_escape_string((int)$_POST['itemgift']-1000000000000).'" LIMIT 1'));
					if( isset($itm_l['id']) && $itm_l['money'] > $u->info['money'] ) {
						...
						if( $itm > 0 ) {
							...
							$spamt1 = $filter->spamFiltr(str_replace('точка','.',$itm['gtxt1']));
							$spamt2 = $filter->spamFiltr(str_replace('точка','.',$itm['gtxt2']));
							
							if( $spamt1 != '0' ) {
								$itm['gtxt1'] = '<b title1="'.$spamt1.' - '.str_replace('"','&quot;',$itm['gtxt1']).'"><i>СПАМ</i></b>';
							}
							if( $spamt2 != '0' ) {
								$itm['gtxt2'] = '<b title1="'.$spamt2.' - '.str_replace('"','&quot;',$itm['gtxt2']).'"><i>СПАМ</i></b>';
							}
							
							if(isset($itm['id'])) {
								
								if( $to['id'] == 33 || $to['id'] == 12345 ) {
									mysql_query('UPDATE `happy_quest` SET `q8` = `q8` + 1 WHERE `uid` = "'.$u->info['id'].'" LIMIT 1');
								}
								
								mysql_query('UPDATE `users_gifts` SET `x` = `x` - 1 WHERE `id` = "'.$itm_l['id'].'" LIMIT 1');
								...
								if( $_POST['from'] == 1 ) {
									$fromg = '</a><i>Анонима</i><a>';
								}elseif( $_POST['from'] == 2 && $u->info['clan'] > 0 ) {
									$fromc = mysql_fetch_array(mysql_query('SELECT * FROM `clan` WHERE `id` = "'.$u->info['clan'].'" LIMIT 1'));
									if(isset($fromc['id'])) {
										$fromg = '</a>клана <b>'.$fromc['name'].'</b><a>';
									}
								}
								
								$upd = mysql_query('UPDATE `items_users` SET `data` = "'.$itm['data'].'",`gtxt1` = "'.mysql_real_escape_string($itm['gtxt1']).'",`gtxt2` = "'.mysql_real_escape_string($itm['gtxt2']).'", `uid` = "'.$to['id'].'", `gift` = "'.$fromg.'",`time_create` = "'.time().'" WHERE `id` = "'.$itm['id'].'" LIMIT 1');
								...
								if($upd)
								{
									...
									mysql_query('UPDATE `achiev` SET `a7` = `a7` + 1 WHERE `uid` = "'.$u->info['id'].'" LIMIT 1');
									mysql_query('UPDATE `achiev` SET `a8` = `a8` + 1 WHERE `uid` = "'.$to['id'].'" LIMIT 1');
									
									$re = '<div align="left">Подарок был успешно отправлен к &quot;'.$to['login'].'&quot; за '.$itm_l['money'].' кр.</div>';
									if( $_POST['from'] == 1 ) {
										$text = ' Получен подарок <b>'.$itm_l['name'].'</b>. От <i>анонима</i>.';
									}elseif( $_POST['from'] == 2 ) {
										$text = ' Получен подарок <b>'.$itm_l['name'].'</b>. От клана '.$fromc['name'].'.';
									}else{
					...
					$itm = mysql_fetch_array(mysql_query('SELECT `im`.*,`iu`.* FROM `items_users` AS `iu` LEFT JOIN `items_main` AS `im` ON (`im`.`id` = `iu`.`item_id`) WHERE `iu`.`data` NOT LIKE "%notransfer%" AND (`im`.`type` = "28" OR `im`.`type` = "38" OR `im`.`type` = "63" OR `im`.`type` = "64") AND `iu`.`id` = "'.mysql_real_escape_string($_POST['itemgift']).'" AND `iu`.`uid` = "'.$u->info['id'].'" AND `iu`.`gift` = "" AND `iu`.`delete` = "0" AND `iu`.`inOdet` = "0" AND `iu`.`inShop` = "0" LIMIT 1'));
					
					$itm['gtxt1'] = $_POST['podarok2'];
					$itm['gtxt2'] = $_POST['txt'];
						
					if( $u->info['exp'] < 300000 ) {
						$_POST['podarok2'] = '<i>*комментарии к подаркам с 300 000 опыта*</i>';
						$_POST['txt'] = $_POST['podarok2'];
						$itm['gtxt1'] = $_POST['podarok2'];
						$itm['gtxt2'] = $_POST['podarok2'];
					}
					...
					if( $spamt1 != '0' ) {
						$itm['gtxt1'] = '<font color=red ><b title1=\\\''.$spamt1.' - '.str_replace('"','&quot;',str_replace("'",'&quot;',$itm['gtxt1'])).'\\\'><i>СПАМ</i></b></font>';
					}
					if( $spamt2 != '0' ) {
						$itm['gtxt2'] = '<b title1=\''.$spamt2.' - '.str_replace('"','&quot;',$itm['gtxt2']).'\'><i>СПАМ</i></b>';
					}
	...
	if(isset($_GET['buy'])){
		...
		if( ($itm['gift'] != '' && $itm['gift'] != '0') && (  $itm['type'] == 37 || $itm['type'] ==  38 || $itm['type'] == 39 || $itm['type'] == 63 ) ) {
			...
			$shpCena01 = $shpCena;
			$shpCena = $u->shopSaleM2( $shpCena , $itm );
			$shpCena = $shpCena/100*(100-$prc1);
			$shpCena01 = $shpCena01/100*(100-$prc1);
			...
			if($col > 0){
				$shpCena = $shpCena*$col;
				$shpCena01 = $shpCena01*$col;
			}	
						
			$premium = mysql_fetch_array(mysql_query('SELECT * FROM `premium` WHERE `uid` = "'.$u->info['id'].'" AND `time_delete` > "'.time().'"'));
			if(isset($premium['id'])){
				if($itm['level'] < 9){
					$shpCena = $u->round2($shpCena01*1);
				}else{
					$shpCena = $u->round2($shpCena01*($premium['sale_prc']/100)); // Цена предметов
				}
			}else{
				if($itm['level'] < 9){
					$shpCena = $u->round2($shpCena*1);
				}else{
					$shpCena = $u->round2($shpCena); // Цена предметов
				}
			}
...
<input name="itemgift" id="itemgift" type="hidden" value="0" />
    ...
    <?php
		...
		{
			$htmlg2 = '';			
			$sp = mysql_query('SELECT * FROM `users_gifts` WHERE `x` > 0 AND `uid` = "'.$u->info['id'].'"');
			while( $pl = mysql_fetch_array($sp) ) {
				...
				//
					'<a href="http://lib.likebk.com/items_info.php?id=0">'.$pl['name'].'</a> &nbsp; (Масса: 1)<br>Долговечность: 0/1<br>Осталось подарков: '.$pl['x'].'<br>'.
					'<small><b>Описание:</b><br>Это именной подарок, его можете подарить только Вы.'.
					// <br>Сделано в Capital city</small>'.
				//
				'</td></tr></table>';
	...
	<tr>
	<!-- <td bgcolor="#D3D3D3"><img src="http://img.likebk.com/i/move/links.gif" width="9" height="7" /></td> -->
	<!-- <td bgcolor="#D3D3D3" nowrap><?php //if($u->info['admin']>0){?><a href="#" id="greyText" class="menutop" onclick="location='main.php?loc=1.180.0.368&rnd=<? //echo $code; ?>';" title="<? //thisInfRm('1.180.0.368',1); ?>">Подпольная лавка</a><?php //}?></td> -->
	</tr>
	</table>
	</td>
	</tr>
	...
	<?php
		...
		if($u->info['id'] == 155){
			echo '<br><div style=""><a href="?otdel=111">Комплект Истиной Земли</a></div><br>';	
			echo '<div style=""><a href="?otdel=112">Комплект Истиного Воздуха</a></div><br>';	
			echo '<div style=""><a href="?otdel=113">Комплект Истиного Огня</a></div><br>';	
			echo '<div style=""><a href="?otdel=114">Комплект Истиной Воды</a></div><br>';	
		}
		...
		echo '<div style="background-color: #C7C7C7"><a href="?otdel=1">Оружие: кастеты,ножи</a></div>
			...
			<div style="background-color: #e2e0e0"><a href="?otdel=32&amp;gifts=1">Сделать подарки</a></div>';
		echo '</div>';
	?>
	</div>
	</td>
