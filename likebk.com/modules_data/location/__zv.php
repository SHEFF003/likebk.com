			...
			if($r>=1 && $r<=5)
			{
				...
				while($pl = mysql_fetch_array($cl))
				{
					if($pl['razdel']==5)
					{
						...
						$zvb .= ''.$rad.'<font class="date">'.date('H:i',$pl['time']).'</font> ('.$tm.') ('.$pl['min_lvl_1'].'-'.$pl['max_lvl_1'].') <IMG SRC="http://img.likebk.com/i/fighttype'.$pl['type'].'.gif" WIDTH="20" HEIGHT="20" title="Хаотичный бой">'.$n1tv.' <font class="dsc"><i>Бой начнется через <B>'.$tmStart.'</B> мин., таймаут '.($pl['timeout']/60).' мин. </font></i><BR>';						
					...
					}elseif($pl['razdel']==4)
					{
						...
						$zvb .= ''.$rad.'<font class="date">'.date('H:i',$pl['time']).'</font> <B>'.$pl['tm1max'].' (</b>'.$pl['min_lvl_1'].'-'.$pl['max_lvl_1'].'<b>) на '.$pl['tm2max'].' (</b>'.$pl['min_lvl_2'].'-'.$pl['max_lvl_2'].'<b>)</B> ('.$tm1.') <font class="dsc"><i><span style=\'color:red; font-weight:bold;\'>против</span></font></i> ('.$tm2.') <IMG SRC="http://img.likebk.com/i/fighttype'.$pl['type'].'.gif" WIDTH="20" HEIGHT="20" title="Групповой бой"> <font class="dsc"><i>Бой начнется через <B>'.$tmStart.'</B> мин., таймаут '.($pl['timeout']/60).' мин. </font></i><BR>';
					}elseif($pl['razdel']>=1 && $pl['razdel']<=3)
					{
						...
						if(isset($uz['id']))
						{
							...
							if(!isset($uze['id']) || $u->info['zv']==$pl['id'])
							{
								...
								$zvb .= '<input name="btl_go" '.$d1.' type="radio" value="'.$pl['id'].'" /> <font class="date">'.date('H:i',$pl['time']).'</font> '.$u->microLogin($uz['id'],1).' '.$enm.'  тип боя: <img src="http://img.likebk.com/i/fighttype'.($pl['type']).'.gif"> (таймаут '.round($pl['timeout']/60).' мин.'.$dp1.')<br>';
								...
								if($upd)
								{
									...
									if($uz1['clan']!=0)
									{
										...
										if(isset($pc['id']))
										{
											...
											$ca = '<img title="'.$pc['name'].'" src="http://img.likebk.com/i/clan/'.$pc['name_mini'].'.gif">';
										}
									}
									if($uz1['align']!=0)
									{
									...
									$this->error = 'Ожидаем подтверждения боя от '.$ca.' '.$uz1['login'].' ['.$uz1['level'].']<a href="inf.php?'.$uz1['id'].'" target="_blank"><img src="http://img.likebk.com/i/inf_capitalcity.gif" title="Инф. о '.$uz1['login'].'"></a>';
									$sa = '';
									if($u->info['sex']==2)
									{
										$sa = 'а';
