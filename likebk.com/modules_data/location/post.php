	...
	if(isset($_POST['itm_post']) && (int)$_POST['itm_post'] > 0) {
		...
		if( $u->info['align'] == 2 ) {
			...
			if($see1 == 1) {
				...
				if($u->info['money'] >= $mny) {
					if($x > 1) {
						mysql_query('UPDATE `items_users` SET `uid` = "-51'.$pu['id'].'",`lastUPD` = "'.(time()+$tmgo*1).'" WHERE `uid`="'.$u->info['id'].'" AND `item_id`="'.$itm['item_id'].'" AND `inGroup` = "'.mysql_real_escape_string($itm['inGroup']).'" LIMIT '.$x);
						$itm['name'] .= ' (x'.$x.')';
					} else {
						mysql_query('UPDATE `items_users` SET `uid` = "-51'.$pu['id'].'",`lastUPD` = "'.(time()+$tmgo*1).'" WHERE `id` = "'.mysql_real_escape_string($itm['id']).'" LIMIT 1');
					}
					$u->info['money'] -= $mny;
					mysql_query('UPDATE `users` SET `money` = "'.$u->info['money'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
					$u->error = 'Вы успешно передали &quot;'.$itm['name'].'&quot; к персонажу &quot;'.$pu['login'].'&quot; за '.$mny.' кр. ';
					//логи
					$txt = 'Предмет от <b>'.$u->info['login'].'</b>: &quot;'.$itm['name'].'&quot;. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
					...
					$txt = 'Отправление предмета к <b>'.$pu['login'].'</b>: &quot;'.$itm['name'].'&quot;. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
					...
					'1','".$pu['city']."','0','','".$pu['login']."','<font color=red>Внимание!</font> Получена новая почта от &quot;".$u->info['login']."&quot;','-".(time()+$tmgo*1)."','5','0')");
				}else{
			...
			if($itm['item_id'] == 1220) {
				...
				if(isset($post['id'])) {
					...
					if($item) {
						...
						$u->error = 'Не удалось забрать предмет &quot;'.$itm['name'].' (x'.$x.')&quot;';	
					}
				}else{
					$x = $u->itemsX(mysql_real_escape_string($itm['id']));
					if( $x > 0 ) {
						mysql_query('INSERT INTO `post` (`uid`,`text`,`sender_id`,`item_id`) VALUES ("'.$u->info['id'].'","Доставка предмета &quot;'.$itm['name'].' (x'.$x.')&quot;","'.$u->info['id'].'","'.$itm['id'].'")');
						$u->error = 'Не удалось найти предмет &quot;<span title="'.$itm['id'].' (x'.$x.')">'.$itm['name'].'</span>&quot;, попробуйте через 1 минуту!';
					}else{
						$u->error = 'Не удалось найти предмет &quot;<span title="'.$itm['id'].'">'.$itm['name'].'</span>&quot;';
					}
				}
			}
		}else{
...
if(isset($pu['id'])) {
	if($r == 1) {
		//Передача предметов
		$itmAll = ''; $itmAllSee = '';
		$itmAll = $u->genInv(67,'`iu`.`uid` = "'.$u->info['id'].'" AND `iu`.`delete` = "0" AND `iu`.`inOdet` = "0" AND `iu`.`inShop` = "0" AND `im`.`inRazdel`="'.mysql_real_escape_string($otdel).'" ORDER BY `lastUPD` DESC'); 
		if($itmAll[0]==0){
		...
		if(isset($_POST['send1'])) {
			...
			if($m > 0) {
				if($u->info['money'] >= round($m+$cm,2)) {
					...
					mysql_query("INSERT INTO `items_users`(`item_id`,`1price`,`uid`,`delete`,`lastUPD`)VALUES('1220','".mysql_real_escape_string($m)."','-51".$pu['id']."','0','".(time()+$tmgo*1)."');");
					$txt = 'Деньги от <b>'.$u->info['login'].'</b>: '.round($m,2).' кр. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
					...
					$txt = 'Деньги к <b>'.$pu['login'].'</b>: '.round($m,2).' кр. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
			...
			if($u->info['money'] >= 0.1) {
				...
				if( !isset($ttest['id']) || ( $ttest['time']-$tmgo*1-time()+10 ) <= 0 ) {
					...
					mysql_query('UPDATE `users` SET `money` = "'.$u->info['money'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
					mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES (
					'1','".$pu['city']."','0','','".$pu['login']."','<font color=red>Внимание!</font> ".date("d.m.y H:i")." Телеграмма от <b>".$u->info['login']."</b>: \'".mysql_real_escape_string($_POST['snd_telegraf'])."\' .','-".(time()+$tmgo*1)."','5','0')");
				}else{
					$e2 = 'Вы не можете отправлять сообщения так часто. Осталось '.( $ttest['time']-$tmgo*1-time()+10 ).' сек.';
				}
			}else{
			...
			if($u->info['money'] >= 1) {
				...
				//логи
				$txt = 'Предмет от <b>'.$u->info['login'].'</b>: &quot;Письмо&quot;. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
				mysql_query('INSERT INTO `post` (`text`,`uid`,`time`,`sender_id`,`item_id`,`money`) VALUES ("'.mysql_real_escape_string($txt).'",
				"'.$pu['id'].'","'.time().'","'.$u->info['id'].'","'.$id.'","0")');
				$txt = 'Отправление предмета к <b>'.$pu['login'].'</b>: &quot;Письмо&quot;. Прибытие: '.date('d.m.Y H:i',(time()+$tmgo*1)).'';
				mysql_query('INSERT INTO `post` (`text`,`sender_id`,`time`,`uid`,`item_id`,`money`) VALUES ("'.mysql_real_escape_string($txt).'",
				"-'.$pu['id'].'","'.time().'","'.$u->info['id'].'","'.$id.'","0")');
				//чат
				mysql_query("INSERT INTO `chat` (`new`,`city`,`room`,`login`,`to`,`text`,`time`,`type`,`toChat`) VALUES (
...
?>
      ...
      <div style="margin-left:10px;"><br />
        <p>&nbsp;</p>
        <p> <b>Деньги: </b><b style="color: #339900"><?php echo $u->info['money']?> кр.</b><br>
		...
		$ot = $ot[0];
		//
		//$ot2 = mysql_fetch_array(mysql_query('SELECT COUNT(`id`) FROM `post` WHERE `uid` = "'.$u->info['id'].'" AND `finish` = 0'));
		//$ot2 = $ot2[0];
		//
		if($ot > 0) {
