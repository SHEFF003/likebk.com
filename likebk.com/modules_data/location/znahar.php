...
if($c['znahar']==1){$raspst=99999;$kr=0;}else{$raspst=0;$kr=0;}

//Платно или бесплатно
$price = 0;

if(date('w') == 6 || date('w') == 0 || $c['holiday'] == false) {
	$price = 1;
}
...
function test_ability() {
	...
	if($u->info['twink'] == 0) {
		$arch = mysql_fetch_array(mysql_query('SELECT * FROM `achiev` WHERE `uid` = "'.$u->info['id'].'" LIMIT 1'));
		$r += $arch['add_stats'];
		$r += $u->rep['add_stats'];
	}
...
function test_s5() {
	...
	$sp = mysql_query('SELECT * FROM `levels` WHERE `exp` <= "'.$u->info['exp'].'" ORDER BY `upLevel` ASC');
	while( $pl = mysql_fetch_array($sp) ) {
		$r += $pl['vinosl'];
	}
	return $r;
...
if(isset($_GET['unprist'])) {
	$epr = mysql_fetch_array(mysql_query('SELECT `a`.`id`,`b`.`mname` FROM `eff_users` AS `a` LEFT JOIN `eff_main` AS `b` ON `b`.`id2` = `a`.`id_eff` WHERE (`a`.`id_eff` >= 301 AND `a`.`id_eff` <= 304) AND `a`.`uid` = "'.$u->info['id'].'" AND `a`.`id` = "'.mysql_real_escape_string((int)$_GET['unprist']).'" LIMIT 1'));
	if(isset($epr['id'])) {
		$z_na = zact(4);
		if($z_na[0] == 1) {
			//сброс пристрастия
...
}elseif(isset($_POST['dropmastery']))
{
	...
	if($z_na[0] == 1) {
		...
		$n3 = $u->info['sskills'];
		$n4 = $u->info['nskills'];
		mysql_query('UPDATE `stats` SET `wipe`="0",`stats`="'.$st.'",`skills`="'.$n2.'",`sskills`="'.$n3.'",`nskills`="'.$n4.'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
		//$u->info['wipe'] = 2;
		//$u->wipe();
		$st = $u->lookStats($u->info['stats']);	
		$err = '<font color=red>Вы заплатили '.$kr.' кр. Все прошло успешно, но вам необходим отдых...</font>';
...
}elseif(isset($_POST['dropperks']))
{
	...
	if($no==0){
		...
		if($z_na[0] == 1) {
			if( $st['s4'] < test_s5() ) {
				$err = '<font color=red>Нельзя опускать выносливость ниже '.test_s5().' (Вы пытаетесь сделать '.$st['s4'].')</font>';
			}elseif($st['s1']>=3 && $st['s2']>=3 && $st['s3']>=3 && $st['s4']>=3 && $st['s5']>=0 && $st['s6']>=0 && $st['s7']>=0 && $st['s8']>=0 && $st['s9']>=0 && $st['s10']>=0){	
...
<TABLE width=100%>
<TR><TD><DIV class='H3' align=center>Хижина Знахаря</DIV>
<?=$err;?><?
if($u->error != '') {
...
<td bgcolor="#D3D3D3" nowrap><a href="javascript:void(0)" class="menutop" onClick="location='main.php?loc=1.180.0.323&rnd=<? echo $code; ?>';" title="<? thisInfRm('1.180.0.323',1); ?>">Западная окраина</a></td>
</tr>
</table>
</td>
</tr>
...
<TABLE>
<TR bgcolor=#D8D8D8>
<TD><IMG width=20 height=20 src='http://img.likebk.com/i/strsmall.gif'> Характеристики</TD>
<TD><!-- <IMG width=20 height=20 src='http://img.likebk.com/i/cureelixirsmall.gif'> Пристрастия --></TD>
<TD><IMG width=20 height=20 src='http://img.likebk.com/i/weaponsmall.gif'> Прочее</TD>
<TR>
...
<?
$prs = '';
$sp = mysql_query('SELECT `a`.*,`b`.* FROM `eff_users` AS `a` LEFT JOIN `eff_main` AS `b` ON `b`.`id2` = `a`.`id_eff` WHERE (`a`.`id_eff` >= 301 AND `a`.`id_eff` <= 304) AND `a`.`uid` = "'.$u->info['id'].'" AND `a`.`delete` = "0"');
$txtl = 'бесплатно';
if($c['znahar4'] == 0) {
	$txtl = ' за '.$pr[4].' кр.';
}
while($pl = mysql_fetch_array($sp)) {
	$prs .= '<img style="display:inline-block;vertical-align:bottom" title="'.$pl['mname'].''."\r\n".''.$pl['name'].'" height="20" src="http://img.likebk.com/i/eff/'.$pl['img'].'"> <a href="main.php?unprist='.$pl['id'].'&rnd='.$code.'"> <small>вылечить '.$txtl.'</small> </a><br>';
}
...
<?
if( $c['holiday'] == true ) {
	if(isset($_POST['travmdelete'])) {
		mysql_query('DELETE FROM `eff_users` WHERE `uid` = "'.$u->info['id'].'" AND `id_eff` = 4 AND `data` NOT LIKE "%|nelich=%"');
		echo '<div><b><font color=red>Вы были успешно исцелены от всех травм!</font></b></div>';
	}
	?>
    Бесплатное лечение травм (Кроме неизлечимых)<BR>
    <form method=post>
        <INPUT type=submit name='travmdelete' value='Излечить от всех травм' onClick="return confirm('Вы действительно хотите вылечиться от всех травм?')"><HR style="border:0;border-bottom:1px solid grey">
    </form>
    <?
}
?>
...
<form method=post>
	<INPUT type=submit name='dropmastery' value='Сбросить <?echo $c['znahar1']==1? "бесплатно":"(".$pr[1]."кр.)"?>' onClick="return confirm('Вы действительно хотите сбросить умения?')"><HR style="border:0;border-bottom:1px solid grey">
</form>
...
<form method=post>
	<INPUT type=submit name='dropstats' value='Сбросить <?echo $c['znahar3']==1? "бесплатно":"(".$pr[3]."кр.)"?>' onClick="return confirm('Вы действительно хотите сбросить все характеристики до минимального уровня?')">
	</form>
</TABLE>
<BR>
</TABLE>
