...
&bull; Сувениры: 50 шт.<BR>
 &bull; Койка<BR>
<A href="?arenda=base&sd4=<?=$u->info['id'];?>" onClick="return confirm('Вы уверены, что хотите заплатить 1 кр.?')">Арендовать</A>
...
<A href="?arenda=advanced&sd4=<?=$u->info['id'];?>" onClick="return confirm('Вы уверены, что хотите заплатить 3 кр.?')">Арендовать</A>
...
while( $pl = mysql_fetch_array($sp) ) {
	...
	$r .= 'position:relative;display:inline-block;width:120px;height:220px;background-image:url(\'http://img.likebk.com/i/obraz/'.$u->info['sex'].'/'.$pl['obraz'].'\')">';
	$r .= '<div align="center" style="position:absolute;bottom:0px;left:0px;width:114px;background-color:#E2E0E0;margin:3px;"><b>'.$pl['login'].'</b> ['.$pl['level'].']<img width="12" height="11" src="http://img.likebk.com/i/inf_capitalcity.gif"></div>';	
	$r .= '</div>';
	if( $pl['id'] != $p_now['id'] ) {
		$r .= '</a>';
	}
...
if( $user_new_pers == true ) {
	$r .= '<div style="';
	$r .= 'border:5px solid #E2E0E0;';
	$r .= 'position:relative;display:inline-block;width:120px;height:220px;background-image:url(\'http://img.likebk.com/chars/'.$u->info['sex'].'/0.png\')">';
...
<A href="/main.php?pet_id='.$cage1['id'].'&sd4='.$u->info['id'].'&room=3&0.'.rand(0,9999999999999999).'" alt="Оставить"><IMG src="http://img.likebk.com/i/obraz/'.$cage1['sex'].'/'.$cage1['obraz'].'.gif" width=120 height=220>';
}else{
echo'<TD width=150 align=center><nobr><B>свободно</B></nobr><BR>
<IMG src="http://img.likebk.com/i/obraz/0/null.gif" width=120 height=220>';
}
?></A></TD>
<?if($cage2['pet_in_cage']==2) {
echo'<TD width=150 align=center><nobr><B>'.$cage2['name'].'</B> ['.$cage2['level'].']</nobr>
<A href="/main.php?pet_id='.$cage2['id'].'&sd4='.$u->info['id'].'&room=3&0.'.rand(0,9999999999999999).'" alt="Оставить"><IMG src="http://img.likebk.com/i/obraz/'.$cage2['sex'].'/'.$cage2['obraz'].'.gif" width=120 height=220>';
}else{
echo'<TD width=150 align=center><nobr><B>свободно</B></nobr><BR>
<IMG src="http://img.likebk.com/i/obraz/0/null.gif" width=120 height=220>';
}
...
<A href="/main.php?pet_id=-'.$pet['id'].'&sd4='.$u->info['id'].'&room=3&0.'.rand(0,9999999999999999).'" alt="Оставить"><IMG src="http://img.likebk.com/i/obraz/'.$pet['sex'].'/'.$pet['obraz'].'.gif" width=120 height=220>';
}?>
</A>
</TD></TR></TABLE>
<?		;break;
