...
<INPUT type="submit" name="savenotes" value="Сохранить текст"></form>';
		...
		if(!isset($p_now['id'])) {
			if( $u->info['level'] >= 10 && $u->info['stopexp'] == 0 ) {
				$u->info['stopexp'] = 1;
				mysql_query('UPDATE `users` SET `stopexp` = 1 WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
			}
		...
		if(isset($_GET['change_pers'])) {
			...
			$p_count = $p_count[0];
			if( $u->info['level'] < 9 ) {
				$u->error = '<b><font color=red>Персонажи ниже 9-го уровня не могут создавать новых персонажей</font></b>';
			}elseif( $p_count > 1 ) {
				...
				mysql_query('INSERT INTO `users_twink` (
						...
						"300000","22","s1=3|s2=3|s3=3|s4=7|rinv=40|m9=5|m6=10","'.time.'",
