...
if($u->room['file']=='hip_shop')
{
	...
	if(isset($_GET['buy'])){
		if($u->newAct($_GET['sd4'])==true){
			$itms = mysql_fetch_array(mysql_query('SELECT * FROM `items_shop` WHERE `sid` = "'.$sid.'" AND `item_id` = "'.mysql_real_escape_string((int)$_GET['buy']).'" LIMIT 1'));
			if($itms['r'] == 1 && $u->rep['hip'] < 10000 ) {
				$re = '...';
			}elseif($itms['r'] == 2 && $u->rep['hip'] < 25000 ) {
				$re = '...';
			}elseif($itms['r'] == 3 && $u->rep['hip'] < 50000 ) {
				$re = '...';
			}else{
...
$otdels_small_array = array (1=>'<b>Отдел&nbsp;&quot;Первая полка&quot;</b>',
				2=>'<b>Отдел&nbsp;&quot;Вторая полка&quot;</b>',
				3=>'<b>Отдел&nbsp;&quot;Третья полка&quot;</b>',
				4=>'<b>Отдел&nbsp;&quot;Лотерея&quot;</b>',
...
<input name="itemgift" id="itemgift" type="hidden" value="0" />
	...
	<td bgcolor="#D3D3D3" nowrap><a href="#" id="greyText" class="menutop" onclick="location='main.php?loc=1.180.0.418&rnd=<? echo $code; ?>';" title="<? thisInfRm('1.180.0.418',1); ?>">Хижины Искателей Приключений</a></td>
	</tr>
	<tr>
	<!-- <td bgcolor="#D3D3D3"><img src="http://img.likebk.com/i/move/links.gif" width="9" height="7" /></td> -->
	...
	<?php
		...
		echo '	<div style="background-color: #C7C7C7"><a href="?otdel=1">Первая полка</a></div>
				<div style="background-color: #C7C7C7"><a href="?otdel=2">Вторая полка</a></div>
';
//if($u->info['admin'] > 0) {
	echo '<div style="background-color: #C7C7C7"><a href="?otdel=3">Третья полка</a></div>';
//}
		echo '</div>';
	?>
	</div>
