...
$dungeon_timeout = $u->testAction('`uid` = "'.$u->info['id'].'" AND `vars` = "psh'.$dungeon['id'].'" AND `time` > '.(time()-60*60*8).' LIMIT 1',1);
...
if(isset($dungeon_timeout['id'])) // Кто-то передумал и не пойдет в пещеру, так-как уже там был.
{
	$dungeonGo = 0;
	if(isset($_GET['start'])){
		$error = 'До следующего похода осталось еще: '.$u->timeOut(60*60*8-time()+$dungeon_timeout['time']);
	}
} 
...
body
{
	...
	background-image: url(http://img.likebk.com/drgn/bg/1.jpg);
...
if($roomSection==0) { echo $dungeonGroupList; }
	...
	<?
		...
		if($dungeonGo == 1){
			...
			echo 'Поход в пещеры разрешен один раз в 8 часов. Осталось еще: '.$u->timeOut(60*60*8-time()+$dungeon_timeout['time']).'<br><small style="color:grey">Но Вы всегда можете приобрести ключ от прохода у любого &quot;копателя пещер&quot; в Торговом зале ;)</small>';
		}
	}
}
?>
