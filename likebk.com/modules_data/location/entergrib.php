...
$g11 = $u->testAction('`uid` = "'.$u->info['id'].'" AND `vars` = "psh10" AND `time` > '.(time()-60*60*3).' LIMIT 1',1);
...
{	
	...
	if($ig>0)
	{
		...
		VALUES ("'.$zv['city'].'","'.$zv['uid'].'","'.$dun.'","Грибница","'.time().'")');
		if($ins)
		{
			$zid = mysql_insert_id();
			...
			while($pu = mysql_fetch_array($su))
			{
				...
				{
					$pl = mysql_fetch_array(mysql_query('SELECT * FROM `quests` WHERE `id` = "'.(str_replace('start_quest','',$pl2['vars'])).'" AND `line` = "10" LIMIT 1'));
					if( isset($pl['id']) ) {
...
$sp = mysql_query('SELECT * FROM `dungeon_zv` WHERE `city` = "'.$u->info['city'].'" AND `dun` = "10" AND `delete` = "0" AND `time` > "'.(time()-60*60*2).'"');
while($pl = mysql_fetch_array($sp))
{
	...
	while($pu = mysql_fetch_array($su))
	{
		$pus .= '<b>'.$pu['login'].'</b> ['.$pu['level'].']<a href="inf.php?'.$pu['id'].'" target="_blank"><img src="http://img.likebk.com/i/inf_capitalcity.gif" title="Инф. о '.$pu['login'].'"></a>';
		$pus .= ', ';
	}
	$pus = trim($pus,', ');
...
$qsee = '';
$qx = 0;
$hgo = $u->testAction('`uid` = "'.$u->info['id'].'" AND `time` >= '.(time()-60*60*24).' AND `vars` = "psh_qt_suncity" LIMIT 1',1);
if(isset($_GET['add_quest'])) {
	if(isset($hgo['id'])) {
		echo '<font color="red"><b>Нельзя получать задания чаще одного раза в сутки</b></font><br>';
	}else{
		
		$sp = mysql_query('SELECT * FROM `quests` WHERE `line` = 10');
		$dq_add = array();
		while($pl = mysql_fetch_array($sp)) {
			if($u->rep['repsuncity'] == 9999) {
				...
				if($u->rep['repsuncity'] < 10000) {
					if( $pl['kin'] == 0 && $pl['fix'] == 1) { //отбираем квесты для 1-2 этажа (до получения рыцаря 1-го круга)
		...
		$act_q = mysql_fetch_array(mysql_query('SELECT `id` FROM `dialog_act` WHERE `uid` = '.$u->info['id'].' AND `var` = "gusinec1" AND `val` = 1 LIMIT 1'));
		
		if($q->testGood($dq_add)==1 && $dq_add > 0 && isset($act_q['id']))
		{
			$q->startq_dn($dq_add['id']);
			echo '<font color="red"><b>Вы успешно получили новое задание &quot;'.$dq_add['name'].'&quot;.</b></font><br>';
			$hgo['id'] = 1;
			if($u->info['admin'] != 1 || $u->info['login'] != "Lu") {
				$u->addAction(time(),'psh_qt_suncity',$dq_add['id']);
			}
		}else{
			if($u->rep['repsuncity'] == 9999) {
				//квет, рыцарского задания
			...
			}elseif(!isset($act_q['id'])) {
				echo '<font color=red><b>Выполните задание Мудрого Гусеница, затем возвращайтесь обратно за новыми заданиями...</font></b>';
			}else{
...
while($pl = mysql_fetch_array($sp))
{
	...
	$qsee .= '<a href="main.php?rz=1&end_qst_now='.$pq['id'].'"><img src="http://img.likebk.com/i/clear.gif" title="Отказаться от задания"></a> <b>'.$pq['name'].'</b><div style="padding-left:15px;padding-bottom:5px;border-bottom:1px solid grey"><small>'.$pq['info'].'<br>'.$q->info($pq).'</small></div><br>';
	$qx++;
}
...
?>
	  ...
	  if(isset($_GET['buy1'])) {
		  $dungeon = array(1=>'capitalcity',2=>'demonscity',3=>'suncity');
		  if($_GET['buy1']==1) {
			  //покупаем статы
