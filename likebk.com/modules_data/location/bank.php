							...
							if($mn<0.01 || $mn>1000000000)
							{
								...
								if($upd)
								{
									...
									if($ut['id']!=$u->info['id'])
									{
										...
										if($u->info['align']==50)
										{
											$alg = '<img src=http://img.likebk.com/i/align/align50.gif >';
										}
...
?>
var payc = {'USD':'<?=(0+$pl['USD'])?>','EUR':'<?=(0+$pl['EUR'])?>','UAH':'<?=(0+$pl['UAH'])?>','BYR':'<?=(0+$pl['BYR'])?>','RUB':'<?=(0+$pl['cur'])?>'};
var pay_bh = {
	1:'Способ оплаты: <b>Webmoney</b><br>Комиссия платежа: <span id="pay_comis">0</span>%<br><br>'+
	  //''+
	  'К оплате:<div style="margin-left:22px;display:inline-block;"><input style="padding-left:2px;width:77px;" onkeydown="pay_resum(pay_now_id,true);" onkeyup="pay_resum(pay_now_id,true);" id="pay_out" value="0.00"></div>'+
	  ' <select onChange="pay_resum(1);" id="pay_t1">'+
		  '<option selected="selected" value="1">WMR</option>'+
		  '<option value="2">WMZ</option>'+
		  '<option value="3">WMB</option>'+
		  '<option value="4">WMU</option>'+
		  '<option value="5">WME</option>'+
		'</select>'+
	   '<div style="padding-top:5px;padding-bottom:5px;border-top:1px solid #AEAEAE;">'+
	   '<b>Как будете платить?</b><br>'+
	   '<label>'+
       '<input onChange="pay_box(1,0)" type="radio" name="RadioGroup1" value="1" id="RadioGroup1_0">'+
       'На сайте Webmoney</label>'+
       '<br>'+
       '<label>'+
       '<input onChange="pay_box(1,1)" type="radio" name="RadioGroup1" value="2" id="RadioGroup1_1">'+
       'Карты Webmoney (Paymer)</label>'+
       '<br><label>'+
       '<input onChange="pay_box(1,2)" type="radio" name="RadioGroup1" value="3" id="RadioGroup1_2">'+
       'Выставить счет WMID</label><br>'+
	   '<div style="padding-top:5px;margin-top:5px;padding-bottom:5px;border-top:1px solid #AEAEAE;" id="pay_box1"></div></div>',
	   
	 2:'Способ оплаты: <b>Яндекс.Деньги</b><br>Комиссия платежа: <span id="pay_comis">0</span>%<br><br>'+
	   'К оплате:<div style="margin-left:22px;display:inline-block;"><input style="padding-left:2px;width:77px;" onkeydown="pay_resum(pay_now_id,true);" onkeyup="pay_resum(pay_now_id,true);" id="pay_out" value="0.00"> руб.</div>'+
	   '<br>&bull; Нажмите кнопку &quot;Оплатить&quot; и следуйте дальнейшей инструкции.',
	   
	 3:'Способ оплаты: <b>QIWI</b><br>Комиссия платежа: <span id="pay_comis">0</span>%<br><br>'+
	   'К оплате:<div style="margin-left:22px;display:inline-block;"><input style="padding-left:2px;width:77px;" onkeydown="pay_resum(pay_now_id,true);" onkeyup="pay_resum(pay_now_id,true);" id="pay_out" value="0.00"> руб.</div>'+
	   '<br>&bull; Укажите номер телефона: + <input style="padding-left:2px;width:77px;" id="pay_qiwi_mob" value=""> <small>(Например: 79601002030)</small>'+
	   '<br>&bull; Нажмите кнопку &quot;Оплатить&quot; и следуйте дальнейшей инструкции.'
	  
};
function pay_box(id,num) {
	if(id == 1) {
		if(num == 0) {
			$('#pay_box1').html('&bull; Нажмите кнопку &quot;Оплатить&quot; и следуйте дальнейшей инструкции.');
		}else if(num == 2) {
			$('#pay_box1').html('&bull; Укажите WMID: <input type="text" id="pay_wmid" name="pay_wmid" value=""><br>&bull; Нажмите кнопку &quot;Оплатить&quot; и следуйте дальнейшей инструкции.');
		}else if(num == 1) {
			$('#pay_box1').html('&bull; Укажите номер карты: <input type="text" id="pay_wmid" name="pay_wmid" value=""><br>&bull; Нажмите кнопку &quot;Оплатить&quot; и следуйте дальнейшей инструкции.');
		}
	}
}
function roundPlus(x, n) {
  ...
  var m = Math.pow(10,n);
  m = Math.round(x*m)/m;
  return m.toFixed( n+1 ).slice( 0, -1 );
}
...
function trn2(v,com) {
	...
	var pin = out/v;
		pin -= pin/100*com;
	$('#pay_in').val(roundPlus(pin,2));
}

var pay_now_id = 0;
function pay_resum(id,nod) {
	if(id > 0) {
		var com = 0;
		var ekr1 = 0;	
		...
		if(id == 1) {
			...
			if($('#pay_t1').val() == 1) {
				//WMR
				ekr1 = payc['RUB'];
			}else if($('#pay_t1').val() == 2) {
				//WMZ
				ekr1 = payc['USD'];
			}else if($('#pay_t1').val() == 4) {
				//WMU
				com += 7;
				ekr1 = payc['UAH'];
			}else if($('#pay_t1').val() == 5) {
				//WME
				ekr1 = payc['EUR'];
			}else if($('#pay_t1').val() == 3) {
				//WMB
				ekr1 = payc['BYR'];
			}
			if(ekr1 != payc['RUB']) {
		...
		$('#pay_out').attr('disabled',true);
		$('#pay_comis').html(com);
	}
}

function open_pay(id) {
	if($('#pay_block_see').html() != '' && id == pay_now_id) {
		close_pay();
	}else{
		$('#pay_block_see').html(pay_bh[id]);
		$('#pay_block_see').fadeIn('slow');
		$('#pay_btn').fadeIn(2000);
		pay_resum(id);
		pay_now_id = id;
	}
...
function close_pay() {
	$('#pay_block_see').fadeOut('slow',false,function(){$('#pay_block_see').html('')});
	$('#pay_btn').fadeOut('fast');
}
</script>
<style>
fieldset {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="50%" valign="top"><table width="100%" cellspacing="5">
      <tr>
        <td width="50%" valign="top"><fieldset style="background-color:;">
         <!-- <legend><img src="http://img.likebk.com/i/align/align50.gif" width="12" height="15" /> <b style="color:#5F3710">Приобретение Екр. онлайн</b> </legend>
<a href="http://d.gamedealer.ru/widget/view/36249a3304b321d2daa3c78d17b6080e" target="_blank">Временная ссылка онлайн продаж (Нажмите для покупки Екр. онлайн через систему автоматических продаж)</a><br />
<table border="0" cellpadding="0" cellspacing="2" class="pay">
  <tr>
    <td><img src="e-payment/webmoney-white.png" title="Webmoney" onclick="open_pay(1);" width="50" height="30"></td>
    <td><img src="e-payment/yandexmoney.png" title="Яндекс.Деньги" onclick="open_pay(2);" width="50" height="30"></td>
    <td><img src="terminal/qiwi.png" title="QIWI" onclick="open_pay(3);" width="50" height="30"></td>
    <td><img src="e-payment/paypal.png" onclick="open_pay(4);" title="Pay Pal" width="50" height="30"></td>
    <td><img src="card/visa.png" onclick="open_pay(5);" title="VISA" width="50" height="30"></td>
    <td><img src="bank/alfabank-white.png" onclick="open_pay(8);" title="Альфабанк" width="50" height="30"></td>
    <td><img src="terminal/eleksnet.png" onclick="open_pay(9);" title="Eleksnet платежи" width="50" height="30"></td>
    <td><img src="e-payment/liqpay.png" onclick="open_pay(10);" title="LiqPay платежи" width="50" height="30"></td>
    <td><img src="other/sms.png" onclick="open_pay(7);" title="SMS Платежи" width="50" height="30"></td>
    <td><img src="card/mastercard.png" onclick="open_pay(6);" title="Mastercard" width="50" height="30"></td>
  </tr>
</table>
<style>
#pay_btn {
...
<div style="padding-top:5px;padding-bottom:5px;border-top:1px solid #AEAEAE;">
Сумма екр.: <input onkeydown="pay_resum(pay_now_id);" onkeyup="pay_resum(pay_now_id);" id="pay_in" style="padding-left:2px;width:77px;" value="0.00">
<input id="pay_btn" name="pay_btn" onclick="alert('Временно не работает!');" value="Оплатить" type="button" style="float:right;padding:5px;display:none;" />
</div>
<div id="pay_block_see" style="display:none;padding-top:5px;border-top:1px solid #AEAEAE;"></div>
</form>
<small style="color:#AEAEAE">* Указанные значения могут не совпадать с действительной суммой оплаты</small>
        </fieldset></td>
      </tr> 
      <tr>
