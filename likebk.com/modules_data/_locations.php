...
if( isset($_GET['nightatack'])) {
	//if( date('H') >= 22 || date('H') < 6 ) {
		if( 
			...
			$u->room['name'] == 'Западная окраина' ||
			$u->room['name'] == 'Восточная окраина'
		) {
			...
			if($u->room['noatack'] == 0) {
				...
				/*if( $ua['battle'] == 0 && $ua['align'] > 1 && $ua['align'] < 2 ) {
					$u->error = 'Нападать на &quot;крестопузов&quot; запрещено!';
				}else*/
				if(isset($ua['id']) && $ua['online'] > time()-520) {
					...
					if( $testb != '-1' ) {
						...
						$re = 'Нельзя вмешиваться в клановые войны!';
					}elseif( $u->info['level'] - 2 > $ua['level'] ) {
						$re = 'Персонаж слишком слаб для вас, подберите жертву покрупнее';
					}elseif( $usta['hpNow'] < $minHp ) {
						$re = 'Персонаж слишком слаб чтобы сражаться...';
						...
						echo 'Нельзя нападать в артефактах во время комендантского часа!';
					}elseif( ($u->info['room'] == 9 || $u->info['room'] == 323 || $u->info['room'] == 11 || $u->info['room'] == 213) && (
						...
						$re = 'Нападать на новичков запрещается! Сейчас придет Мироздатель и превратит тебя в лягушку...';
...
if(isset($_GET['loc']))
{
	...
	if( isset($sleep['id']) ) {
		$re = 'Передвигаться во время сна запрещено!';
	...
	}elseif( $u->room['block_all'] == 0 && $u->info['admin'] == 0 && (!isset($zadej) || $zadej == 0) && $u->aves['now']>=$u->aves['max'] && $u->room['name']!='Общежитие' && $u->room['name']!='Общ. Этаж 1' && $u->room['name']!='Общ. Этаж 2' && $u->room['name']!='Общ. Этаж 3')
	{
	...
	}elseif(isset($go['id']))
	{
		...
		}elseif($rmgo[$go['id']]==1 || $u->info['admin']>0)
		{
			...
			if( $u->info['dn'] > 0 && true == false ) {
				$re = 'Вы подали заявку в пещере!';
			}elseif( $u->info['inTurnirnew'] > 0 && true == false ) {
				$re = 'Вы находитесь в турнире!';
			...
			}elseif($mlvl[0]>$u->info['level'] && $u->info['admin']==0)
			{
				...
				$re = 'Твинком вход запрещен! Переключитесь на основного персонажа';
			}elseif($u->info['twink'] == 0 && $go['id'] == 428) {
				$re = 'Вход разрешен только Твинком';
...
if(isset($_GET['mAjax'])){
	...
	$rowonmax = '<span class="onlineusers">Сейчас в городе: <strong>'.$rowonmax2[0].' чел.</strong></span>';
	$rowonmax =  ''.$rowonmax.'';
...
function errortxt(){
	$('#moveto').css({'display':'','height':'auto'});
	$('#moveto').html('<div style="cursor:pointer;padding:5px;color: Red;"><b>Персонажам младше 8 уровня вход запрещен</b></div>');
}
...
if(isset($u->room['id'])){
	...
	$goLis = '<table height="15" border="0" cellspacing="0" cellpadding="0">
               <tr>
               <td id="locobobr" rowspan="3" valign="bottom"><a href="main.php?rnd='.$code.'"><img style="display:block;" src="http://img.likebk.com/i/move/rel_1.gif" width="15" height="16" title="Обновить" border="0" /></a></td>
	...
	if( /*($u->info['room'] == 9 || $u->info['room'] == 323 || $u->info['room'] == 11 || $u->info['room'] == 213) && */ 
		...
		//$rowonmax .= '<br><br><span style="cursor:help;font-size:11pt;" onMouseOver="top.hi(this,\'<b>Комендантский час:</b> <br> вторник 20:00-23:00 <br>среда 18:00-00:00 <br>четверг 20:00-23:00 <br>пятница 20:00-00:00 <br>суббота 18:00-00:00 <br> <br> <b>Описание:</b> На центральной площади, страшилкиной улице, восточной и западной окраинах действует комендантский час по расписанию. Во время его действия все нападения в артефактах на перечисленных улицах запрещены.<br>Время сервера: '.date('(w) H:i').'\',event,3,0,1,1,\'width:500px\')" onMouseOut="top.hic();" onMouseDown="top.hic();">Комендантский час <img src="http://likebk.com/qst.png" width="14"></span>';
	}
