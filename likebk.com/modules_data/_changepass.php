...
$u->info['pass2']=md5(array2HStr(md5m($pass2)));
		...
		$headers .= "From: Бойцовский Клую <support@likebk.com>\r\n";
	...
	if ($_POST['oldpass'] && $_POST['npass'] && $_POST['npass2']) {
		...
		if($u->info['pass'] == md5($_POST['oldpass'])) {
			if ($_POST['npass'] == $_POST['npass2']){
				...
				$headers .= "From: Бойцовский Клуб <support@likebk.com>\r\n";
				...
				$headers = stripslashes($headers);
					...
					$aa='<html>
							...
							<body>
								...
								http://'.$u->info['city'].'.likebk.com/confirm.php?id='.$u->info['id'].'&code='.$code.'<br>
								<br>
								--<br>
								Бойцовский Клуб http://www.likebk.com<br>
								Администрация Бойцовского Клуба: support@likebk.com<br>
				...
				$headers = stripslashes($headers);
					...
					$aa='<html>
							...
							<body>
								...
								http://likebk.com/confirm.php?id='.$u->info['id'].'&code='.$code.'<br>
								<br>
								--<br>
								Бойцовский Клуб http://www.likebk.com<br>
								Администрация Бойцовского Клуба: support@likebk.com<br>
...
if(isset($_POST['change2pass'])) {
	if( $u->info['2pass'] == '' || $u->info['2pass'] == '0' ) { 
		$u->info['2pass'] = 1;
	}else{
		$u->info['2pass'] = '';
	}
	mysql_query('UPDATE `users` SET `2pass` = "'.$u->info['2pass'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
}

if( $u->info['admin'] > 0 ) { ?>
<FORM ACTION="main.php?security" METHOD=POST>
<FIELDSET><LEGEND><B> Двухфакторная авторизация </B> </LEGEND>
<? if( $u->info['2pass'] == '' || $u->info['2pass'] == '0' ) { ?>
<TABLE>
	<TR><TD align=center colspan=2><INPUT TYPE=submit value="Включить двухфакторную авторизацию" name="change2pass"></TD></TR>
</TABLE>
<? }else{
...
if( $u->info['2pass'] == 1 ) {
	$u->info['2pass'] = $ga->generateSecret();
	mysql_query('UPDATE `users` SET `2pass` = "'.$u->info['2pass'].'" WHERE `id` = "'.$u->info['id'].'" LIMIT 1');
}

$time = floor((time() + 2.5*60) / 30);

$key1 =	$ga->getCode($u->info['2pass'],$time);

$url =  sprintf("otpauth://totp/%s?secret=%s", $u->info['id'].'@likebk.com', $u->info['2pass']);
...
?>
<br>
Отсканируйте QR-код чтобы добавить двухфакторную цепочку.

<TABLE>
	<TR><TD align=center colspan=2><INPUT TYPE=submit value="Выключить двухфакторную авторизацию" name="change2pass"></TD></TR>
</TABLE>
<? } ?>
</FIELDSET>
</FORM>
