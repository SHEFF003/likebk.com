...
while ($ir <= 3) {
	...
	$sp = mysql_query('SELECT `zv`.* FROM `zayvki` AS `zv` LEFT JOIN `users` AS `u` ON `u`.`id` = `zv`.`creator` WHERE (`u`.`pass` = "saintlucia" OR `u`.`level` = "8"  OR `u`.`level` = "9" OR `u`.`level` = "10") AND `zv`.`start`="0" AND `zv`.`cancel` = "0" AND `zv`.`noart` = "0"');
