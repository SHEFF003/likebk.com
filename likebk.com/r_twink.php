...
function  microLogin($id,$t,$nnz = 1) {
	...
	if($inf['clan']>0)
	{
		...
		if(isset($cln['id']))
		{
			$r .= '<img width="24" height="15" src="http://img.likebk.com/i/clan/'.$cln['name'].'.gif" />';
		}
	}
	if($inf['cityreg'] == '') {
		$inf['cityreg'] = 'capitalcity';
	}
	$r .= ' <b>'.$inf['login'].'</b> ['.$inf['level'].']<a target="_blank" href="http://likebk.com/inf.php?'.$inf['uid'].'"><img title="Инф. о '.$inf['login'].'" src="http://img.likebk.com/i/inf_'.$inf['cityreg'].'.gif" /></a>';	
	return $r;
}

?>
<HTML><HEAD><TITLE>Рейтинг Твинков</TITLE>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgColor=#3D3D3B>
  <tr valign=top>
	...
	<td align=center>
		...
		<td rowspan=2>
<br>
        <h3 style="font-size: 20px;">Рейтинг Твинков</h3>
...
$r = '';
$i = 1;
$sp = mysql_query('SELECT `id`,`uid`,`login`,`level`,`exp`,`align`,`clan` FROM `users_twink` WHERE `twink` > 0 ORDER BY `exp` DESC LIMIT 1000');
while( $user = mysql_fetch_array($sp) ) {
	if(!isset($user['id'])) {
		$r .= '<td height="20" style="font-size:10pt" align=center valign="top" class="mystrong">&nbsp;'.$i.'&nbsp;</td>';
		//login
		...
		$r .= '<td height="20" style="font-size:10pt" align=center valign="top" class="mystrong"><a name="id'.$pl['uid'].'">&nbsp;<b>'.$i.'.</b>&nbsp;</a></td>';
		//login
