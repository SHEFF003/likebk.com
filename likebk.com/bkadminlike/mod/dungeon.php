		...
		while(j<=17)
		{
			...
			if(pix!=undefined)
			{
				...
				if(thm!=undefined && thm[0]>0)
				{
					...
					pix.innerHTML = '<div style="height:32px;width:32px;cursor:pointer;" <? if($u->info['admin']>0 || $u->info['id']==1000010){  ?>oncontextmenu="bkadminlike('+thm[1]+','+thm[2]+',event); return false;"<? } ?> onclick="testGo('+nwx+','+nwy+');" id="content_'+j+'_'+i+'">'+objst+'</div>';
				}else{
					<? if($u->info['admin']>0 || $u->info['id']==1000010){ ?>
					pix.className = '';
					pix.innerHTML = '<div class="newpix" style="height:30px;width:30px;cursor:pointer;" <? if($u->info['admin']>0 || $u->info['id']==1000010){  ?>onClick="testGo('+nwx+','+nwy+')" oncontextmenu="bkadminlike('+nwx+','+nwy+',event); return false;"<? } ?>>'+objst+'</div>';
					<? }else{ ?>
					pix.className = '';
					pix.innerHTML = '';
					<? } ?>
...
{
	$.post('../../jx/dungeon.php',{x:xxx,y:yyy,ddid:<?=(int)$_GET['r']?>,id:<? echo time(); ?>,bkadminlike:1,action:act},function(data){$("#rd").html(data);});
}

function bkadminlike(x,y,event)
{
	...
	if(ed!=undefined)
	{
		...
		ed.style.display = '';
		ed.innerHTML += '<Br>&nbsp; X: '+x+',Y: '+y+' <a href="#" title="Закрыть редактор" onClick="closebkadminlike(); return false;">[x]</a>, ';
