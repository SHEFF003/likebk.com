		...
		if(isset($_POST['add_ekr'])) {
			if(is_numeric($_POST['add_ekr']) && $_POST['add_ekr'] > 0){
				if(isset($_POST['plus'])){
					...
					if($b){
						echo '<script>location.href="http://likebk.com/bkadminlike/index.php?mod=alhim&plus='.$_POST['add_ekr'].'"</script>';
					}
				}
				elseif(isset($_POST['minus'])){
					$b = mysql_query('UPDATE `bank_alh` SET `ekr` = `ekr` - "'.mysql_real_escape_string($_POST['add_ekr']).'" WHERE `uid` = "'.$usr['id'].'" LIMIT 1');
					if($b){
						echo '<script>location.href="http://likebk.com/bkadminlike/index.php?mod=alhim&minus='.$_POST['add_ekr'].'"</script>';
					}
				}
			}else{
				$err = 'Неверная сумма';
...
<form class="rep_pass" method="post" action="http://likebk.com/bkadminlike/index.php?mod=alhim">
<?
if($step == 1){ ?>
