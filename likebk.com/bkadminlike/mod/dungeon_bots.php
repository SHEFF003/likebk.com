...
<form method="post" action="../../РЕДАКТОРЫ/bkadminlike/mod/index.php?mod=dungeon_bots">
<table>
	<tbody>
		<tr>
			<td>По пещере:
...
<script>
	...
	function linkGif (th){
		$('#linkGo').attr('href', 'http://img.likebk.com/i/obraz/0/'+$('input[name=obraz]').val()).html('Open '+$(th).val()).click();
	}
	function linkPng (th){
		var t = $('input[name=obraz]').val(); t = t.split('.');
		$('#linkGo').attr('href', 'http://img.likebk.com/chars/0/'+t[0]+'.png').html('Open '+$(th).val()).click();
	}
</script>
<form method="post" action="../../РЕДАКТОРЫ/bkadminlike/mod/index.php?mod=dungeon_bots"><hr>
			...
			<td valign="top" width="55%" align="left">
				...
				<script>
					function loadItemInfo(th){
						$.post('/bkadminlike/lib/loadItemInfo.php', {item_id:$(th).parent('td').children('.items').val()}, function(data){
							$(th).html(data);
						});
					}
					
					function loadListItems(th){
						$.post('/bkadminlike/lib/loadItemInfo.php', {getListItems:true}, function(data){
							$(th).html(data);
						});
					}
					...
					$('#editUseItem').live('click', function() {
						if ($('#editorUseItem').html() != '') {
							...
							$.each(useItem, function( n, row ){
								text += '<tr><td><input class="items useItem" value="' + row + '" type="text" name="itemsId" size="7"> <input type="button" name="delete" class="delete" value="X"> <a href="javascript:void(0);" id="'+ row +'" onclick="loadItemInfo(this);">[load]</a> or <a target="_blank" href="http://lib.likebk.com/items_info.php?id='+ row +'" id="'+ row +'">[lib]</a>  <br/></td></tr>';
							});
