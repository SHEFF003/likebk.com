...
<div style="float: right; width: 210px;">
  <INPUT type='button' value='Обновить' onclick='location="/main.php?newybonus10=<?=$bns['id']?>"';'>
&nbsp;<INPUT TYPE=button value="Вернуться" onclick='location="/main.php"'>
</div>
<div id="wrapper">
<h3 style="font-size: 24px; font-weight: bold; padding: 20px; padding-bottom: 0px; margin-bottom: 10px; text-align: center;">Получи свою награду за <?=$bns['name2']?>!</h3>
<div style="text-align: center; font-family: Arial; line-height: 24px;">
<span style="font-size: 16px; font-weight: bold;"><br>
<small style="font-weight:normal;font-size:14px;">(Срок проведения марафона с <?=$bns['dd']?>.<?=$bns['mm']?>.<? if( $bns['mm'] > $bns['mm2'] ) { echo $bns['yy']-1; }else{ echo $bns['yy']; }?> по <?=$bns['dd2']?>.<?=$bns['mm2']?>.<?=$bns['yy2']?>)</small><br>
За каждый час проведенный в игре, вы сможете забрать <img src="http://img.likebk.com/i/items/<?=$itm1['img']?>"> "<?=$itm1['name']?>", в хаотах выпадает <img src="http://img.likebk.com/i/items/<?=$itm2['img']?>"> "<?=$itm2['name']?>", а в пещерах выпадают <img src="http://img.likebk.com/i/items/<?=$itm3['img']?>"> &quot;<?=$itm3['name']?>&quot;! Собрав необходимое количество, вы сможете поменять их на очень ценные призы<? if( $bns['loto'] > 0 ) { ?>, а также <font color="#b40611"><img src="http://img.likebk.com/i/items/loto1b.jpg"> лотерейные билеты</font>, с призовым фондом <font color="#b40611" style="font-size:19px;">более 1300 екр</font>.
<?
...
?>
&nbsp; и многое другое! </font><? } ?>. <? if( $bns['gift'] == 1 ) { ?>За каждые 100  часов в онлайне вы получаете право на бесплатный <font color="#b40611">именной подарок</font> или <font color="#b40611">смайлик</font>!<? } ?><br>
Получить вышеперечисленные вещи вы можете в <font color="#b40611">&quot;Сезонный Магазин&quot;</font>, который находится на <font color="#b40611">&quot;Центральной Площади&quot; <? if( date('m') == 12 || date('m') < 2 ) { echo ' под Елкой в <font color="#b40611">&quot;Новогоднем магазине&quot;</font>!'; } ?></font>.
<?
...
if( $bns['gift'] == 1 ) {
	if( $xps[0] >= 100 ) {
		echo '<div><a href="javascript:void(0);" onclick="if(confirm(\'Вы уверены что хотите приобрести именной подарок?\')){ location.href = \'/main.php?newybonus10='.$bns['id'].'&obmen123=1\'; } return false;"><span class="button" style="pointer-events: none; cursor: default;">Обменять 100 &quot;'.$itm1['name'].'&quot; на именной подарок!</span></a></div>';
		echo '<div><a href="javascript:void(0);" onclick="if(confirm(\'Вы уверены что хотите приобрести именной смайлик?\')){ location.href = \'/main.php?newybonus10='.$bns['id'].'&obmen123=2\'; } return false;"><span class="button" style="pointer-events: none; cursor: default;">Обменять 100 &quot;'.$itm1['name'].'&quot; на именной смайлик!</span></a></div>';
	}else{
		$xcfasd = 100 - $xps[0];
		echo '<div style="pointer-events: none; cursor: default;opacity: 0.2;"><b><font color="red">Для получения бесплатного подарка или смайлика осталось еще '.$xcfasd.' &quot;'.$itm1['name'].'&quot;!</font></b></div><hr>';
	}
}
?>
...
if(isset($_GET['takeitnowfastbig'])) {
	if( $o1['time_all'] - $o2['time_online'] >= 3600 ) {
		...
		echo '<div><font color="red"><b>Открыв сундук вы обнаружили предмет &quot;'.$itm1['name'].'&quot;!</b></font></div>';
...
if( $o1['time_all'] - $o2['time_online'] >= 3600 ) {
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=1"><span class="button" style="pointer-events: none; cursor: default;">Открыть сундук №1</span></a>';
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=2"><span class="button" style="pointer-events: none; cursor: default;">Открыть сундук №2</span></a>';
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=3"><span class="button" style="pointer-events: none; cursor: default;">Открыть сундук №3</span></a>';
}else{
	echo 'Вам необходимо провести в онлайне еще '.$u->timeOut(( 3600 - ($o1['time_all'] - $o2['time_online']) )).'!';
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=1"><span class="button" style="pointer-events: none; cursor: default;opacity: 0.2;">Открыть сундук №1</span></a>';
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=2"><span class="button" style="pointer-events: none; cursor: default;opacity: 0.2;">Открыть сундук №2</span></a>';
	echo '<a href="/main.php?newybonus10='.$bns['id'].'&takeitnowfastbig=3"><span class="button" style="pointer-events: none; cursor: default;opacity: 0.2;">Открыть сундук №3</span></a>';
}	
