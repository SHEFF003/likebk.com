...
function e($t) {
	mysql_query('INSERT INTO `chat` (`text`,`city`,`to`,`type`,`new`,`time`) VALUES ("core #'.date('d.m.Y').' %'.date('H:i:s').' ( ритическа€ ошибка): <b>'.mysql_real_escape_string($t).'</b>","capitalcity","ѕовелитель Ѕагов","6","1","-1")');
}
//e('['.$_COOKIE['login'].'] я обращаюсь к файлу reflesh_bot.php');
...
if(isset($_GET['cron_core'])) {
	...
	//if(md5($id['id'].'_brfCOreW@!_'.$id['pass']) == $_GET['cron_core']) {
		$uzr = mysql_fetch_array(mysql_query('SELECT `id`,`login`,`pass` FROM `users` WHERE `id` = "'.mysql_real_escape_string($id['id']).'" LIMIT 1'));
		if(isset($uzr['id'])) {
			$CRON_CORE = true;
